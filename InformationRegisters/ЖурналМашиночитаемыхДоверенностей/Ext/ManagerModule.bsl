...
Функция НовыйДоверитель(Тип, ДанныеЗаполнения = Неопределено) Экспорт
	...
	ОписаниеДоверителя.Вставить("ДанныеФилиалаЮридическогоЛица", Неопределено);
	...
	ИначеЕсли Тип = ТипыДоверителей.ИностраннаяОрганизация
		...
		КонецЕсли;
	ИначеЕсли Тип = ТипыДоверителей.ФилиалЮридическогоЛица
		Или Тип = ТипыПредставителей.ФилиалЮридическогоЛица Тогда
		ОписаниеДоверителя.ДанныеФилиалаЮридическогоЛица = НовыйФилиалОрганизации();
		Если ЗначениеЗаполнено(ДанныеЗаполнения) Тогда
			ЗаполнитьЗначенияСвойств(ОписаниеДоверителя.ДанныеФилиалаЮридическогоЛица, ДанныеЗаполнения);
		КонецЕсли;
	...
	ИначеЕсли Тип = ТипыДоверителей.ФилиалЮридическогоЛица
		Или Тип = ТипыПредставителей.ФилиалЮридическогоЛица Тогда
		ОписаниеДоверителя.ДанныеФилиалаЮридическогоЛица = НовыйФилиалЮридическогоЛица();
		Если ЗначениеЗаполнено(ДанныеЗаполнения) Тогда
			ЗаполнитьЗначенияСвойств(ОписаниеДоверителя.ДанныеФилиалаЮридическогоЛица, ДанныеЗаполнения);
		КонецЕсли;
	КонецЕсли;
...
// 
// Возвращаемое значение:
//  Структура -  Новый филиал юридического лица:
// * Наименование - Строка 
// * ИНН - Строка 
// * КПП - Строка 
// * ОГРН - Строка 
// * Руководитель - См. РегистрыСведений.ЖурналМашиночитаемыхДоверенностей.НовоеФизическоеЛицо
Функция НовыйФилиалЮридическогоЛица() Экспорт
	ФилиалЮридическогоЛица = Новый Структура;
	ФилиалЮридическогоЛица.Вставить("Наименование", "");
	ФилиалЮридическогоЛица.Вставить("ИНН", "");
	ФилиалЮридическогоЛица.Вставить("КПП", "");
	ФилиалЮридическогоЛица.Вставить("ОГРН", "");
	ФилиалЮридическогоЛица.Вставить("Руководитель", НовоеФизическоеЛицо());
	Возврат ФилиалЮридическогоЛица;
КонецФункции
...
// * ИНН - Строка 
// * КПП - Строка 
// * РуководителиФилиала - Массив Из См. РегистрыСведений.ЖурналМашиночитаемыхДоверенностей.НовоеФизическоеЛицо
Функция НовыйФилиалОрганизации()
	ФилиалОрганизации = Новый Структура;
	ФилиалОрганизации.Вставить("Наименование", "");
	ФилиалОрганизации.Вставить("ИНН", "");
	ФилиалОрганизации.Вставить("КПП", "");
	ФилиалОрганизации.Вставить("РуководителиФилиала", Новый Массив);
	Возврат ФилиалОрганизации;
КонецФункции 
