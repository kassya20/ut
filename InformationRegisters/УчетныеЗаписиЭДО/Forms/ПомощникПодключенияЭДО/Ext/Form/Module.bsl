...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ПроверитьПометкуУдаленияДоверенности(ИмяСобытия, Параметр);
...
Процедура СертификатЭПЧерезОператораОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ВыбранноеЗначение) И Не ВидДобавляемогоСертификатаСоответствуетВидуОрганизации(
		ВыбранноеЗначение, Организация, ТекстСообщения) Тогда
		СтандартнаяОбработка = Ложь;
		Ошибки.Добавить(ТекстСообщения);
	КонецЕсли;
...
&НаКлиенте
Процедура КППОрганизацииПриИзменении(Элемент)
	
	ЭтоФилиалИностраннойОрганизации =
		ОбщегоНазначенияБЭДКлиентСервер.ЭтоКППФилиалаИностраннойОрганизации(КППОрганизации);
	УправлениеФормой(ЭтотОбъект);
...
Процедура УправлениеФормой(Форма)
	...
	Если ЗначениеЗаполнено(Форма.Организация)
		...
		И (Не Форма.ИспользоватьЦентрЭДО Или ЗначениеЗаполнено(Форма.УчетнаяЗаписьЦентраЭДО)) Тогда
	...
	Элементы.КППОрганизации.Видимость = Форма.ЭтоОрганизацияЮЛ;
	Элементы.ОГРНОрганизации.Заголовок = ?(Форма.ЭтоОрганизацияЮЛ, НСтр("ru = 'ОГРН'"), НСтр("ru = 'ОГРНИП'"));
	Элементы.ОГРНОрганизации.Видимость = Не Форма.ЭтоФилиалИностраннойОрганизации;
	
	Элементы.УчетнаяЗаписьЦентраЭДО.Видимость = Форма.ИспользоватьЦентрЭДО;
...
&НаСервере
Функция ВидДобавляемогоСертификатаСоответствуетВидуОрганизации(Сертификат, Организация, СообщениеОбОшибке)
	
	Возврат УчетныеЗаписиЭДОСлужебный.ВидДобавляемогоСертификатаСоответствуетВидуОрганизации(Сертификат, Организация, 
		СообщениеОбОшибке);
...
Процедура УстановитьЗначенияПоУмолчанию()
	...
	Если Операция = Неопределено И ЗначениеЗаполнено(Организация) Тогда
		...
		ПараметрыДанных.УчетнаяЗаписьЦентраЭДО = УчетнаяЗаписьЦентраЭДО;
...
Процедура ЗаполнитьРеквизитыОрганизацииДляРегистрационногоПакета()
	...
	ЭтоОрганизацияЮЛ = Не ИнтеграцияЭДО.ЭтоФизЛицо(Организация);

	НаименованиеОрганизации = РеквизитыОрганизации.Наименование;
	ИННОрганизации = РеквизитыОрганизации.ИНН;
	КППОрганизации = РеквизитыОрганизации.КПП;
	ЭтоФилиалИностраннойОрганизации =
		ОбщегоНазначенияБЭДКлиентСервер.ЭтоКППФилиалаИностраннойОрганизации(КППОрганизации);
	ОГРНОрганизации = РеквизитыОрганизации.ОГРН;
...
Функция ПроверкаЗаполнения()
	...
	Если Не ЗначениеЗаполнено(НаименованиеОрганизации) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			ОбщегоНазначенияБЭДКлиентСервер.ТекстСообщения("Поле", "Заполнение",
				НСтр("ru = 'Наименование организации'")),, "НаименованиеОрганизации",, Отказ);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ИННОрганизации) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			ОбщегоНазначенияБЭДКлиентСервер.ТекстСообщения("Поле", "Заполнение",
				НСтр("ru = 'ИНН организации'")),, "ИННОрганизации",, Отказ);
	КонецЕсли;
	
	Если ЭтоОрганизацияЮЛ И Не ЗначениеЗаполнено(КППОрганизации) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			ОбщегоНазначенияБЭДКлиентСервер.ТекстСообщения("Поле", "Заполнение",
				НСтр("ru = 'КПП организации'")),, "КППОрганизации",, Отказ);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ОГРНОрганизации) И Не ЭтоФилиалИностраннойОрганизации Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			ОбщегоНазначенияБЭДКлиентСервер.ТекстСообщения("Поле", "Заполнение",
				НСтр("ru = 'ОГРН организации'")),, "ОГРНОрганизации",, Отказ);
	КонецЕсли;
	...
	Если ИспользоватьЦентрЭДО И Не ЗначениеЗаполнено(УчетнаяЗаписьЦентраЭДО) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
				ОбщегоНазначенияБЭДКлиентСервер.ТекстСообщения("Поле", "Заполнение",
					НСтр("ru = 'Учетная запись центра ЭДО'")),,"УчетнаяЗаписьЦентраЭДО",,Отказ);
	КонецЕсли;
...
Процедура ЗарегистрироватьСертификат1СЭДО()
	...
	ПараметрыРегистрации.НаименованиеОрганизации = НаименованиеОрганизации;
	ПараметрыРегистрации.ИННОрганизации = ИННОрганизации;
	ПараметрыРегистрации.КППОрганизации = КППОрганизации;
	ПараметрыРегистрации.ОГРНОрганизации = ОГРНОрганизации;
...
Процедура ЗарегистрироватьСертификатЧерезЦентрЭДО()
	...
	Если ЗначениеЗаполнено(ДолжностьПоСертификату) Тогда
		ПараметрыРегистрации.Должности.Вставить(СертификатКриптографии, ДолжностьПоСертификату);
	КонецЕсли;
	Если ТипРегистрации = ТипРегистрацииСуществующийИдентификатор() Тогда
		ПараметрыРегистрации.ИдентификаторОрганизации = ИдентификаторОрганизацииСуществующий;
	КонецЕсли;
	ПараметрыРегистрации.Назначение = НазначениеУчетнойЗаписи;
	ПараметрыРегистрации.ПодробноеОписание = ПодробноеОписаниеУчетнойЗаписи;
...
Процедура ВыполнитьОтправкуДанныхВСервисНастроек()
	...
	Если ИспользоватьЦентрЭДО Тогда
		Оповещение = Новый ОписаниеОповещения("ОтправкаДанныхВСервисНастроекЧерезЦентрЭДОЗавершение", ЭтотОбъект, ИдентификаторОрганизации);
...
Функция СохранитьУчетнуюЗапись()
	...
	Если ЗначениеЗаполнено(СертификатКриптографии) Тогда
		...
		КонецЕсли;
		Если ЗначениеЗаполнено(ДолжностьПоСертификату) Тогда
			ОписаниеУчетнойЗаписи.Должности.Вставить(СертификатКриптографии, ДолжностьПоСертификату);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьНеобходимостьДоверенности()
	...
	Если ТребуетсяДоверенность Тогда
		...
		ПустаяСсылка = Справочники.МЧД003.ПустаяСсылка();
		
		НовыйОтборМЧД = МашиночитаемыеДоверенности.НовыйОтборМЧД();
		НовыйОтборМЧД.Доверитель = Организация;
		НовыйОтборМЧД.Сертификат = СертификатКриптографии;
		Доверенности = МашиночитаемыеДоверенности.ПолучитьДоверенностиОрганизации(НовыйОтборМЧД);
					
		Если Доверенности.Количество() = 1 Тогда
			Доверенность = Доверенности[0];
			МашиночитаемыеДоверенностиВызовСервера.ТипПоддержанОператором(ОператорЭДО, ТипЗнч(Доверенность));
		ИначеЕсли Доверенности.Количество() = 0 Тогда
			Доверенность = ПустаяСсылка;
		ИначеЕсли Доверенности.Найти(Доверенность) = Неопределено Тогда
			Доверенность = ПустаяСсылка;	
		КонецЕсли;
...
Процедура ПолучитьДолжностьПоСертификату()
	...
	Если УчетныеЗаписиЭДОСлужебный.ТребуетсяЗаполнитьДолжностьДляСертификата(Организация, СертификатКриптографии) Тогда
		ДолжностьПоСертификату = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СертификатКриптографии, "Должность");
	Иначе
		ДолжностьПоСертификату = "";
	КонецЕсли;
...
Функция НовыйПараметрыОткрытияФормыВыбораДоверенности()
	...
	Результат.Вставить("ДополнительныеПараметры", 
		Новый Структура("Организация, СертификатКриптографии", Организация, СертификатКриптографии));
...
&НаКлиенте
Процедура ПроверитьПометкуУдаленияДоверенности(Знач ИмяСобытия, Знач Параметр)
	
	Если Не ИмяСобытия = МашиночитаемыеДоверенностиКлиент.ИмяОповещенияИзмененаПометкаУдаленияМЧД() Тогда
		Возврат;
	КонецЕсли;	
	
	Если ЗначениеЗаполнено(Доверенность)
		И Параметр.Доверенности.Найти(Доверенность) = Неопределено Тогда
		Возврат;	
	КонецЕсли;		
	
	ДоверенностьДействует = ДоверенностьДействительнаПоСвойствам(Доверенность);
	Если Не ДоверенностьДействует Тогда
		Доверенность = ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Справочник.МЧД003.ПустаяСсылка");
	КонецЕсли;
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ДоверенностьДействительнаПоСвойствам(Знач Доверенность)
	
	Возврат МашиночитаемыеДоверенности.ПроверитьДоверенностьПоСвойствам(Доверенность);
