...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Оповестить(УчетныеЗаписиЭДОКлиент.ИмяСобытияСозданаУчетнаяЗаписьЭДО(), Запись.Организация);
КонецПроцедуры
...
Процедура СписокСертификатовВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Элементы.СписокСертификатов.ТекущийЭлемент = Элементы.СписокСертификатовДолжность Тогда
...
Процедура ЗаполнитьСписокСертификатов()
	...
	Запрос.Текст =
		...
		|	СертификатыУчетныхЗаписей.Должность
		|ИЗ
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			Должности.Вставить(ВыборкаДетальныеЗаписи.Сертификат, ВыборкаДетальныеЗаписи.Должность);
		КонецЦикла;
	Иначе
		Возврат;
	КонецЕсли;
	...
	Для Каждого СвойстваСертификата Из СвойстваСертификатов Цикл
		...
		НоваяСтрока.Должность = Должности.Получить(СвойстваСертификата.Значение.Ссылка);
		НоваяСтрока.ДолжностьПоСертификату = СвойстваСертификата.Значение.Должность;
		НоваяСтрока.ТребуетсяЗаполнитьДолжность = 
			УчетныеЗаписиЭДОСлужебный.ТребуетсяЗаполнитьДолжностьДляСертификата(Запись.Организация, 
...
Процедура ДобавитьСертификатВСписок(Сертификат)
	...
	СтрокаСертификата.ДолжностьПоСертификату = СвойстваСертификатов[Сертификат].Должность;
	СтрокаСертификата.ТребуетсяЗаполнитьДолжность = 
		УчетныеЗаписиЭДОСлужебный.ТребуетсяЗаполнитьДолжностьДляСертификата(Запись.Организация, Сертификат);
...
Процедура ЗаписатьСертификатыУчетнойЗаписи(Отказ)
	...
	Должности = Новый Соответствие;
	Для Каждого СтрокаСертификата Из СписокСертификатов Цикл
		
		Если Не ЗначениеЗаполнено(СтрокаСертификата.Должность) Тогда
			Продолжить;
		КонецЕсли;
		
		Должности.Вставить(СтрокаСертификата.Ссылка, СтрокаСертификата.Должность);
		
	КонецЦикла;
	
	УчетныеЗаписиЭДОСлужебный.ЗаписатьСертификатыУчетнойЗаписи(ИсходныйИдентификаторЭДО, СертификатыДляЗаписи,
		Ложь, Неопределено, Должности);
