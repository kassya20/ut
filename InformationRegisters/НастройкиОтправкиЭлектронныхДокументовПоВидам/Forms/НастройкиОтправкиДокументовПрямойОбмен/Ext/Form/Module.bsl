...
Функция ЗаписатьНастройкиНаСервере(КонтекстДиагностики)
	...
	Если ВсеПустыеЗначенияДоговораСКонтрагентомЭДО.Количество() > 1 И Не ЗначениеЗаполнено(Договор) Тогда
	...
	Запрос.УстановитьПараметр("Получатель", Контрагент);
	Если ЗначениеЗаполнено(Договор) Тогда
		Запрос.УстановитьПараметр("Договора", Договор);
	Иначе
	...
	Попытка
		...
		Если Договор = Неопределено И ВсеПустыеЗначенияДоговораСКонтрагентомЭДО.Количество() > 1 Тогда
			ЭлементБлокировки.ИсточникДанных = ИсточникБлокировкиПустыхДоговоров;
		...
		Если Не ЭтоНоваяЗапись И Выборка.Количество() > 1 Тогда
			Пока Выборка.Следующий() Цикл
				Если Выборка.Договор = Договор Тогда
					Продолжить;
				КонецЕсли;
		...
		Если КлючНастройкиОтправкиПустой(ЭтотОбъект) Или ЭтоНоваяЗапись Тогда
		...
		Если НаборЗаписей.Количество() = 0 И Договор = Неопределено Тогда
			Для Каждого ПустойДоговор Из ВсеПустыеЗначенияДоговораСКонтрагентомЭДО  Цикл
				Если ПустойДоговор = Договор Тогда
					Продолжить;
				КонецЕсли;
...
Функция КлючНастройкиОтправкиИзменен()
	...
	Если Договор = Неопределено И Не ЗначениеЗаполнено(КлючНастройкиОтправки.Договор) Тогда
...
Функция ЕстьОшибкиНастройкиТранспорта(Выводить = Ложь)
	...
	Иначе
		...
		ПроверитьСовпадениеАдресов(ЕстьОшибки);
	КонецЕсли;
...
&НаСервере
Процедура ПроверитьСовпадениеАдресов(ЕстьОшибки)
	
	Если ЗначениеЗаполнено(АдресПолучателя) И АдресПолучателя = АдресОтправителя Тогда
		ЕстьОшибки = Истина;
		ТекстСообщения = НСтр("ru = 'Каталоги отправителя и получателя не могут совпадать.'");
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, , Элементы.АдресПолучателя);
	КонецЕсли;
