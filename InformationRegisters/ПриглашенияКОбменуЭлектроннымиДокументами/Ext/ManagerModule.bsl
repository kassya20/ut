...
Процедура УдалитьПоКлючу(Ключ, ИдентификаторЭДО = "") Экспорт
	НаборЗаписей = СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.Ключ.Установить(Ключ);
	Если ЗначениеЗаполнено(ИдентификаторЭДО) Тогда
		НаборЗаписей.ДополнительныеСвойства.Вставить("ИдентификаторЭДО", ИдентификаторЭДО);
	КонецЕсли;
...
Процедура ОбработкаПолученияФормыЗаписи(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	ВыбраннаяФорма = ФормаПриглашенияПоВариантуИнтерфейса();
...
Процедура ОбработкаПолученияФормыОтправкиПриглашения(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	Если МассивКонтрагентов.Количество() = 1 Тогда
		...
		Если ИнтеграцияЭДО.ТипУчастникаЭДО(МассивКонтрагентов[0]).ЭтоИностранноеЛицо Тогда
			ИНН = ИнтеграцияЭДО.КлючевыеРеквизитыКонтрагента(МассивКонтрагентов[0]).ИНН;
			Если НЕ ЗначениеЗаполнено(СтрЗаменить(ИНН, "0", "")) Тогда
				ВыбраннаяФорма = "ПриглашениеИностранногоКонтрагента";
			КонецЕсли;
		Иначе
...
//  Строка - Путь к форме помощника отправки приглашения
Функция ФормаПриглашенияПоВариантуИнтерфейса()
	...
	ВыбраннаяФорма = ПриглашенияЭДОКлиентСервер.ФормаПриглашенияПоВариантуИнтерфейса(ВариантИнтерфейса);
