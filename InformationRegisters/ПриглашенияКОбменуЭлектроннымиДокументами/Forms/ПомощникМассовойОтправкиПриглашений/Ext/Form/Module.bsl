...
Функция ЗаполнитьПараметрыСвязиИдентификаторовЭДОПослеСинхронизацииАбонентов()
	...
	ПараметрыПолученияСвязи = ПриглашенияЭДО.НовыеПараметрыПолученияСвязиИдентификаторов();
	...
	ПараметрыПолученияСвязи.ПриоритетПодключенные = Истина;
	
	Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения,
		"ПриглашенияЭДО.ПараметрыСвязиИдентификаторов", ПараметрыПолученияСвязи,, Истина);
...
Процедура ЗаполнитьРеквизитыПриглашения(ТекущиеДанные, ПараметрыСвязи)
	...
	ТекущиеДанные.ИспользоватьЦентрЭДО = ПараметрыСвязи.ИспользоватьЦентрЭДО;
...
Процедура ЗаполнитьПредупреждениеПриглашения(ТекущиеДанные, ПараметрыСвязиПолучены);
	...
	Если НЕ ЗначениеЗаполнено(СтрЗаменить(ТекущиеДанные.ИНН, "0", "")) Тогда
		
		Если ИнтеграцияЭДО.ТипУчастникаЭДО(ТекущиеДанные.Контрагент).ЭтоИностранноеЛицо Тогда
			ТекущиеДанные.Предупреждение = НСтр("ru = 'Иностранный контрагент. Подключение через техподдержку.'");
		Иначе
