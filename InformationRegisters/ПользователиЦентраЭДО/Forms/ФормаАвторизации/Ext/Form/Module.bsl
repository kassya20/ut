...
// Параметры:
//  Форма - См. РегистрСведений.ПользователиЦентраЭДО.Форма.ФормаАвторизации
...
Процедура ПриИзмененииУчетнойЗаписи()
	...
	ЭтоСервис1СФреш = Справочники.УчетныеЗаписиЦентровЭДО.ЭтоСервис1СФреш(АдресСервиса);
...
Функция АвторизоватьсяВФоне(Знач ИдентификаторПриложения = Неопределено)
	...
	ПараметрыПользователя = РегистрыСведений.ПользователиЦентраЭДО.НовыеПараметрыПользователя();
	...
	ИначеЕсли ИдентификаторПриложения = Неопределено Тогда
		ПараметрыПользователя.АдресСервиса = Справочники.УчетныеЗаписиЦентровЭДО.АдресСервиса1СФреш();
	Иначе
	...
	// См. РегистрыСведений.ПользователиЦентраЭДО.Авторизоваться
	ДлительнаяОперация = ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения,
		"РегистрыСведений.ПользователиЦентраЭДО.Авторизоваться", ПараметрыПользователя, ЭтоНоваяУчетнаяЗапись);
...
Процедура АвторизоватьсяЗавершение(ДлительнаяОперация, ПараметрыПользователя) Экспорт
	...
	ИначеЕсли ДлительнаяОперация.Статус <> "Выполнено" Тогда
		...
		Иначе
			ТекстОшибки = НСтр("ru = 'Не удалось подключиться к центру ЭДО. Повторите попытку позже.'");
		КонецЕсли;
		НастроитьОтображениеОшибки(ЭтотОбъект, ТекстОшибки);
...
Функция ОбработатьРезультатАвторизации(Знач АдресРезультата)
	...
	РезультатАвторизации = ПолучитьИзВременногоХранилища(АдресРезультата);// См. РегистрыСведений.ПользователиЦентраЭДО.Авторизоваться
...
Функция НавигационнаяСсылкаЗабылиПароль()
	Возврат Справочники.УчетныеЗаписиЦентровЭДО.АдресСервиса1СФреш() + "/recover";
КонецФункции
