...
Функция РезультатПроверкиКМ(КодМаркировки) Экспорт 
	...
	РезультатПроверки.Вставить("ЗапросКМ");
	...
	Запрос.Текст = 
		...
		|	РегистрСведений.ОперацииПроверкиКМ КАК ОперацииПроверкиКМ
		|ГДЕ
		|	НАЧАЛОПЕРИОДА(ОперацииПроверкиКМ.Дата, ДЕНЬ) = &Дата
		|	И ОперацииПроверкиКМ.КодМаркировки = &КодМаркировки";
	...
	Выборка = РезультатЗапроса.Выбрать();
	Пока Выборка.Следующий() Цикл       
		РезультатПроверки.Результат = Истина;   
		ЗапросКМXML = Выборка.ЗапросКодаМаркировки.Получить();
		ЗапросКМ = МенеджерОборудованияМаркировка.ЗапросКМИзXMLПакета(ЗапросКМXML);  
		РезультатПроверки.ЗапросКМ = ЗапросКМ;
		ПланируемыеСтатусы = Перечисления.ПланируемыйСтатусМаркируемогоТовара.ПланируемыеСтатусыМаркируемогоТовараПоКоду();
		РезультатПроверки.Вставить("ПланируемыйСтатус", ПланируемыеСтатусы.Получить(ЗапросКМ.ПланируемыйСтатусТовара)); 
	КонецЦикла;          
