...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли СвУпПред[0].ТипПред = ТипыПредставителей.ИндивидуальныйПредприниматель Тогда
		...
		УстановитьВидимостьПоляСрокДействия(Элементы.СвУпПредПредСведИПСведФЛУдЛичнФЛСрокДействия,
			СвУпПред[0].Пред[0].СведИП[0].СведФЛ[0].УдЛичнФЛ[0].КодВидДок);
	Иначе
		ОтображаемаяГруппа = Элементы.ГруппаПредставитель_ФизическоеЛицо;
		УстановитьВидимостьПоляСрокДействия(Элементы.СвУпПредПредСведФизЛСведФЛУдЛичнФЛСрокДействия,
			СвУпПред[0].Пред[0].СведФизЛ[0].СведФЛ[0].УдЛичнФЛ[0].КодВидДок);
	КонецЕсли;
...
Процедура СвУпПредПредСведФизЛСведФЛУдЛичнФЛКодВидДокПриИзменении(Элемент)
	...
	МашиночитаемыеДоверенностиКлиент.УстановитьОформлениеНастройкиОтображенияИЗначенияПолейДУЛФЛПоУмолчанию(
		...
		Элементы.СвУпПредПредСведФизЛСведФЛУдЛичнФЛСрокДействия);
...
Процедура СвУпПредПредСведФизЛСведФЛУдЛичнФЛКодВидДокОчистка(Элемент, СтандартнаяОбработка)
	...
	МашиночитаемыеДоверенностиКлиент.УстановитьОформлениеНастройкиОтображенияИЗначенияПолейДУЛФЛПоУмолчанию(
		Элемент.Родитель,
		ПоляУдостоверяющиеЛичностьФизическогоЛица,
		Элементы.СвУпПредПредСведИПСведФЛУдЛичнФЛСрокДействия);
