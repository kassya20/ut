...
Процедура ПриЗаписи(Отказ)
	...
	Если Подписана Тогда
		...
		ТребуетсяСоздатьПравила = Ложь;
		Если ДополнительныеСвойства.ЭтоСозданиеНового Тогда
			ТребуетсяСоздатьПравила = Истина;
		Иначе
			ПравилоПроверки = РегистрыСведений.ПравилаПроверкиПолномочийПоМЧД.ПравилоПроверки(Ссылка);
			ТребуетсяСоздатьПравила = Не ЗначениеЗаполнено(ПравилоПроверки.Правило);
		КонецЕсли;
		
		Если ТребуетсяСоздатьПравила Тогда
			Справочники.ПравилаПроверкиПолномочийМЧД.СоздатьТиповыеПравилаПриЗагрузкеМЧД(Ссылка);
		КонецЕсли;
		
	КонецЕсли;
	
	ПараметрыСинхронизации = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
		ДополнительныеСвойства, "ПараметрыСинхронизацииСЦентромЭДО"); // См. ИнтеграцияЦентраЭДО.НовыеПараметрыСинхронизацииОбъекта
	Если ЗначениеЗаполнено(ПараметрыСинхронизации) Тогда
		МодульИнтеграцияЦентраЭДО = ОбщегоНазначения.ОбщийМодуль("ИнтеграцияЦентраЭДО");
		МодульИнтеграцияЦентраЭДО.УстановитьПараметрыСинхронизацииОбъекта(Ссылка, ПараметрыСинхронизации);
	КонецЕсли;
...
Процедура ЗаполнитьПоДаннымФайла(ДанныеФайла) Экспорт
	
	РезультатЧтенияДанных = Справочники.МЧД003.ОбъектXDTOМЧДИзДвоичныхДанных(ДанныеФайла);
	
	ДанныеДляЗаполнения = Справочники.МЧД003.ДанныеДляЗаполненияМЧД(РезультатЧтенияДанных.Доверенность);
	
	ДанныеДляЗагрузки = МашиночитаемыеДоверенностиКлиентСервер.НовыеДанныеДляЗагрузкиМЧД();
	ДанныеДляЗагрузки.ДанныеДоверенности = ДанныеФайла;
	
	Справочники.МЧД003.ПерезаполнитьОбъектМЧД(ЭтотОбъект, ДанныеДляЗаполнения, ДанныеДляЗагрузки);
...
Процедура ЗаполнитьДоверителейВЗаписиЖурнала(СведенияФайла, Запись)
	...
	Для Каждого СведенияОДоверителе Из СписокДоверителей Цикл
		...
		ИначеЕсли СведенияОДоверителе.ТипПерПолн = КлассификаторФормата.ФилиалЮридическогоЛица Тогда

			Доверитель = Журнал.НовыйДоверитель(КлассификаторЖурнала.ФилиалЮридическогоЛица);
			СведенияОбОрганизации = СведенияОДоверителе.ПередПолн.ФилПерПолн.СвФил; // См. XDTOПакет.ON_EMCHD_1_928_00_01_01_01.СвОргТип
			Доверитель.ДанныеФилиалаЮридическогоЛица.Наименование = Строка(СведенияОбОрганизации.НаимОрг);
			Доверитель.ДанныеФилиалаЮридическогоЛица.ИНН = Строка(СведенияОбОрганизации.ИННЮЛ);
			Доверитель.ДанныеФилиалаЮридическогоЛица.КПП = Строка(СведенияОбОрганизации.КПП);
			Доверитель.ДанныеФилиалаЮридическогоЛица.ОГРН = Строка(СведенияОбОрганизации.ОГРН);
			
			СведенияОРуководителеФилиала = СведенияОДоверителе.ПередПолн.ФилПерПолн.СвРукФил; // СписокXDTO Из См. XDTOПакет.ON_EMCHD_1_928_00_01_01_01.СвФЛТип
			Доверитель.ДанныеФилиалаЮридическогоЛица.Руководитель.ФИО 
				= ОбщегоНазначенияБЭДКлиентСервер.ФИОСтрокой(СведенияОРуководителеФилиала.СведФЛ.ФИО);
			Доверитель.ДанныеФилиалаЮридическогоЛица.Руководитель.ИНН = Строка(СведенияОРуководителеФилиала.ИННФЛ);
			Доверитель.ДанныеФилиалаЮридическогоЛица.Руководитель.СНИЛС = Строка(СведенияОРуководителеФилиала.СНИЛС);
...
Процедура ЗаполнитьДоверителейПередоверияВЗаписиЖурнала(СведенияФайла, Запись)
	...
	Для Каждого СведенияОДоверителе Из СписокДоверителей Цикл
		...
		ИначеЕсли СведенияОДоверителе.ТипПерПолн = КлассификаторФормата.ФилиалЮридическогоЛица Тогда
			...
			Доверитель = Журнал.НовыйДоверитель(КлассификаторЖурнала.ФилиалЮридическогоЛица);
			СведенияОбОрганизации = СведенияОДоверителе.ПередПолн.ФилПерПолн.СвФил; // См. XDTOПакет.ON_EMCHD_1_928_00_01_01_01.СвФил
			Доверитель.ДанныеФилиалаЮридическогоЛица.Наименование = Строка(СведенияОбОрганизации.НаимОрг);
			Доверитель.ДанныеФилиалаЮридическогоЛица.ИНН = Строка(СведенияОбОрганизации.ИННЮЛ);
			Доверитель.ДанныеФилиалаЮридическогоЛица.КПП = Строка(СведенияОбОрганизации.КПП);
			
			РуководительФилиала = СведенияОДоверителе.ПередПолн.ФилПерПолн.СвРукФил; // СписокXDTO Из См. XDTOПакет.ON_EMCHD_1_928_00_01_01_01.СвРукФил
			
			ФизЛицо = Журнал.НовоеФизическоеЛицо();
			Доверитель.ДанныеФилиалаЮридическогоЛица.РуководителиФилиала.Добавить(ФизЛицо);
			ФизЛицо.ФИО = ОбщегоНазначенияБЭДКлиентСервер.ФИОСтрокой(РуководительФилиала.СведФЛ.ФИО);
			ФизЛицо.ИНН = Строка(РуководительФилиала.ИННФЛ);
			ФизЛицо.СНИЛС = Строка(РуководительФилиала.СНИЛС);
