...
Процедура КассыККМПередУдалением(Элемент, Отказ)
	...
	Если ТекущаяСтрока <> Неопределено
		И ЗначениеЗаполнено(ТекущаяСтрока.ПодключаемоеОборудование) Тогда
		НайденныеЭквайринговыеТерминалы = Объект.ЭквайринговыеТерминалы.НайтиСтроки(Новый Структура("ПодключаемоеОборудованиеККТ", ТекущаяСтрока.ПодключаемоеОборудование));
		Если НайденныеЭквайринговыеТерминалы.Количество() > 0 Тогда
			НайденныеКассы = Объект.Кассы.НайтиСтроки(Новый Структура("ПодключаемоеОборудование", ТекущаяСтрока.ПодключаемоеОборудование));
			НайденныеКассыККМ = Объект.КассыККМ.НайтиСтроки(Новый Структура("ПодключаемоеОборудование", ТекущаяСтрока.ПодключаемоеОборудование));
			Если НайденныеКассы.Количество() = 0 И 
				НайденныеКассыККМ.Количество() = 1 Тогда
				Отказ = Истина;
				...
				СтрокиСообщения.Добавить(СтрШаблон(ШаблонСообщения, НайденныеЭквайринговыеТерминалы[0].ЭквайринговыйТерминал,
												ТекущаяСтрока.ПодключаемоеОборудование));
