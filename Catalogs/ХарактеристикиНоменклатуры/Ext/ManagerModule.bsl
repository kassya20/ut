...
//
// Параметры:
//  Номенклатура					 - СправочникСсылка.Номенклатура				 - владелец новой характеристики.
...
//
Функция ИдентичнаяХарактеристика(Номенклатура, ХарактеристикаВДокументе) Экспорт
	...
	Запрос = Новый Запрос(
		...
		|	И СОКРЛП(ХарактеристикиНоменклатуры.Наименование) В
		|			(ВЫБРАТЬ
		|				СОКРЛП(Т.Наименование) КАК Наименование
		|			ИЗ
		|				Справочник.ХарактеристикиНоменклатуры КАК Т
		|			ГДЕ
		|				Т.Ссылка = &Характеристика)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("Характеристика", ХарактеристикаВДокументе);
	...
	ЗначенияЗаполнения = РезультатыЗапроса[1].Выгрузить()[0];
	
	УстановитьПривилегированныйРежим(Истина);
	
	НоваяХарактеристика = Справочники.ХарактеристикиНоменклатуры.СоздатьЭлемент();
	ЗаполнитьЗначенияСвойств(НоваяХарактеристика, ЗначенияЗаполнения,,"Владелец");
	НоваяХарактеристика.Владелец = Номенклатура;
	Для Каждого ТабличнаяЧасть Из ХарактеристикаВДокументе.Метаданные().ТабличныеЧасти Цикл
		НоваяХарактеристика[ТабличнаяЧасть.Имя].Загрузить(ЗначенияЗаполнения[ТабличнаяЧасть.Имя]);
	КонецЦикла;
