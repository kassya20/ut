...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтоРежимСозданияНастройки = Параметры.ЭтоРежимСозданияНастройки;
	ПараметрыСозданияКассы = Параметры.ПараметрыСозданияКассы;
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(Параметры.ОблачнаяКасса)
		Или ЭтоРежимСозданияНастройки Тогда
		СозданиеНастройкиДоступно = Истина;
	Иначе
		СозданиеНастройкиДоступно = Ложь;
	КонецЕсли;
	
	Если Объект.Ссылка.Пустая()
		И Не СозданиеНастройкиДоступно Тогда
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ЭтоРежимСозданияНастройки
		И Не ЗначениеЗаполнено(Объект.ОблачнаяКасса) Тогда
		
		ТекущийОбъект.ОблачнаяКасса = ОблачныеКассыСлужебный.КассаПриЗаписиНастройкиПодключения(
			ПараметрыСозданияКассы.Организация,
			Объект.Наименование);
...
Процедура ИдентификаторПриИзменении(Элемент)
	...
	Если ПустаяСтрока(ИдентификаторДоИзменения) Тогда
		ПредставлениеНеРедактировалось = Истина;
	Иначе
		ОтборКассы    = Новый Структура("Идентификатор", ИдентификаторДоИзменения);
		ОтобранныеОблачныеКассы = ДоступныеОблачныеКассы.НайтиСтроки(ОтборКассы);
		Если ОтобранныеОблачныеКассы.Количество() = 0 Тогда
			ПредставлениеНеРедактировалось = Истина;
		Иначе
			ПредставлениеНеРедактировалось = Объект.Наименование = Лев(ОтобранныеОблачныеКассы[0].Представление, 150);
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаписатьИЗакрыть(Команда)
	...
	Если Записать() Тогда
		...
		Если Не ПустаяСтрока(РезультатСохранения.КодОшибки) Тогда
			ПоказатьПредупреждение(, РезультатСохранения.СообщениеОбОшибке);
		ИначеЕсли ЭтоРежимСозданияНастройки Тогда
...
Процедура ЗаполнитьДоступныеОблачныеКассы()
	...
	ДлительнаяОперация    = ПолучитьДоступныеОблачныеКассыВФоне();
...
&НаСервере
Функция ПолучитьДоступныеОблачныеКассыВФоне()
...
Функция ЗаполнитьДоступныеОблачныеКассыНаСервере(Знач АдресРезультата)
	...
	Если ПустаяСтрока(РезультатОперации.КодОшибки) Тогда
		...
		Элементы.ФормаПроверить.Видимость = Истина;
		...
		Если РезультатПоиска.Количество() = 0 Тогда
			...
			ОбщегоНазначения.СообщитьПользователю(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Облачная касса с идентификатором ""%1"" больше не доступна для фискализации.'"),
...
Процедура ПоказатьОшибкуЗаполненияСпискаДоступныхОблачныхКасс(ТекстОшибки)
	...
	ОтобразитьОшибкуЗагрузки();
	ПоказатьПредупреждение(, ТекстОшибки);
	
КонецПроцедуры

&НаСервере
Процедура ОтобразитьОшибкуЗагрузки()
	
	Элементы.ГруппаЗагрузкаКасс.Видимость = Ложь;
	Элементы.ГруппаОшибкаЗагрузки.Видимость = Истина;
	
КонецПроцедуры

&НаСервере
Процедура ОтобразитьЗагрузкуКасс()
	
	Элементы.ФормаЗаписатьИЗакрыть.Видимость = Ложь;
	Элементы.ФормаПроверить.Видимость = Ложь;
	Элементы.ДекорацияОтступ.Видимость = Истина;
	Элементы.ГруппаЗагрузкаКасс.Видимость = Истина;
	Элементы.ГруппаОшибкаЗагрузки.Видимость = Ложь;
	Элементы.ГруппаСтраницы.ТекущаяСтраница = Элементы.СтраницаДлительныеОперации;
