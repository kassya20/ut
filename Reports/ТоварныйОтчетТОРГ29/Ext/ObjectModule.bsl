...
Функция СформироватьОтчетТОРГ29(ДатаНачала, ДатаОкончания, Склад, Организация, НомерОтчета)
	...
	Если Не ЗначениеЗаполнено(РозничныйВидЦены) Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Для склада не установлен розничный вид цены.'"));
		Возврат ТабДокумент;
	КонецЕсли;
	Если Не ЗначениеЗаполнено(ДанныеОтСклада.РозничныйВидЦеныВалютаЦены) Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не установлена валюта розничного вида цены для склада.'"));
		Возврат ТабДокумент;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ВалютаРегламентированногоУчета) Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не установлена валюта регламентированного учета.'"));
		Возврат ТабДокумент;
	КонецЕсли;
	...
	|	СУММА(ВЫБОР
	|			КОГДА ТаблицаДокументов.КонОстКол - ТаблицаДокументов.НачОстКол > 0
	|				ТОГДА ТаблицаДокументов.КонОстКол - ТаблицаДокументов.НачОстКол
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК ПриходКол,
	|	СУММА(ВЫБОР
	|			КОГДА ТаблицаДокументов.НачОстКол - ТаблицаДокументов.КонОстКол > 0
	|				ТОГДА ТаблицаДокументов.НачОстКол - ТаблицаДокументов.КонОстКол
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК РасходКол,
	|	СУММА(ВЫБОР
	|			КОГДА ТаблицаДокументов.КонОст - ТаблицаДокументов.НачОст > 0
	|				ТОГДА ТаблицаДокументов.КонОст - ТаблицаДокументов.НачОст
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК Приход,
	|	СУММА(ВЫБОР
	|			КОГДА ТаблицаДокументов.НачОст - ТаблицаДокументов.КонОст > 0
	|				ТОГДА ТаблицаДокументов.НачОст - ТаблицаДокументов.КонОст
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК Расход
	|ИЗ
	...
	Иначе
		...
		Если ИсходныеДанные.Количество() > 1 Тогда
			...
			Если ИсходныеДанные[0].Регистратор = Неопределено И ИсходныеДанные[1].Регистратор <> Неопределено Тогда
				НачОст = ?(ИсходныеДанные[1].СуммаНачальныйОстаток = Null, 0, ИсходныеДанные[1].СуммаНачальныйОстаток);
			Иначе
				НачОст = ?(ИсходныеДанные[0].СуммаНачальныйОстаток = Null, 0, ИсходныеДанные[0].СуммаНачальныйОстаток);
			КонецЕсли;
		...
		СуммаКонечныйОстаток = ИсходныеДанные[ИсходныеДанные.Количество()-1].СуммаКонечныйОстаток;
		КонОст = ?(СуммаКонечныйОстаток = Null, 0, СуммаКонечныйОстаток);
