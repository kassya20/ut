...
&НаКлиенте
Процедура ТитулГрузоотправителяАдресаПунктовПогрузкиИВыгрузкиОперацияПриИзменении(Элемент)
	
	ТекущиеДанные = Элементы.ТитулГрузоотправителяАдресаПунктовПогрузкиИВыгрузки.ТекущиеДанные;
	
	Если ТекущиеДанные <> Неопределено И ТекущиеДанные.Операция = "Выгрузка" Тогда
		ТекущиеДанные.ВладелецПунктаПогрузкиИНН = Неопределено;
		ТекущиеДанные.ВладелецПунктаПогрузкиНаименование = Неопределено;
	КонецЕсли;
...
Процедура ДобавитьПунктПодачи(Команда)
	...
	Если ЗначениеЗаполнено(ТитулГрузоотправителяАдресПогрузкиКодСтраны) 
		И ЗначениеЗаполнено(ТитулГрузоотправителяАдресПогрузкиПредставление) Тогда
			
		НоваяСтрока.АдресПунктаПогрузкиКодСтраны = ТитулГрузоотправителяАдресПогрузкиКодСтраны;
		НоваяСтрока.АдресПунктаПогрузкиПредставление = ТитулГрузоотправителяАдресПогрузкиПредставление;
