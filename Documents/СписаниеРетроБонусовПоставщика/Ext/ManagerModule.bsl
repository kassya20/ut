...
//
// Параметры:
//  Документ - ДокументСсылка.СписаниеРетроБонусовПоставщика, ДокументОбъект.СписаниеРетроБонусовПоставщика - ссылка на документ или объект, по которому необходимо получить данные
...
Функция ДанныеДокументаДляПроведения(Документ, Регистры, ДопПараметры = Неопределено) Экспорт
	...
	Если ТипЗнч(Документ) = Тип("ДокументОбъект.СписаниеРетроБонусовПоставщика") Тогда
...
Функция ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	Если ПравоДоступа("Добавление", Метаданные.Документы.СписаниеРетроБонусовПоставщика) Тогда
		
		КомандаСоздатьНаОсновании = КомандыСозданияНаОсновании.Добавить();
		КомандаСоздатьНаОсновании.Менеджер = Метаданные.Документы.СписаниеРетроБонусовПоставщика.ПолноеИмя();
		КомандаСоздатьНаОсновании.Представление = Метаданные.Документы.СписаниеРетроБонусовПоставщика.Синоним;
...
//  * Ссылка - ДокументСсылка.СписаниеРетроБонусовПоставщика
//  * Номер - Строка -
//  * Дата - Дата
...
Функция ШаблонЗаголовкаДокумента() Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ВыводитьПолныеНаименованияРетроБонусовПоставщиков") Тогда
		...
		ШаблонЗаголовкаДокумента = НСтр("ru = 'Списание ретро-бонусов поставщика %1'");
		
	Иначе
		
		ШаблонЗаголовкаДокумента = НСтр("ru = 'Списание РБ поставщика %1'");
...
Функция ТекстЗапросаТаблицаРетроБонусыПоставщиков(ТекстыЗапроса, Регистры, ДокументОбъект)
	...
	|	ДанныеТаблицы.Валюта КАК Валюта,
	|	ДанныеТаблицы.Сумма КАК Списано,
	|	0 КАК КСписанию
	|ИЗ
...
Процедура ТекстЗапросаДанныеДокументаШапка(СписокЗапросов)
	...
	|ИЗ
	|	Документ.СписаниеРетроБонусовПоставщика КАК ДанныеДокумента
	|ГДЕ
	|	ДанныеДокумента.Ссылка = &Ссылка";
...
Процедура ТекстЗапросаДанныеТаблицаРасшифровкаСписаний(СписокЗапросов)
	...
	|ИЗ
	|	Документ.СписаниеРетроБонусовПоставщика.РасшифровкаСписаний КАК ТаблицаДанных
	|ГДЕ
