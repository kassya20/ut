...
Процедура СоздатьДокументУчета(Команда)
	...
	ОбменСКонтрагентамиКлиентПереопределяемый.ПриСозданииУчетногоДокумента(Документооборот, СтандартнаяОбработка);
...
Процедура СоздатьДокументУчетаНаСервере(СпособОбработки, КонтекстОперации)
	...
	Если ОтражениеВУчетеЭДО.ТребуетсяОтражениеДвумяОбъектамиУчета(ТипДокумента, МассивСпособов) Тогда
		СпособОбработки = ОтражениеВУчетеЭДО.НовыйСпособОбработкиУПДУКД();
		СпособОбработки.ПервичныйДокумент = МассивСпособов[1];
		СпособОбработки.СчетФактура = МассивСпособов[0];
	КонецЕсли;
	
	ЭлектронныеДокументы = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Документооборот);
	ПараметрыЗаполнения = ЭлектронныеДокументыЭДО.ДанныеДокументовДляОтраженияВУчете(ЭлектронныеДокументы); 
	
	Если ЗначениеЗаполнено(ПараметрыЗаполнения) Тогда
		НачатьТранзакцию();
		Попытка
			ЭлектронныеДокументыЭДО.ЗаблокироватьДокументДляИзменения(Документооборот, Истина);
...
Процедура ПерепривязатьЭлектронныйДокумент(ВыбранноеЗначение, СпособОбработки)
	...
	Если Элементы.ГруппаНастройкиОтраженияВУчете.Видимость Тогда
		...
		Реквизиты = "ИдентификаторКонтрагента, ИдентификаторОрганизации, Контрагент, Организация, ВидДокумента";
		РеквизитыДокументооборота = ЭлектронныеДокументыЭДО.СвойстваДокумента(Документооборот, Реквизиты);
		КлючНастройки = НастройкиЭДОКлиентСервер.НовыйКлючНастроекОтраженияВУчете();
		КлючНастройки.ИдентификаторОтправителя = РеквизитыДокументооборота.ИдентификаторКонтрагента;
		КлючНастройки.ИдентификаторПолучателя = РеквизитыДокументооборота.ИдентификаторОрганизации;
		КлючНастройки.Отправитель = РеквизитыДокументооборота.Контрагент;
		КлючНастройки.Получатель = РеквизитыДокументооборота.Организация;	
		КлючНастройки.ВидДокумента = РеквизитыДокументооборота.ВидДокумента;
...
Процедура ОтображениеГруппыНастройкиОтраженияВУчете()
	...
	Реквизиты = "ИдентификаторКонтрагента, ИдентификаторОрганизации, Контрагент, Контрагент.Представление,
	|	Организация, ВидДокумента";
	РеквизитыДокументооборота = ЭлектронныеДокументыЭДО.СвойстваДокумента(Документооборот, Реквизиты);
	
	Элементы.НадписьНастройкаОтраженияВУчете.Заголовок =
		СтрШаблон(Элементы.НадписьНастройкаОтраженияВУчете.Заголовок, РеквизитыДокументооборота.КонтрагентПредставление);
...
Процедура ПодготовитьОтражениеВУчете()
	...
	Реквизиты = "ВидДокумента, ВидДокумента.ТипДокумента, ФорматОсновногоТитула";
	РеквизитыДокументооборота = ЭлектронныеДокументыЭДО.СвойстваДокумента(Документооборот, Реквизиты);
	ВидДокумента = РеквизитыДокументооборота.ВидДокумента;
	ТипДокумента = РеквизитыДокументооборота.ВидДокументаТипДокумента;
	ФорматОсновногоТитула = РеквизитыДокументооборота.ФорматОсновногоТитула;
	...
	Если ТипДокумента = Перечисления.ТипыДокументовЭДО.УПД
		...
		ЭлектронныеДокументы = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Документооборот);
		ДанныеДокументооборота = ЭлектронныеДокументыЭДО.ДанныеДокументовДляОтраженияВУчете(ЭлектронныеДокументы)[0];
	...
	ИначеЕсли ТипДокумента = Перечисления.ТипыДокументовЭДО.УКД
		И Не ОтражениеВУчетеЭДО.ОтражатьУниверсальныеДокументыОднимОбъектомУчета() Тогда
	...
	Если Не ИнтеграцияЭДО.ДоступноОтражениеВУчете(ФорматОсновногоТитула, ТипДокумента, СостояниеЭДО) Тогда
		Элементы.ГруппаСоздать.Видимость = Ложь;
		Элементы.ФормаСоздатьДокумент.Видимость = Ложь;
	КонецЕсли;
