...
Процедура НайтиСоздатьЗапросКоммерческихПредложений(Знач ДанныеВходящегоДокумента, УчетныйДокумент) Экспорт
	...
	Попытка
		...
		Иначе
			...
			УчетныйДокумент = ПолучитьСсылку(Новый УникальныйИдентификатор());
			ДокументОбъект.УстановитьСсылкуНового(УчетныйДокумент);
		КонецЕсли;
		...
		СтрокиФайловИзображенийДокумента = ДокументОбъект.Изображения.Выгрузить();
		ДокументОбъект.Изображения.Очистить();		
		ДокументОбъект.Штрихкоды.Очистить();
		...
		Для Каждого СведенияОТоваре Из СведенияОТоварах.Строки Цикл
			...
			ЗагрузитьАдресаИзображенийСтрокиДокумента(ДокументОбъект, НоваяСтрока.ИдентификаторСтрокиЗапроса, 
				СведенияОТоваре, СтрокиФайловИзображенийДокумента);
									
			ЗагрузитьШтрихкодыТовара(ДокументОбъект, НоваяСтрока, СведенияОТоваре);
			
		КонецЦикла;
		
		// Помечаем на удаление неактуальные изображения
		НеактуальныеИзображенияПометитьНаУдаление(СтрокиФайловИзображенийДокумента); 
	...
	ОбновитьИзображенияТоварныхПозицийДокумента(УчетныйДокумент, ДокументОбъект.Изображения.Количество());
...
Функция ПолучитьФайлыДляИзменения(ТаблицаИменФайлов, ВладелецФайлов)
	...
	Запрос.Текст = 
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ЗапросКоммерческогоПредложенияОтКлиентаПрисоединенныеФайлы.Ссылка,
		|	ЗапросКоммерческогоПредложенияОтКлиентаПрисоединенныеФайлы.Наименование
		|ПОМЕСТИТЬ ВТ_ВложенияДокумента
		|ИЗ
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗапросКоммерческогоПредложенияОтКлиента.Изображения КАК
		|			ЗапросКоммерческогоПредложенияОтКлиентаИзображения
		|		ПО ЗапросКоммерческогоПредложенияОтКлиентаПрисоединенныеФайлы.Ссылка = ЗапросКоммерческогоПредложенияОтКлиентаИзображения.ФайлИзображения
		|ГДЕ
		|	ЗапросКоммерческогоПредложенияОтКлиентаПрисоединенныеФайлы.ВладелецФайла = &ВладелецФайла
		|	И ЗапросКоммерческогоПредложенияОтКлиентаИзображения.Ссылка ЕСТЬ NULL
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТаблицаИменФайлов.ИмяБезРасширения КАК ИмяБезРасширения,
		|	ЕСТЬNULL(ВТ_ВложенияДокумента.Ссылка, НЕОПРЕДЕЛЕНО) КАК СсылкаНаФайл,
		|	ВЫБОР
		...
		|ИЗ
		|	ТаблицаИменФайлов КАК ТаблицаИменФайлов
		|		ПОЛНОЕ СОЕДИНЕНИЕ ВТ_ВложенияДокумента КАК ВТ_ВложенияДокумента
		|		ПО ВЫРАЗИТЬ(ТаблицаИменФайлов.ИмяБезРасширения КАК СТРОКА(1024)) = ВЫРАЗИТЬ(ВТ_ВложенияДокумента.Наименование КАК
		|			СТРОКА(1024))";
...
Процедура НеактуальныеИзображенияПометитьНаУдаление(СтрокиФайловИзображенийДокумента)
	
	УстановитьПривилегированныйРежим(Истина);
		
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("СтрокиФайловИзображенийКУдалению", СтрокиФайловИзображенийДокумента);
	Запрос.Текст =
		"ВЫБРАТЬ
		|	СтрокиФайловИзображенийКУдалению.ФайлИзображения
		|ПОМЕСТИТЬ ВТ_ФайлыИзображенийКУдалению
		|ИЗ
		|	&СтрокиФайловИзображенийКУдалению КАК СтрокиФайловИзображенийКУдалению
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ЗапросКоммерческогоПредложенияОтКлиентаПрисоединенныеФайлы.Ссылка
		|ИЗ
		|	ВТ_ФайлыИзображенийКУдалению КАК ВТ_ФайлыИзображенийКУдалению
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ЗапросКоммерческогоПредложенияОтКлиентаПрисоединенныеФайлы КАК
		|			ЗапросКоммерческогоПредложенияОтКлиентаПрисоединенныеФайлы
		|		ПО ВТ_ФайлыИзображенийКУдалению.ФайлИзображения = ЗапросКоммерческогоПредложенияОтКлиентаПрисоединенныеФайлы.Ссылка
...
Процедура ЗагрузитьАдресаИзображенийСтрокиДокумента(ДокументОбъект, ИдентификаторСтрокиЗапроса, СведенияОТоваре,
	СтрокиФайловИзображенийДокумента)
		
	МассивАдресовИзображений = ЭлектронноеВзаимодействие.ЗначениеРеквизитаВДереве(
		СведенияОТоваре, "Товары.НомерСтроки.Товар.Изображения");

	Если ТипЗнч(МассивАдресовИзображений) <> Тип("Массив") Тогда
		Возврат;
	КонецЕсли;

	Для Каждого АдресИзображения Из МассивАдресовИзображений Цикл
		
		Если Не ЗначениеЗаполнено(АдресИзображения) Тогда
			Продолжить;
		КонецЕсли;		
				
		ИмяФайлаИРасширение = КоммерческиеПредложенияСлужебный.ИмяФайлаИРасширение(АдресИзображения);

		НоваяСтрокаИзображений = ДокументОбъект.Изображения.Добавить();
		НоваяСтрокаИзображений.ИдентификаторСтрокиЗапроса = ИдентификаторСтрокиЗапроса;
		НоваяСтрокаИзображений.ИмяФайлаБезРасширения = ИмяФайлаИРасширение.ИмяБезРасширения;
		НоваяСтрокаИзображений.ВнешнийАдресURL = АдресИзображения;
			
		СтрокаНайденногоФайлаИзображения = 
			СтрокиФайловИзображенийДокумента.Найти(ИмяФайлаИРасширение.ИмяБезРасширения, "ИмяФайлаБезРасширения");
			
		Если СтрокаНайденногоФайлаИзображения <> Неопределено Тогда
			
			// Файл существует, его нужно будет обновить
			НоваяСтрокаИзображений.ФайлИзображения = СтрокаНайденногоФайлаИзображения.ФайлИзображения;
			
			// Удаляем строку, т.к. все что останется в коллекции, пометим на удаление
			СтрокиФайловИзображенийДокумента.Удалить(СтрокаНайденногоФайлаИзображения);
...
Процедура ОбновитьИзображенияТоварныхПозицийДокумента(УчетныйДокумент, КоличествоИзображений)
	...
	Для Каждого СтрокаИзображения Из ДокументОбъект.Изображения Цикл
		
		ИмяФайлаИРасширение = КоммерческиеПредложенияСлужебный.ИмяФайлаИРасширение(СтрокаИзображения.ВнешнийАдресURL);
		ЭтоОтносительныйАдрес = СтрНайти(СтрокаИзображения.ВнешнийАдресURL, "http") = 0;
		АдресИзображенияВХранилище = КоммерческиеПредложенияСлужебный.АдресИзображенияВХранилище(
			СтрокаИзображения.ВнешнийАдресURL, ЭтоОтносительныйАдрес);

		Если ПустаяСтрока(АдресИзображенияВХранилище) Тогда
			ВызватьИсключение СтрШаблон(
				НСтр("ru = 'Не удалось скачать файл изображения по ссылке ""%1"". 
				| Попробуйте еще раз или обратитесь к администратору системы'", 
					ОбщегоНазначения.КодОсновногоЯзыка()),
				СтрокаИзображения.ВнешнийАдресURL);
		КонецЕсли;

		Если ЗначениеЗаполнено(СтрокаИзображения.ФайлИзображения) Тогда
			ПараметрыФайла = Новый Структура;
			ПараметрыФайла.Вставить("АдресФайлаВоВременномХранилище", АдресИзображенияВХранилище);
			ПараметрыФайла.Вставить("АдресВременногоХранилищаТекста", "");
			ПараметрыФайла.Вставить("ИмяБезРасширения", ИмяФайлаИРасширение.ИмяБезРасширения);
			ПараметрыФайла.Вставить("Расширение", ИмяФайлаИРасширение.РасширениеБезТочки);	
			РаботаСФайлами.ОбновитьФайл(СтрокаИзображения.ФайлИзображения, ПараметрыФайла);
		Иначе
			ПараметрыФайла = РаботаСФайлами.ПараметрыДобавленияФайла();
			ПараметрыФайла.ВладелецФайлов = УчетныйДокумент;
			ПараметрыФайла.ИмяБезРасширения = ИмяФайлаИРасширение.ИмяБезРасширения;
			ПараметрыФайла.РасширениеБезТочки = ИмяФайлаИРасширение.РасширениеБезТочки;
			СтрокаИзображения.ФайлИзображения = РаботаСФайлами.ДобавитьФайл(ПараметрыФайла, АдресИзображенияВХранилище);
		КонецЕсли;
	...
	ВидОперации = НСтр("ru = 'Обновление изображений товарных позиций запроса коммерческих предложений'", 
		ОбщегоНазначения.КодОсновногоЯзыка());
	ИнформацияОбОшибке = ИнформацияОбОшибке();
	ПодробныйТекстОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
	КраткийТекстОшибки = 
		НСтр("ru = 'Не удалось обновить изображения товарных позиций документа запрос коммерческих предложений'", 
		ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ЗагрузитьШтрихкодыТовара(ДокументОбъект, НоваяСтрока, СведенияОТоваре)

	МассивШтрихкодов = ЭлектронноеВзаимодействие.ЗначениеРеквизитаВДереве(
	...
	Если ТипЗнч(МассивШтрихкодов) = Тип("Массив") Тогда
		 ...
		 Для Каждого Штрихкод Из МассивШтрихкодов Цикл
		 	НоваяСтрокаШтрихкодов = ДокументОбъект.Штрихкоды.Добавить();
