...
Процедура ПриОткрытииНаСервере()
	...
	Если Параметры.Свойство("Организация") Тогда
		Период = Параметры.Организация;
	КонецЕсли;
	
	Если Параметры.Свойство("Подразделение") Тогда
		Период = Параметры.Подразделение;
	КонецЕсли;
	...
	ДобавитьФоновоеЗадание("ОбновитьГиперссылкуКОформлению", ТекущиеФоновыеЗадания);
	ДобавитьФоновоеЗадание("ЗаполнитьСтатьиРасходов", ТекущиеФоновыеЗадания);
	
	ПараметрыЗаданий = Неопределено;
	ПодготовитьСерверныеПараметрыЗаданий(ПараметрыЗаданий);
	ЗапуститьФоновыеЗаданияНаСервере(ПараметрыЗаданий);
...
Процедура ДобавитьФоновоеЗадание(ИмяЗадания, ТекущиеФоновыеЗадания, ОчиститьОчередь = Ложь)

	Если ОчиститьОчередь Тогда
		
		ОтменитьВыполнениеТекущихЗаданий(ТекущиеФоновыеЗадания);
...
Процедура ЗапуститьФоновоеЗадание(ИмяЗадания)
	...
	ДобавитьФоновоеЗадание(ИмяЗадания, ТекущиеФоновыеЗадания, Истина);
...
Функция ЗапуститьФоновыеЗаданияНаСервере(Знач ПараметрыЗаданий)
	...
	Для Каждого ЗаданиеВОчереди Из ТекущиеФоновыеЗадания Цикл
		...
		ЗаданиеВОчереди.РезультатЗапуска = РезультатЗапуска;
...
// Отменить выполнение текущих служебны фоновых заданий формы.
// 
// Параметры:
//  ТекущиеФоновыеЗадания см. Документ.РаспределениеПрочихЗатрат.Форма.ФормаРабочееМесто.ТекущиеФоновыеЗадания
&НаСервереБезКонтекста
Процедура ОтменитьВыполнениеТекущихЗаданий(Знач ТекущиеФоновыеЗадания)
...
Процедура ПроверитьАктивностьФоновогоЗаданияОбновленияФормы()
	...
	Для Каждого Задание Из ТекущиеФоновыеЗадания Цикл
		...
		ОповещениеОЗавершении = Новый ОписаниеОповещения("ОбработатьРезультатВыполненияЗадания", ЭтотОбъект);
	
		ДлительныеОперацииКлиент.ОжидатьЗавершение(
			Задание.РезультатЗапуска, ОповещениеОЗавершении, ПараметрыОжидания);
...
&НаКлиенте
Процедура ОбработатьРезультатВыполненияЗадания(Результат, Дополнительныепараметры) Экспорт
	...
	Если Результат.Статус = "Выполнено" Тогда
		...
		Для Каждого ФоновоеЗадание Из ТекущиеФоновыеЗадания Цикл
			Если ФоновоеЗадание.Идентификатор = Результат.ИдентификаторЗадания Тогда
				ФоновоеЗадание.Выполнено = Истина;
			КонецЕсли;
			ВсеЗаданияЗавершены = Мин(ВсеЗаданияЗавершены, ФоновоеЗадание.Выполнено);
		КонецЦикла;
