...
Функция ПараметрыМакетаЭПЛ(СсылкаНаОбъект, ДанныеЗаполнения)
	...
	Для Каждого СтрПрицеп Из ОбменСГИСЭПД.ПолучитьЗначениеСтруктуры(ДанныеЗаполнения, "ТитулОформлениеТранспортноеСредствоПрицепы", Новый Массив) Цикл
		МассивНомераПрицепов.Добавить(СтрПрицеп.ТитулОформлениеТранспортноеСредствоРегистрационныйНомер);
	КонецЦикла;   
