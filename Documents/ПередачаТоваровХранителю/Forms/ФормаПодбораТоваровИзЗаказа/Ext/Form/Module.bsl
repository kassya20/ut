...
Процедура ЗаполнитьТаблицуТоваров(Документ, ВалютаДокумента)
	...
	Документы.ПередачаТоваровХранителю.ЗаполнитьПоОстаткамЗаказов(
		ДанныеОтбора, ТаблицаТоваров, Параметры.Склад, Неопределено, ПараметрыЗаполнения, ТаблицаШтрихкодыУпаковок);
...
Процедура ПерезаполнитьКоличествоПодобрано(СЗаполнением = Ложь, ТоварыПередачи = Неопределено)
	...
	ОбрабатываемыеСтрокиТоваров = Новый Массив;

	Если СЗаполнением Тогда

		ОбрабатываемыеСтрокиПередачи = Новый Массив;
	...
	Для Каждого ТекСтрока Из ТаблицаТоваров Цикл
		...
		Если ТекСтрока.КоличествоУпаковок <> КоличествоУпаковок Тогда
			ОбрабатываемыеСтрокиТоваров.Добавить(ТекСтрока);
		КонецЕсли;
		...
		Если СЗаполнением Тогда
			...
			Если Строки.Количество() > 0 Тогда
				...
				Если ТекСтрока.КоличествоУпаковок <> СтрокаПередачи.КоличествоУпаковок Тогда
					ОбрабатываемыеСтрокиПередачи.Добавить(ТекСтрока);
				КонецЕсли;
...
Функция ПоместитьТоварыВХранилище()
	...
	ВыбранныеШтрихкодыУпаковок = Новый Массив;
	
	Для Каждого Строка Из ТаблицаВыбранныхТоваров Цикл
		...
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			ВыбранныеШтрихкодыУпаковок.Добавить(НайденнаяСтрока.ШтрихкодУпаковки);
		КонецЦикла;
		
	КонецЦикла;
	
	АдресТоваровВХранилище = ПоместитьВоВременноеХранилище(
		Новый Структура("Товары, ШтрихкодыУпаковок", ТаблицаВыбранныхТоваров, ВыбранныеШтрихкодыУпаковок));
