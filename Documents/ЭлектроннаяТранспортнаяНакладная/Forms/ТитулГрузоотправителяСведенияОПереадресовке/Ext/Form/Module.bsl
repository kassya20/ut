...
&НаКлиенте
Процедура ТитулГрузоотправителяЛицоИмеющееПолномочияПереадресовкиПриИзменении(Элемент)

	ИзменитьОформлениеКнопок(Новый Структура ("ИмяКнопки, ИдентификаторСтроки", 
		"ЗаполнитьТитулГрузоотправителяПолномочияПереадресовки", Неопределено));
