...
Процедура ЗапроситьУточнение(Команда)
	...
	Если ТребуетсяЗаписатьДокумент() Тогда
		Обработчик = Новый ОписаниеОповещения("ЗапроситьУточнениеПослеЗаписи", ЭтотОбъект, Истина);
		НачатьЗаписьДокумента(Обработчик);
	Иначе
...
Процедура ЗапроситьУточнениеДокумент(Команда)
	Если ТребуетсяЗаписатьДокумент() Тогда
		Обработчик = Новый ОписаниеОповещения("ЗапроситьУточнениеПослеЗаписи", ЭтотОбъект, Ложь);
		НачатьЗаписьДокумента(Обработчик);
	Иначе
		ЛегкийИнтерфейсДокументовЭДОКлиент.ВыполнитьДействиеЗапроситьУточнениеИзКарточкиДокумента(ЭтотОбъект,
			ЭтоПакетДокументов);
...
&НаКлиенте
Процедура ЗапроситьУточнениеПослеЗаписи(Результат, ЭтоПакетнаяОбработка = Ложь) Экспорт
...
Процедура ОбновитьВизуализациюДокументаНаСервере()
	...
	ДанныеКомпонентов = ЛегкийИнтерфейсДокументовЭДО.НовыеДанныеКомпонентовПросмотраДокумента(
		"ПанельВизуализации, ПанельКомандНастроекВизуализацииЛегкийКлиентЭДО, ПанельПредставленияФайла");
...
Процедура ОбновитьДанныеПослеИзмененияПодписейДокументаНаСервере()
	...
	ДанныеКомпонентов = ЛегкийИнтерфейсДокументовЭДО.НовыеДанныеКомпонентовПросмотраДокумента(
		...
		|ПанельЗакладокИсходящегоДокументаЛегкийКлиентЭДО, ПанельВизуализации,
		|ПанельКомандНастроекВизуализацииЛегкийКлиентЭДО,
		|ПанельПодписей");
...
Процедура ЗаполнитьДокументПоПодготовленнымДанным(ПросмотрПодготовленныхДанных)
	...
	Если ПросмотрПодготовленныхДанных.НастройкиФормирования.ИспользоватьЦентрЭДО Тогда
		ЗаполнитьДокументПоПодготовленнымДаннымЦентраЭДО(ПросмотрПодготовленныхДанных);
	Иначе
...
#Область ЗаполнениеПодготовленныхДанныхЦентраЭДО

// Параметры:
//  ПросмотрПодготовленныхДанных - См. ЗаполнитьДокументПоПодготовленнымДанным.ПросмотрПодготовленныхДанных
&НаСервере
Процедура ЗаполнитьДокументПоПодготовленнымДаннымЦентраЭДО(ПросмотрПодготовленныхДанных)
	
	Если ЭлектронныеДокументыЭДО.ЕстьИнтеграцияЦентраЭДО() Тогда
		ОбщийМодульИнтерфейсДокументовЭДОИнтеграцияЦентраЭДО = 
			ОбщегоНазначения.ОбщийМодуль("ИнтерфейсДокументовЭДОИнтеграцияЦентраЭДО");
		ОбщийМодульИнтерфейсДокументовЭДОИнтеграцияЦентраЭДО.ЗаполнитьДокументПоПодготовленнымДанным(ЭтотОбъект,
...
Процедура ЗаполнитьДанныеДокументаПриИзмененииКлючевыхРеквизитов()
	...
	ИспользоватьЦентрЭДО = УчетныеЗаписиЭДО.ИспользоватьЦентрЭДО(Объект.ИдентификаторОрганизации);
	
	Объект.ИспользоватьЦентрЭДО = ИспользоватьЦентрЭДО;
	...
	Если Не ИспользоватьЦентрЭДО И Не ЗначениеЗаполнено(Объект.ИдентификаторДокументооборота) Тогда
		Объект.ИдентификаторДокументооборота = ЭлектронныеДокументыЭДО.НовыйИдентификаторДокументооборота();
	КонецЕсли;
...
Процедура ПередЗаписьюНаСервереПоИнтеграциям(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ТекущийОбъект.ИспользоватьЦентрЭДО Тогда
		ПередЗаписьюНаСервереИнтеграцияЦентраЭДО(Отказ, ТекущийОбъект, ПараметрыЗаписи);
	Иначе
...
&НаСервере
Процедура ПередЗаписьюНаСервереИнтеграцияЦентраЭДО(Отказ, ТекущийОбъект, ПараметрыЗаписи)
...
Процедура ПриЗаписиНаСервереПоИнтеграциям(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	Если ТекущийОбъект.ИспользоватьЦентрЭДО Тогда
		ПриЗаписиНаСервереИнтеграцияЦентраЭДО(Отказ, ТекущийОбъект, ПараметрыЗаписи);
	Иначе
...
&НаСервере
Процедура ПриЗаписиНаСервереИнтеграцияЦентраЭДО(Отказ, ТекущийОбъект, ПараметрыЗаписи)
...
Процедура ПослеЗаписиНаСервереПоИнтеграциям(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ТекущийОбъект.ИспользоватьЦентрЭДО Тогда
		ПослеЗаписиНаСервереИнтеграцияЦентраЭДО(ТекущийОбъект, ПараметрыЗаписи);
	Иначе
...
&НаСервере
Процедура ПослеЗаписиНаСервереИнтеграцияЦентраЭДО(ТекущийОбъект, ПараметрыЗаписи)
...
Процедура ПослеЗаписиПоИнтеграциям(ОповещениеОЗавершении, ЭтоНовыйДокумент)
	Если Объект.ИспользоватьЦентрЭДО Тогда
		ПослеЗаписиИнтеграцияЦентраЭДО(ОповещениеОЗавершении, ЭтоНовыйДокумент);
	Иначе
...
&НаКлиенте
Процедура ПослеЗаписиИнтеграцияЦентраЭДО(ОповещениеОЗавершении, ЭтоНовыйДокумент)
	...
	ОповещениеПослеВыполненияДействияПоЭДО = Новый ОписаниеОповещения(
		"ПослеВыполненияДействияПоЭДОИнтеграцияЦентраЭДО", ЭтотОбъект, ОповещениеОЗавершении);
...
&НаКлиенте
Процедура ПослеВыполненияДействияПоЭДОИнтеграцияЦентраЭДО(Результат, ОповещениеОЗавершении) Экспорт
...
Процедура ПереформироватьДокументПоИнтеграциям(Настройки, Комментарий)
	Если Объект.ИспользоватьЦентрЭДО Тогда
		ПереформироватьДокументИнтеграцияЦентраЭДО(Настройки, Комментарий);
	Иначе
...
&НаКлиенте
Процедура ПереформироватьДокументИнтеграцияЦентраЭДО(Настройки, Комментарий)
...
Функция ПараметрыДобавленияВПакет()
	...
	ПараметрыДобавленияВПакет.ИспользоватьЦентрЭДО = Объект.ИспользоватьЦентрЭДО;
...
Процедура ОткрытьФайлПослеЗаписи(Результат, ДополнительныеПараметры) Экспорт
	...
	ЛегкийИнтерфейсДокументовЭДОКлиент.ОткрытьФайлИнформацииОтправителяДокумента(Объект.Ссылка, Объект.ИспользоватьЦентрЭДО,
		УникальныйИдентификатор);
