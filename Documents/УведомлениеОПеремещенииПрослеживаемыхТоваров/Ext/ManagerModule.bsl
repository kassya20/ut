...
Функция ЗаполнитьДокументДанными(Период, Организация, ДокументОснование = Неопределено) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ВЫРАЗИТЬ(ВТ_ВыборкаДанных.РНПТ КАК Справочник.НомераГТД).ТипНомераГТД = ЗНАЧЕНИЕ(Перечисление.ТипыНомеровГТД.НомерРНПТ)
	|			ТОГДА ВЫРАЗИТЬ(ВТ_ВыборкаДанных.РНПТ КАК Справочник.НомераГТД).ЕдиницаИзмерения
	|		ИНАЧЕ ВЫРАЗИТЬ(ВТ_ВыборкаДанных.КодТНВЭД КАК Справочник.КлассификаторТНВЭД).ЕдиницаИзмерения
