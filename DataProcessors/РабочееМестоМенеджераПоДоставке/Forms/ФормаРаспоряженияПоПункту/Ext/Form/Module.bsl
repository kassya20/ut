...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РейсПункт = Новый Массив; // Массив из ФорматированнаяСтрока, Строка
	...
	Если РейсПункт.Количество() > 0 Тогда
		...
		РейсПункт.Вставить(
			0,
			Новый ФорматированнаяСтрока(НСтр("ru = 'Рейс:'") + " ", ШрифтыСтиля.ЖирныйШрифт));
	...
	Если Параметры.Свойство("Адрес")
	   И ЗначениеЗаполнено(Параметры.Адрес) Тогда
		...
		КонецЕсли;
		РейсПункт.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Пункт:'") + " ", ШрифтыСтиля.ЖирныйШрифт));
	...
	ДоставкаТоваровКлиентСервер.ОтобразитьНедогрузПерегруз(ЭтотОбъект, ЭтотОбъект);
...
#Область РасчетТоваров
	...
	ТекстЗапроса = "";
	Если Параметры.ВидДоставки = Перечисления.ВидыДоставки.НаСклад Тогда
		Для Каждого Ссылка Из СсылкиНаРаспоряжения Цикл // ДокументСсылка, СправочникСсылка
...
Процедура РаспоряженияПередУдалением(Элемент, Отказ)
	...
	Для каждого ИДСтр Из Элементы.Распоряжения.ВыделенныеСтроки Цикл // Число
