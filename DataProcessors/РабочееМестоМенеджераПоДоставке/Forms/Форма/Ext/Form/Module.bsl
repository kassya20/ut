...
&НаКлиенте
Перем КоличествоВыделенныхСтрокРаспоряжений; // Число
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДоставкаТоваров.УстановитьДоступностьАдресовДоставки(ЭтотОбъект.Элементы);
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыЗаданияНаПеревозкуПланируемые
...
Процедура ЗаданияНаПеревозкуПланируемыеТранспортНачалоВыбораЗавершение(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ВыбранныйТип.Значение = НСтр("ru = 'Транспортное средство'") Тогда
		ОткрытьФорму("Справочник.ТранспортныеСредства.Форма.ФормаВыбора",, ЭтотОбъект,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	Иначе
		ОткрытьФорму("Справочник.ТипыТранспортныхСредств.Форма.ФормаВыбора",, ЭтотОбъект,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
...
Процедура РазнестиРаспоряженияПоВремени(Команда)
	...
	Если НЕ ПроверитьРазбитьПунктДоставкиСервер() Тогда
		...
		ОткрытьФорму(ОбработкаПолноеИмя + ".Форма.ФормаВыбораРаспоряжений", ПараметрыФормы, ЭтотОбъект);
	КонецЕсли;
...
Процедура ОбъединитьПоВремени(Команда)
	...
	Если Элементы.ЗаданияНаПеревозкуПланируемые.ВыделенныеСтроки.Количество() < 2 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Для объединения необходимо выделить хотя бы 2 строки.'"));
...
Процедура СоздатьПоручениеЭкспедитору(Команда)
	...
	ОткрытьФорму("Документ.ПоручениеЭкспедитору.Форма.ФормаДокумента", ПараметрыФормы, ЭтотОбъект,,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ВернутьИсключенныеВДоставку(Команда)
	...
	ПараметрыФормы = Новый Структура("Склад", Склад);
	ОткрытьФорму(ОбработкаПолноеИмя + ".Форма.ФормаОтменыИсключенияИзДоставки", ПараметрыФормы, ЭтотОбъект);
...
Процедура ОткрытьПодборТранспорта(ПеренестиРаспоряженияВНовоеЗадание = Ложь)
	...
	ОткрытьФорму(ОбработкаПолноеИмя + ".Форма.ФормаВыбораТранспорта", ПараметрыФормы, ЭтотОбъект,,,,
...
Процедура ПеренестиСтрокиЗавершение(ИДСтрокиНазначения)
	...
	КонецЕсли;
	Если ИДСтрокиНазначения = Неопределено Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Не выбрана строка назначения в списке заданий на перевозку.'"));
		Возврат
	КонецЕсли;
	...
	Если РаспоряженияСОсобымиУсловиями.Количество() > 0 Тогда
		...
		ОткрытьФорму(ОбработкаПолноеИмя + ".Форма.ФормаПредупреждения",	ПараметрыФормы, ЭтотОбъект);
	КонецЕсли;
...
Функция ПроверитьСообщитьЗаданиеИзменено(СтрокаЗадание)
	...
	Если Не НовоеЗадание Тогда
		...
		Если ВерсияДанныхДокумента <> СтрокаЗадание.ВерсияДанных Тогда
			...
			ОбщегоНазначения.СообщитьПользователю(Текст, СтрокаЗадание.Ссылка);
			Возврат Истина;
		КонецЕсли;
	КонецЕсли;
...
Процедура СохранитьЗаданиеВСтроке(СтрокаЗадание, ПометитьНаУдаление = Ложь)
	...
	Иначе
		...
		Исключение
			...
			ОбщегоНазначения.СообщитьПользователю(Текст, ДокОбъект.Ссылка);
			Возврат;
		КонецПопытки;
	КонецЕсли;
...
Процедура ОбновитьСписокРаспоряженийНаДоставку(РаспоряжениеАдресИлиСсылка = Неопределено)
	...
	ДоставкаТоваров.ОбновитьСписокРаспоряженийНаДоставку(ЭтотОбъект, ОбновляемыеРаспоряжения);
