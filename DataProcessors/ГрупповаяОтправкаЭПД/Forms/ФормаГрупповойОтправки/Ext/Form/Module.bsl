...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ТекущийТитул.СписокВыбора.Добавить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЛ_Титул1"), НСтр("ru='ЭПЛ - Оформление'"));
	...
	Элементы.ТекущийТитул.СписокВыбора.Добавить(ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул1"), НСтр("ru='ЭТрН - Оформление'"));
...
Функция ТитулТолькоПроверяемИОтправляем(Титул)
	
	Результат =
		Титул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул1") 
		Или
...
Процедура ДействиеДалее(Команда)
	...
	ИначеЕсли НавигацияПараметрФормы = "СписокДокументов" Тогда
		Если ТитулТолькоПроверяемИОтправляем(ТекущийТитул) Тогда
...
Функция ПолучитьДанныеРеквизитовДляДокумента(СсылкаНаДокумент, Титул)
	
	СсылкаНаОрганизацию = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СсылкаНаДокумент, "Организация");

	ДанныеРеквизитовКонтейнер = Неопределено;
	ОбменСГИСЭПД.ЗаполнитьТаблицуЗначенийРеквизитов(СсылкаНаДокумент, СсылкаНаОрганизацию, ДанныеРеквизитовКонтейнер, Титул);
	
	ИнформацияПоПрефиксамТитула = ОбменСГИСЭПДКлиентСервер.ПрефиксТитулаПоЭлементуРегламентаЭДО(Титул);
	
	МассивВерсий = ДанныеРеквизитовКонтейнер[ИнформацияПоПрефиксамТитула.ВПрограмме];
	
	Возврат ОбменСГИСЭПДКлиентСервер.СкопироватьИзФиксированногоЗначенияРекурсивно(МассивВерсий[МассивВерсий.ВГраница()]);
...
Процедура СобратьДанные(КонтейнерОтображаемойВерсии)
	...
	Если ТитулТолькоПроверяемИОтправляем(ТекущийТитул) Тогда
		...
		Документ = ДоступныеДокументы.НайтиПоИдентификатору(КонтейнерОтображаемойВерсии.НомерВерсии - 1);
		
		Если Документ <> Неопределено Тогда
		
			РеквизитыДокумента = ПолучитьДанныеРеквизитовДляДокумента(Документ.Ссылка, ТекущийТитул);
		
			КонтейнерОтображаемойВерсии.СтруктураРеквизитовЗаполнение[КонтейнерОтображаемойВерсии.ПрефиксТитула][КонтейнерОтображаемойВерсии.НомерВерсии] = РеквизитыДокумента;
...
Процедура ЗаполнитьДанныеПоСтрокам() Экспорт
	...
	ТолькоПроверкаИОтправкаТекущегоТитула = ТитулТолькоПроверяемИОтправляем(ТекущийТитул);
	...
	Для Каждого Строка Из ДоступныеДокументы Цикл
		...
		Если ТолькоПроверкаИОтправкаТекущегоТитула Тогда
			Продолжить;
		КонецЕсли;
...
Процедура УстановитьТекущийШагПомощника()
	...
	ИначеЕсли НавигацияПараметрФормы = "Отправка" Или НавигацияПараметрФормы = "Отправлено" Тогда
		Если ТитулТолькоПроверяемИОтправляем(ТекущийТитул) Тогда
			УстановитьВидимостьСтраницыОтправка("Шаг3");
		Иначе
...
Процедура ЗаполнитьТаблицу()
	...
	Если ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭПЛ_Титул1") Тогда
		ИмяКлючевогоРеквизита = "ХранимыеДанныеТитулОформлениеТранспортноеСредство";
	...
	ИначеЕсли ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул1") Тогда
		ИмяКлючевогоРеквизита = "СсылкаТитулГрузоотправителяГрузополучатель";
		ЗаголовокКлючевогоРеквизита = НСтр("ru='Грузополучатель'");
	...
	Если ТитулТолькоПроверяемИОтправляем(ТекущийТитул) Тогда
		...
		Если ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовРегламентаЭДО.ЭТрН_Титул1") Тогда
			...
			ИмяРеквизита = "СсылкаТитулГрузоотправителяПеревозчик";
			ОписаниеРеквизита = НовыйОписаниеРеквизита();
			ОписаниеРеквизита.Тип = Новый ОписаниеТипов("Строка");
			ОписаниеРеквизита.Заголовок = "Перевозчик";
			ОписаниеРеквизита.АвтоОтметкаНезаполненного = Истина;
			СтруктураРеквизитовТитула.Вставить(ИмяРеквизита, ОписаниеРеквизита);
	...
	Если ТитулТолькоПроверяемИОтправляем(ТекущийТитул) Тогда
		...
		ДоступныеСостоянияЭДО.Добавить(Перечисления.СостоянияДокументовЭДО.НеСформирован);
...
Функция НавигацияПомощника()
	...
	Если ТитулТолькоПроверяемИОтправляем(ТекущийТитул) Тогда	
		СтруктураНавигации.Вставить(НавигацияПомощниковСВозвратомЭПД.ИмяШага(3), СтруктураШагаОтправка(3));
	Иначе
