...
Функция ПолучитьОперациюДляЗапросаВСервисе(ИдентификаторКатегории, Знач НайтиПутьККатегории, ПолучитьДанныеИзКэша, ВосстановитьДеревоКатегорий)
	...
	ИначеЕсли ЗначениеЗаполнено(СтрокаПоискаКатегорий) И ИдентификаторКатегории = Неопределено Тогда
		...
		ДлительнаяОперация = РаботаСНоменклатуройСлужебныйКлиент.ПоискКатегорийПоСтроке(СтрокаПоискаКатегорий,
			УникальныйИдентификатор, ИдентификаторЗаданияОбработкиРубрикатора, Истина);
...
Процедура ДополнитьДанныеПоНоменклатуре(ДанныеНоменклатуры)
	...
	ДанныеНоменклатуры.Вставить("ВнешниеАдресаИзображений", Новый Массив);

	ТаблицаДанныхНоменклатурыСервиса = РаботаСНоменклатурой.
		ДанныеНоменклатурыСервиса(ДанныеНоменклатуры.ИдентификаторНоменклатуры);

	Если Не ЗначениеЗаполнено(ТаблицаДанныхНоменклатурыСервиса) Тогда
		Возврат;
	КонецЕсли;
	
	ДанныеНоменклатурыСервиса = ТаблицаДанныхНоменклатурыСервиса[0];
	
	ДанныеНоменклатуры.ИдентификаторКатегории       = ДанныеНоменклатурыСервиса.Категория.Идентификатор;
	ДанныеНоменклатуры.НаименованиеКатегории        = ДанныеНоменклатурыСервиса.Категория.Наименование;
	ДанныеНоменклатуры.НаименованиеЕдиницыИзмерения = ДанныеНоменклатурыСервиса.ЕдиницаИзмерения.Наименование;
	ДанныеНоменклатуры.КодЕдиницыИзмерения          = ДанныеНоменклатурыСервиса.ЕдиницаИзмерения.ОКЕИ;
	
	Для Каждого Изображение Из ДанныеНоменклатурыСервиса.Изображения Цикл
		ДанныеНоменклатуры.ВнешниеАдресаИзображений.Добавить(Изображение.ИзображениеURL);
	КонецЦикла;
