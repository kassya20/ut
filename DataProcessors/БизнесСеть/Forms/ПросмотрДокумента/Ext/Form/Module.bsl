...
Процедура ЗагрузитьДокументЭДО()
	...
	СпособОбмена = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.Через1СБизнесСеть");
	
	Если СпособЗагрузкиДокумента = 0 Тогда
		ОбменСКонтрагентамиИнтеграцияКлиент.ОтразитьДокументВУчете(ОповещениеОкончанияОтраженияВУчете, ОписаниеПакета,
			Неопределено, Контрагент, ТипОбъекта, СпособОбмена);
	Иначе
		ОбменСКонтрагентамиИнтеграцияКлиент.ОтразитьДокументВУчете(ОповещениеОкончанияОтраженияВУчете, ОписаниеПакета,
			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ДокументИБ), Контрагент, ТипОбъекта, СпособОбмена);
	КонецЕсли;
...
Функция УстановитьСтатусЗагруженВФоне(Организация, Идентификатор, ДокументСсылка)
	...
	Возврат ДлительныеОперации.ВыполнитьПроцедуру(
		ПараметрыВыполнения, 
		"БизнесСеть.УстановитьСтатусЗагружен", 
		Организация, 
		Идентификатор, 
		ДокументСсылка);
