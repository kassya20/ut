...
Процедура ДокументНажатие(Элемент, СтандартнаяОбработка)
	...
	Если ТаблицаДанных.Количество() > 1 Тогда
		...
		Для Каждого СтрокаДанных Из ТаблицаДанных Цикл
			...
			Если ЭтоАдресВременногоХранилища(СтрокаДанных.АдресХранилищаКартинкиQRКода) Тогда
...
Процедура ОбновитьФайлПредставления()
	...
	Для Каждого СтрокаДанных Из ТаблицаДанных Цикл
		...
		Если ДанныеQRКода = Неопределено 
			Или Отказ Тогда
			Продолжить;
		КонецЕсли;
		
		ПечатнаяФорма = БизнесСеть.ФайлПредставленияПрикладногоДокументаСQRКодом(СтрокаДанных.ВладелецЭД,
			Организация, СтрокаДанных.ВидЭД, ДанныеQRКода);
		
		Если ПечатнаяФорма <> Неопределено Тогда
			ДвоичныеДанные = ОбщегоНазначенияБЭД.ДвоичныеДанныеТабличногоДокумента(ПечатнаяФорма);
			
			СтрокаДанных.АдресХранилищаПредставления  = ПоместитьВоВременноеХранилище(ДвоичныеДанные);
