...
Процедура ЗагрузитьДеревоФорматов(Дерево, СхемаОбеспечения, Номенклатура, Характеристика, СкладыДляОбновления, ФорматыДляОбновления) Экспорт
	...
	Запрос.Текст =
		...
		|	ЕСТЬNULL(СпрСпособ.ВариантОбеспечиваемогоПериода, ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечиваемогоПериода.ПустаяСсылка)) КАК ВариантОбеспечиваемогоПериода,
		...
		|	ЕСТЬNULL(СпрСпособФормата.ВариантОбеспечиваемогоПериода, ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечиваемогоПериода.ПустаяСсылка)) КАК ВариантОбеспечиваемогоПериодаФормата,
			...
			Если Формат <> Выборка.Формат Тогда
				...
				Для Каждого ВетвьКорня Из Корень Цикл
					Если Формат = ВетвьКорня.ФорматСклад
						...
						ВетвьФормата.ВариантОбеспечиваемогоПериода = Выборка.ВариантОбеспечиваемогоПериодаФормата;
	...
	Иначе
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Формат <> Выборка.Формат Тогда
				...
				УзелКорня.ВариантОбеспечиваемогоПериода = Выборка.ВариантОбеспечиваемогоПериодаФормата;
