...
Процедура ДобавитьИзФайла(Команда)
	...
	ПараметрыДобавления.Вставить("МаксимальныйРазмер", 0);  
	ПараметрыДобавления.Вставить("ФильтрДиалогаВыбора", Фильтр);
	ПараметрыДобавления.Вставить("НеОткрыватьКарточку", Истина);
	
	РежимСоздания = 2;
	
	РаботаСФайламиКлиент.ДобавитьФайл(Оповещение, Документ, ЭтотОбъект, РежимСоздания, ПараметрыДобавления);
...
Процедура ДобавитьИзФайлаПослеВыбораФайла(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(Результат) Тогда
		...
		Если ТипЗнч(Результат) = Тип("Структура")
			И Файл.Свойство("ФайлСсылка")  Тогда

			Файл = Файл.ФайлСсылка;
		КонецЕсли;
...
Функция ПроверитьПриложенныеДокументыРасчетРазмера(ПроверятьВидДокументаТипФайла = Ложь, ВыводитьСообщениеОПревышенииРазмера = Ложь)
	...
	Если ПриложенныеДокументы.Количество() > 0 Тогда
		...
		ОстатокПревышение = Макс(ЕщеМожноПриложить, -ЕщеМожноПриложить);
		...
		НадписьОбщийРазмерФайлов = СтрШаблон(Шаблон,
			ОбщийРазмерДляВывода, ОстатокПревышение, ЕдиницаОбщегоРазмера, ЕдиницаЕщеПриложить);
