...
Функция ДанныеДополнительныхПолей(ИдентификаторКонтрагента, ПараметрыЗаполнения) Экспорт
	...
	Формат = ПараметрыЗаполнения.НастройкиОбмена.Формат;
	
	Если НЕ ЭтоОператорРБ(ИдентификаторКонтрагента) ИЛИ ФорматыДляДополнительныхДанных()[Формат] = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	ИначеЕсли Формат = Форматы[ПоддерживаемыеФорматы.ФНС.УКД2020.ИнформацияПродавца] Тогда
		...
		Основания = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(
			ПараметрыЗаполнения.ОписаниеДанныхЭД, "ДокументыОснованияСчетаФактуры");
		...
		Если ЗначениеЗаполнено(Основания) Тогда
			...
			Для Каждого Основание Из Основания Цикл // ДокументСсылка
				Если ТипЗнч(Основание) = ТипЗнч(ПараметрыЗаполнения.СсылкаНаОбъект) Тогда
					СсылкаНаОбъект = Основание; // ОпределяемыйТип.ОснованияЭлектронныхДокументовЭДО
					Прервать;
				КонецЕсли;
			КонецЦикла;
	...
	Если ЗначениеЗаполнено(СсылкаНаОбъект) Тогда
		Результат.Вставить(ИмяПоляИдентификатора(), Строка(СсылкаНаОбъект.УникальныйИдентификатор()));
		Возврат Результат;
	КонецЕсли;
