...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.Контрагент) Тогда
		...
		Если ЗначениеЗаполнено(ИННКонтрагента) Тогда
			...
			ВидимостьПоискаКонтрагентов = Ложь;
		Иначе
			ВидимостьПоискаКонтрагентов = Истина;
		КонецЕсли;
	...
	Если СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(СтрокаПоиска)
	...
	НастройкиФормы.Вставить("Отказ", Отказ);
	НастройкиФормы.Вставить("СтандартнаяОбработка", СтандартнаяОбработка);
	НастройкиФормы.Вставить("ВидимостьПоискаКонтрагентов", ВидимостьПоискаКонтрагентов);
	
	ИнтеграцияПодсистемБИП.ПриСозданииНаСервереПроверкаКонтрагента(ЭтотОбъект, НастройкиФормы);
	ПроверкаКонтрагентаПереопределяемый.ПриСозданииНаСервере(ЭтотОбъект, НастройкиФормы);
	
	Если НастройкиФормы.Отказ Тогда
		Отказ = Истина;
	КонецЕсли;
	
	СтандартнаяОбработка = НастройкиФормы.СтандартнаяОбработка;
	Элементы.ГруппаПоискКонтрагентов.Видимость = НастройкиФормы.ВидимостьПоискаКонтрагентов;
...
&НаКлиенте
Процедура Подключаемый_ДекорацияРуководитель0СведенияОбработкаНавигационнойСсылки(
		Элемент,
...
Процедура СформироватьОтчетНаКлиенте(ТекстПоиска, ЭтоПроверкаРезультата = Ложь)
	...
	Если ПоискПоИНН Тогда
		...
		Если ОперацияЗагрузкиСостоянияФНС <> Неопределено Тогда
			ПараметрыОжидания = ДлительныеОперацииКлиент.ПараметрыОжидания(ЭтотОбъект);
			ПараметрыОжидания.ВыводитьОкноОжидания = Ложь;
			ДлительныеОперацииКлиент.ОжидатьЗавершение(
				ОперацияЗагрузкиСостоянияФНС,
				Новый ОписаниеОповещения("ПриЗавершенииЗагрузкиСостоянияФНС", ЭтотОбъект),
				ПараметрыОжидания);
		КонецЕсли;
...
Процедура ЗагрузитьПодготовленныеДанные()
	...
	ОжиданиеОтвета = ПустаяСтрока(ОписаниеОшибки)
		...
		Или СостояниеФормированияОтчета.СостояниеФакторыРиска = "Ожидание");
...
Процедура ВывестиСтраницуПодразделения(ДанныеПроверкиКонтрагента)
	...
	Иначе
		...
		Элементы.РезультатПодразделения.ИспользуемоеИмяФайла =
			СокрЛП(ДанныеПроверкиКонтрагента.ИмяФайлаПодразделения);
...
Процедура ОчиститьСодержимоеДокументов()
	...
	ОчиститьИСкрытьТабличныйДокумент("ВидыДеятельности");
	ОчиститьИСкрытьТабличныйДокумент("ДанныеГосРеестров");
	ОчиститьИСкрытьТабличныйДокумент("СвязанныеОрганизации");
	
	ОчиститьИСкрытьТабличныйДокумент("АнализОтчетности");
	ОчиститьИСкрытьТабличныйДокумент("БухгалтерскийБаланс");
	ОчиститьИСкрытьТабличныйДокумент("ОтчетОПрибыляхИУбытках");
	ОчиститьИСкрытьТабличныйДокумент("ОтчетОбИзмененияхКапитала");
	ОчиститьИСкрытьТабличныйДокумент("ОтчетОДвиженииДенежныхСредств");
	ОчиститьИСкрытьТабличныйДокумент("РасчетСтоимостиЧистыхАктивов");
	ОчиститьИСкрытьТабличныйДокумент("ИспользованиеСредств");
	
	ОчиститьИСкрытьТабличныйДокумент("ФинансовыйАнализ");
	ОчиститьИСкрытьТабличныйДокумент("Налоги");
	ОчиститьИСкрытьТабличныйДокумент("Проверки");
	ОчиститьИСкрытьТабличныйДокумент("СобытияМониторинга");
	ОчиститьИСкрытьТабличныйДокумент("Лицензии");
	ОчиститьИСкрытьТабличныйДокумент("Подразделения");
	ОчиститьИСкрытьТабличныйДокумент("ДанныеПрограммы");
	ОчиститьИСкрытьТабличныйДокумент("ДанныеОбАрбитражныхДелах");
	ОчиститьИСкрытьТабличныйДокумент("ДанныеГосударственныеКонтракты");
	ОчиститьИСкрытьТабличныйДокумент("ФакторыТребующиеВнимания");
	
	ОчиститьИСкрытьТабличныйДокумент("ИзмененияНаименования");
	ОчиститьИСкрытьТабличныйДокумент("ИзмененияКПП");
	ОчиститьИСкрытьТабличныйДокумент("ИзмененияРуководителя");
	ОчиститьИСкрытьТабличныйДокумент("ИзмененияАдрес");
	ОчиститьИСкрытьТабличныйДокумент("ИзмененияФИО");
...
&НаСервере
Процедура ОчиститьИСкрытьТабличныйДокумент(ИмяРеквизита)
	
	ОчиститьТабличныйДокумент("Результат" + ИмяРеквизита);
	Элементы["Страница" + ИмяРеквизита].Видимость = Ложь;
...
Функция Группы1СКонтрагент()
	...
	Результат.Добавить(Элементы.ГруппаОсновныеРеквизиты);
...
Процедура ВывестиЗначениеСтатуса(Сведения)
	...
	ДекорацияСтатус = Элементы.ДекорацияСтатусЕГРЮЛ;
	Если Сведения.Статус.Код <> Неопределено Тогда
		Если Сведения.Статус.Код = "001" Тогда
			
			Если ЭтоЮридическоеЛицо Тогда
				ДекорацияСтатус.Заголовок = НСтр("ru='Действующая организация (по данным ЕГРЮЛ)'");
			Иначе
				ДекорацияСтатус.Заголовок = НСтр("ru='Действующий ИП (по данным ЕГРИП)'");
			КонецЕсли;
			
			ДекорацияСтатус.ЦветТекста = ЦветаСтиля.ДосьеХорошаяОценкаЦвет;
		Иначе
			
			Если ЭтоЮридическоеЛицо Тогда
				ДекорацияСтатус.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			ДекорацияСтатус.ЦветТекста = ЦветаСтиля.ДосьеПлохаяОценкаЦвет;
			
		КонецЕсли;
	КонецЕсли;
	
	Обработки.ПроверкаКонтрагента.ПолучитьСостояниеКонтрагентаПоДаннымЕГРН(ЭтотОбъект, Сведения);
	Элементы.ДекорацияСтатусФНС.Видимость = Ложь;
	Элементы.КартинкаОжиданиеЗагрузкиСтатусаИзФНС.Видимость = Истина;
		...
		Для Итератор = 0 По ВГраница Цикл
			...
			Если Не ПустаяСтрока(ИнформацияОРуководителе) Тогда
				...
				ЭлементФормы.УстановитьДействие(
					...
					"Подключаемый_ДекорацияРуководитель0СведенияОбработкаНавигационнойСсылки");
			Иначе
...
Процедура ВывестиКарточкуИзмененияВЕГР(Сведения)
	...
	Если ИзмененияВРеестре.Количество() = 0 Тогда
		...
		Иначе
			Элементы.ДекорацияВсеИзмененияЕГР.Заголовок = НСтр("ru = 'Все изменения ЕГРИП'");
		КонецЕсли;
	...
	Для Индекс = 0 По 2 Цикл
		Элементы["ГруппаИзменениеВЕГР" + Индекс].Видимость = Ложь;
	КонецЦикла;
...
Процедура УстановитьЗначения(Диаграмма, МассивЗначений, Серия = 0)
	...
	Для ИндексТочки = 0 По Диаграмма.КоличествоТочек - 1 Цикл
		...
		Стр1 = Новый ФорматированнаяСтрока(
			"" + СокращенноеЗначениеЧисла(ТекущееЗначение),
			ВажнаяНадписьШрифт);
		...
		Диаграмма.УстановитьЗначение(
			ИндексТочки,
			Серия,
			МассивЗначений[ИндексТочки],
			,
			Новый ФорматированнаяСтрока(Стр1, Стр2, Стр3, Стр4));
	КонецЦикла;
КонецПроцедуры
...
Функция СокращенноеЗначениеЧисла(
	...
	Если Цел(Множитель * Значение / 1000000000000) > 0 Тогда
		Сокращение = НСтр("ru='трлн'");
		Значение = Окр(Значение / 1000000000000, 1);
	ИначеЕсли Цел(Множитель * Значение / 1000000000) > 0 Тогда
		Сокращение = НСтр("ru='млрд'");
		Значение = Окр(Значение / 1000000000, 1);
	ИначеЕсли Цел(Множитель * Значение / 1000000) > 0 Тогда
		Сокращение = НСтр("ru='млн'");
		Значение = Окр(Значение / 1000000, 1);
	ИначеЕсли Цел(Множитель * Значение / 1000) > 0 Тогда
		Сокращение = НСтр("ru='тыс'");
...
&НаКлиенте
Процедура ПриЗавершенииЗагрузкиСостоянияФНС(Результат, ДополнительныеПараметры) Экспорт
	
	Элементы.КартинкаОжиданиеЗагрузкиСтатусаИзФНС.Видимость = Ложь;
	ОперацияЗагрузкиСостоянияФНС = Неопределено;
	
	Если Результат <> Неопределено
		И Результат.Статус = "Выполнено" Тогда
		
		ВывестиЗначениеСтатусаФНС();
...
&НаСервере
Процедура ВывестиЗначениеСтатусаФНС()
	
	СостояниеПоДаннымЕГРН = ПолучитьИзВременногоХранилища(АдресХранилищаСостоянияФНС);
	ДекорацияСтатус = Элементы.ДекорацияСтатусФНС;
	Если СостояниеПоДаннымЕГРН = ОбщегоНазначения.ПредопределенныйЭлемент(
			"Перечисление.СостоянияСуществованияКонтрагента.КонтрагентЕстьВБазеФНС") Тогда
		
		Если ЭтоЮридическоеЛицо Тогда
			ДекорацияСтатус.Заголовок = НСтр("ru='Организация есть в базе ЕГРН'");
		Иначе
			ДекорацияСтатус.Заголовок = НСтр("ru='ИП есть в базе ЕГРН'");
		КонецЕсли;
	...
	Иначе
		...
		Если Не ЭтоЮридическоеЛицо
			И СостояниеПоДаннымЕГРН = ОбщегоНазначения.ПредопределенныйЭлемент(
				"Перечисление.СостоянияСуществованияКонтрагента.НеДействуетИлиИзмененКПП") Тогда
			
			ДекорацияСтатус.Заголовок = НСтр("ru='Не действует (по данным ЕГРН)'");
			
		Иначе
			
			ДекорацияСтатус.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='%1 (по данным ЕГРН)'"),
				СостояниеПоДаннымЕГРН);
	...
	Элементы.ДекорацияСтатусФНС.Видимость = Истина;
...
Функция РезультатПроверкаКонтрагентаДляПечати()
	...
	ДобавитьТабличныйДокументДляПечати(
		Результат,
		РезультатВидыДеятельности);
	ДобавитьТабличныйДокументДляПечати(
		Результат,
		РезультатЛицензии);
	ДобавитьТабличныйДокументДляПечати(
		Результат,
		РезультатСвязанныеОрганизации);
	ДобавитьТабличныйДокументДляПечати(
		Результат,
	...
	ДобавитьТабличныйДокументДляПечати(
		Результат,
		РезультатАнализОтчетности);
	ДобавитьТабличныйДокументДляПечати(
		Результат,
		РезультатБухгалтерскийБаланс);
	ДобавитьТабличныйДокументДляПечати(
		Результат,
		РезультатОтчетОПрибыляхИУбытках);
	ДобавитьТабличныйДокументДляПечати(
		Результат,
		РезультатОтчетОбИзмененияхКапитала);
	ДобавитьТабличныйДокументДляПечати(
		Результат,
		РезультатОтчетОДвиженииДенежныхСредств);
	ДобавитьТабличныйДокументДляПечати(
		Результат,
		РезультатРасчетСтоимостиЧистыхАктивов);
	ДобавитьТабличныйДокументДляПечати(
		Результат,
		РезультатИспользованиеСредств);
	ДобавитьТабличныйДокументДляПечати(
		Результат,
		ТабличныйДокументСПолнойВидимостью(
			"РезультатДанныеГосРеестров"));
	...
	ДобавитьТабличныйДокументДляПечати(
		Результат,
		РезультатИзмененияНаименования);
	ДобавитьТабличныйДокументДляПечати(
		Результат,
		РезультатИзмененияКПП);
	ДобавитьТабличныйДокументДляПечати(
		Результат,
		РезультатИзмененияРуководителя);
	ДобавитьТабличныйДокументДляПечати(
		Результат,
		РезультатИзмененияАдрес);
	ДобавитьТабличныйДокументДляПечати(
		Результат,
		РезультатИзмененияФИО);
	...
	Если ТабличныйДокумент.ВысотаТаблицы > 1 Тогда
		Если СоздаватьФорматСтрок Тогда
			Для НомерСтроки = 1 По ТабличныйДокумент.ВысотаТаблицы Цикл
				...
				РезультатДляПечати.ВставитьОбласть(ОбластьПрямоугольная, РезультатДляПечати.Область(НачалоНовогоФорматаСтрок, 1));
				РезультатДляПечати.Область(НачалоНовогоФорматаСтрок, ,НачалоНовогоФорматаСтрок).СоздатьФорматСтрок();
				...
				Для Счетчик = 1 По Макс(СтрокаРезультата.ШиринаТаблицы,1) Цикл
					РезультатДляПечати.Область(НачалоНовогоФорматаСтрок, Счетчик).РастягиватьПоГоризонтали = СтрокаРезультата.Область(1, Счетчик).РастягиватьПоГоризонтали;
...
Функция РезультатОбщиеСведения()
	...
	ОбластьСтатусЕГРЮЛСтрока = Макет.ПолучитьОбласть("СтатусФНССтрока");
	ОбластьСтатусЕГРЮЛСтрока.Параметры.СтатусФНС = Элементы.ДекорацияСтатусЕГРЮЛ.Заголовок;
	ОбластьСтатусЕГРЮЛСтрока.Области.СтатусФНС.ЦветТекста = Элементы.ДекорацияСтатусЕГРЮЛ.ЦветТекста;
	Результат.Вывести(ОбластьСтатусЕГРЮЛСтрока);
	...
	Если Элементы.ГруппаСПАРКСобытияМониторинга.Видимость Тогда
...
Процедура АдаптироватьФормуПодМобильныйКлиент()
	...
	КнопкиМеню.Добавить("ДанныеОбАрбитражныхДелах");
	КнопкиМеню.Добавить("ДанныеГосударственныеКонтракты");
	КнопкиМеню.Добавить("ФакторыТребующиеВнимания");
