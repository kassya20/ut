...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ДанныеКарточкиКонтрагента", Истина);
	Методы.Вставить("ОбновитьСобытияМониторингСПАРК", Истина);
	Методы.Вставить("РасширеннаяИнформацияСПАРКРиски", Истина);
...
Процедура СформироватьРезультатБухгалтерскаяОтчетность(
	...
	ПараметрыОтчета.Вставить(
		"ДелатьГруппыСтрок",
		(Сведения.ДанныеБухгалтерскойОтчетности.Количество() > 0));
	ПараметрыОтчета.Вставить("ПериодыОтчетности", ПериодыОтчетности);
	Для Каждого СтрокаТаблицы Из Сведения.ДанныеБухгалтерскойОтчетности Цикл
		...
		Если ПараметрыОтчета.ДелатьГруппыСтрок Тогда
			РезультатБухгалтерскийБаланс.НачатьГруппуСтрок(, Истина);
		КонецЕсли;
		...
		Если ПараметрыОтчета.ДелатьГруппыСтрок Тогда
			РезультатБухгалтерскийБаланс.ЗакончитьГруппуСтрок();
		КонецЕсли;
...
Процедура ВывестиОтчетОПрибыляхИУбытках(
	...
	Если РезультатыБухгалтерскаяОтчетность.РезультатОтчетОПрибыляхИУбытках = Неопределено Тогда
		РезультатОтчетОПрибыляхИУбытках = Новый ТабличныйДокумент;
	Иначе
		РезультатОтчетОПрибыляхИУбытках = РезультатыБухгалтерскаяОтчетность.РезультатОтчетОПрибыляхИУбытках;
	КонецЕсли;
	РезультатОтчетОПрибыляхИУбытках.Вывести(
		Макет.ПолучитьОбласть(ИмяОбластиШапка));
	РезультатОтчетОПрибыляхИУбытках.Вывести(
		Макет.ПолучитьОбласть("ШапкаКомментарий"));
	Если Параметры.ДелатьГруппыСтрок Тогда
		РезультатОтчетОПрибыляхИУбытках.НачатьГруппуСтрок(, Истина);
	КонецЕсли;
	...
	Пока Год >= 1 Цикл
		...
		ОбластьПоказателиГод.Параметры.Заполнить(Параметры.ПериодыОтчетности);
	...
	КонецЦикла;
	Если Параметры.ДелатьГруппыСтрок Тогда
		РезультатОтчетОПрибыляхИУбытках.ЗакончитьГруппуСтрок();
	КонецЕсли;
...
Процедура СлужебнаяПодготовитьДанныеБухгалтерскойОтчетности(Сведения, ПериодыОтчетности)
	...
	Если Сведения.ЕстьОтчетностьГод_3 Тогда
		...
		Если ОтчетностьГод_3.ТипОтчетности <> ТипОтчетности
			...
			ТипОтчетности = ОтчетностьГод_3.ТипОтчетности;
			КодВалютыОтчетности = ОтчетностьГод_3.КодВалютыОтчетности;
		Иначе
	...
	Если Сведения.ЕстьОтчетностьГод_4 Тогда
		...
		Если ОтчетностьГод_4.ТипОтчетности <> ТипОтчетности
			...
			ТипОтчетности = ОтчетностьГод_4.ТипОтчетности;
			КодВалютыОтчетности = ОтчетностьГод_4.КодВалютыОтчетности;
	...
	Если Сведения.ЕстьОтчетностьГод_5 Тогда
		...
		Если ОтчетностьГод_5.ТипОтчетности <> ТипОтчетности
			...
			ТипОтчетности = ОтчетностьГод_5.ТипОтчетности;
			КодВалютыОтчетности = ОтчетностьГод_5.КодВалютыОтчетности;
