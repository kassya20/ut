...
// Типы сторон договора.
//
// Возвращаемое значение:
//  ФиксированнаяСтруктура:
//   * ИностранноеЛицо - Тип - ОбработкаТабличнаяЧастьСтрока.ФорматДокументПредприятия.ИдентификационныеСведенияИностранногоЛица
//   * Организация     - Тип - ОбработкаТабличнаяЧастьСтрока.ФорматДокументПредприятия.ИдентификационныеСведенияЮрЛица
//   * Предприниматель - Тип - ОбработкаТабличнаяЧастьСтрока.ФорматДокументПредприятия.ИдентификационныеСведенияИП
//   * ФизическоеЛицо  - Тип - ОбработкаТабличнаяЧастьСтрока.ФорматДокументПредприятия.ИдентификационныеСведенияФизЛица
//
...
// Возвращаемое значение:
//  ФиксированнаяСтруктура:
//   * КЛАДР         - Тип -  ОбработкаТабличнаяЧастьСтрока.ФорматДокументПредприятия.АдресКЛАДР
//   * ЗаПределамиРФ - Тип - ОбработкаТабличнаяЧастьСтрока.ФорматДокументПредприятия.АдресЗаПределамиРФ
//
...
Функция ТекстСправки() Экспорт
	...
	ТекстСправки = НСтр("ru='Документ предприятия может формироваться по различным видам документов, обеспечивающих деятельность компании.'");
...
// При определении соответствия титулов для двухтитульного формата.
// 
// Параметры:
...
Процедура ПрочитатьДокументИнформации(КонструкторЭД, Знач Объект)
	...
	СвойстваФайлаXDTO = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(Объект, "СвойстваФайлов.СвойстваФайла", , , Истина);
	ПрочитатьСвойстваФайлов(ИнформацияДокумента, СвойстваФайлаXDTO, КонструкторЭД);
...
Процедура ПрочитатьСвойстваФайлов(ИнформацияДокумента, Знач Объект, Знач КонструкторЭД)
	
	Если ТипЗнч(Объект) <> Тип("Массив") Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого СтрокаОбъекта Из Объект Цикл

		Если ТипЗнч(СтрокаОбъекта) <> Тип("ОбъектXDTO") Тогда
			Продолжить;
		КонецЕсли;
		...
		СвойстваФайла.ДатаМодификации = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(СтрокаОбъекта, "ДатаМодификации", ,
			"Дата");
		...
		ИнформацияДокумента.СвойстваФайлов.Добавить(СвойстваФайла);
	КонецЦикла;
...
Процедура ПрочитатьИдентификационныеСведения(ИдентификационныеСведения, Знач Объект, Знач КонструкторЭД)
	...
	ИдентификационныеСведенияXDTO = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(Объект, "ИностранноеЛицо");
	Если ТипЗнч(ИдентификационныеСведенияXDTO) = Тип("ОбъектXDTO") Тогда
	...
	ИдентификационныеСведенияXDTO = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(Объект, "ФизЛицо");
	Если ТипЗнч(ИдентификационныеСведенияXDTO) = Тип("ОбъектXDTO") Тогда
...
Процедура ЗаполнитьДокументИнформации(ДокументXDTO, Ошибки, КонструкторЭД, ТипыОбъектов)
	...
	Если ЗначениеЗаполнено(СоставБумажногоДокумента) Тогда
		СоставБумажногоДокументаXDTO = КонструкторЭД.ПолучитьXDTOОбъект(ТипыОбъектов.Состав);
		ЗаполнитьСоставБумажногоДокумента(СоставБумажногоДокументаXDTO, СоставБумажногоДокумента, Ошибки);
		РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ДокументXDTO, "Состав", СоставБумажногоДокументаXDTO, , Ошибки);
	КонецЕсли;
	...
	Для Каждого СвойстваФайла Из ИнформацияДокумента.СвойстваФайлов Цикл
		СвойстваФайлаXDTO = КонструкторЭД.ПолучитьXDTOОбъект(ТипыОбъектов.СвойстваФайла);
		ЗаполнитьСвойстваФайла(СвойстваФайлаXDTO, СвойстваФайла, Ошибки);
		СвойстваФайловXDTO.СвойстваФайла.Добавить(СвойстваФайлаXDTO);
	КонецЦикла;
	РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(ДокументXDTO, "СвойстваФайлов", СвойстваФайловXDTO, , Ошибки);
...
Процедура ЗаполнитьСторонуДокумента(СторонаДокументаXDTO, Ошибки, Знач ДанныеСтороныДокумента, Знач КонструкторЭД, Знач ТипыОбъектов);
	...
	Если ЗначениеЗаполнено(КонтактныеДанные) Тогда
		...
		РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(СторонаДокументаXDTO, "КонтактныеДанные", КонтактныеДанныеXDTO, , Ошибки);
	КонецЕсли;
