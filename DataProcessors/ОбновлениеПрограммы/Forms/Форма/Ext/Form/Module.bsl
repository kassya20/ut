...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли ОбщегоНазначения.ЭтоВебКлиент()
		...
		СообщениеЖурнала =
			НСтр("ru = 'Обновление программы невозможно. Обновление программы недоступно при работе в режиме веб-клиента и мобильного клиента.'");
		ПолучениеОбновленийПрограммы.ЗаписатьОшибкуВЖурналРегистрации(СообщениеЖурнала);
		ОтобразитьВнутреннююОшибку(
			НСтр("ru = '<b>Обновление программы невозможно</b>
				|Обновление программы недоступно при работе в режиме веб-клиента и мобильного клиента.
	...
	Если ЭтоСценарийСообщениеОНерекомендуемойВерсииПлатформы(ЭтотОбъект) Тогда
		...
		ЗапрещенаРаботаСХостом    = Не ПолучениеОбновленийПрограммы.РазрешенаРаботаСХостом();
...
Процедура ПриОткрытии(Отказ)
	...
	// серверных вызовов.
	ПолучитьИОтобразитьИнформациюОДоступномОбновленииАсинх();
...
Процедура ЗагрузитьДистрибутивПлатформы(Команда)
	...
	ИнтернетПоддержкаПользователейКлиент.ОткрытьСтраницуИнтегрированногоСайта(URLСтраницыПлатформы);
...
Процедура ОбработатьНавигационнуюСсылку(Ссылка)
	...
	ИначеЕсли НРег(Лев(Ссылка, 7)) = "http://"
		...
		ИнтернетПоддержкаПользователейКлиент.ОткрытьСтраницуИнтегрированногоСайта(Ссылка);
...
Процедура ОтправитьСообщениеВТехПоддержку()
	...
	КонецЕсли;
	Если ТипЗнч(ВложенияТехподдержке) = Тип("ФиксированныйМассив") Тогда
		Вложения = Новый Массив(ВложенияТехподдержке);
	Иначе
...
&НаКлиенте
Асинх Процедура ПолучитьИОтобразитьИнформациюОДоступномОбновленииАсинх()
	
	СнятьЗапретРаботыСХостом = Ложь;
	Если ЗапрещенаРаботаСХостом Тогда
		Ответ = Ждать ВопросАсинх(
			НСтр("ru = 'Обращение к сервисам получения обновления программы запрещена настройками.
				|Снять запрет на работу с сервисами получения обновления программы?'"),
			РежимДиалогаВопрос.ДаНет);
		СнятьЗапретРаботыСХостом = (Ответ = КодВозвратаДиалога.Да);
	КонецЕсли;
	
	Состояние(, , НСтр("ru = 'Получение информации о доступном обновлении'"));
	ПолучитьИОтобразитьИнформациюОДоступномОбновленииНаСервере(СнятьЗапретРаботыСХостом);
	Состояние();
...
&НаСервере
Процедура ПолучитьИОтобразитьИнформациюОДоступномОбновленииНаСервере(Знач СнятьЗапретРаботыСХостом)
	
	Если СнятьЗапретРаботыСХостом Тогда
		ПолучениеОбновленийПрограммы.РазблокироватьРаботуСХостом();
		ЗапрещенаРаботаСХостом = Ложь;
	КонецЕсли;
...
Процедура ПриЗавершенииПолученияИУстановкиОбновлений(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ОписательСостояния.КодСостояния = "Ошибка" Тогда
		...
		ИначеЕсли РежимУстановкиПлатформы = 0
			И (ПолучениеОбновленийПрограммыКлиентСервер.ЭтоКодВозвратаОграниченияСистемныхПолитик(КонтекстОбновления.КодВозвратаПрограммыУстановки)
				Или ПолучениеОбновленийПрограммыКлиентСервер.ЭтоКодВозвратаНеустранимойОшибкиПриУстановке(КонтекстОбновления.КодВозвратаПрограммыУстановки)
				Или ПолучениеОбновленийПрограммыКлиентСервер.ЭтоКодВозвратаОшибкиНарушенияДоступа(КонтекстОбновления.КодВозвратаПрограммыУстановки)) Тогда
...
Функция ТекстИнструкцииПоПереходуНаНовуюВерсиюПлатформы(ПараметрСоздатьРезервнуюКопию)
	...
	ВнедренаПодсистемаЗавершенияРаботыПользователей =
	...
	Если ВнедренаПодсистемаРезервногоКопирования
		...
		Если ВнедренаПодсистемаЗавершенияРаботыПользователей Тогда
			Если ЭтоБазоваяВерсияКонфигурации Тогда
				Текст = НСтр(
					"ru = 'Будет автоматически завершена работа <a href=""open:ActiveUsers"">активных пользователей</a> программы, создана резервная копия информационной базы и запущен сеанс работы с программой на новой версии платформы.'");
			Иначе
				Текст = НСтр(
					"ru = 'Будет автоматически завершена работа <a href=""open:ActiveUsers"">активных пользователей</a> программы, создана резервная копия информационной базы и запущен сеанс работы с программой на новой версии платформы.
		...
		Иначе
			Если ЭтоБазоваяВерсияКонфигурации Тогда
				Текст = НСтр(
					"ru = 'Будет автоматически завершена работа активных пользователей программы, создана резервная копия информационной базы и запущен сеанс работы с программой на новой версии платформы.'");
			Иначе
				Текст = НСтр(
					"ru = 'Будет автоматически завершена работа активных пользователей программы, создана резервная копия информационной базы и запущен сеанс работы с программой на новой версии платформы.
					|На компьютерах других пользователей программы необходимо обновить платформу 1С:Предприятие вручную.'");
	...
	ИначеЕсли ВнедренаПодсистемаРезервногоКопирования Тогда
		
		Если ВнедренаПодсистемаЗавершенияРаботыПользователей Тогда
			Если ЭтоБазоваяВерсияКонфигурации Тогда
				Текст = НСтр(
					"ru = 'Чтобы начать работать на новой версии платформы:
					|  - завершите работу всех <a href=""open:ActiveUsers"">активных пользователей</a> программы;
					|  - завершите текущий сеанс работы с программой;
					|  - откройте программу на новой версии платформы.
					|
					|Перед началом работы на новой версии платформы рекомендуется создать резервную копию.'");
			Иначе
				Текст = НСтр(
					"ru = 'Чтобы начать работать на новой версии платформы:
					|  - завершите работу всех <a href=""open:ActiveUsers"">активных пользователей</a> программы;
					|  - завершите текущий сеанс работы с программой;
					|  - установите новую версию платформы на компьютеры других пользователей программы;
					|  - откройте программу на новой версии платформы.
					|
					|Перед началом работы на новой версии платформы рекомендуется создать резервную копию.'");
			КонецЕсли;
		Иначе
			Если ЭтоБазоваяВерсияКонфигурации Тогда
				Текст = НСтр(
					"ru = 'Чтобы начать работать на новой версии платформы:
					|  - завершите работу всех активных пользователей программы;
					|  - завершите текущий сеанс работы с программой;
					|  - откройте программу на новой версии платформы.
					|
					|Перед началом работы на новой версии платформы рекомендуется создать резервную копию.'");
		...
		Если ВнедренаПодсистемаЗавершенияРаботыПользователей Тогда
			Если ЭтоБазоваяВерсияКонфигурации Тогда
				Текст = НСтр(
					...
					|  - откройте программу на новой версии платформы.
					|
					|Перед началом работы на новой версии платформы рекомендуется <a href=""open:RCInstruction"">создать резервную копию</a>.'");
			Иначе
				Текст = НСтр(
					"ru = 'Чтобы начать работать на новой версии платформы:
					|  - завершите работу всех <a href=""open:ActiveUsers"">активных пользователей</a> программы;
...
Функция ИнициализацияПолученияОбновленийНаСервере()
	...
	ИначеЕсли ОписаниеФайловОбновлений <> Неопределено
		...
		Если Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаСообщениеСервиса
			...
			И (ПолучениеОбновленийПрограммыКлиентСервер.ЭтоКодВозвратаОграниченияСистемныхПолитик(КонтекстОбновления.КодВозвратаПрограммыУстановки)
				Или ПолучениеОбновленийПрограммыКлиентСервер.ЭтоКодВозвратаНеустранимойОшибкиПриУстановке(КонтекстОбновления.КодВозвратаПрограммыУстановки)
				Или ПолучениеОбновленийПрограммыКлиентСервер.ЭтоКодВозвратаОшибкиНарушенияДоступа(КонтекстОбновления.КодВозвратаПрограммыУстановки))
