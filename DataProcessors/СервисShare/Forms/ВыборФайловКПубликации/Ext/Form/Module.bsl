...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИзменитьИнтерфейсПодВерсиюТакси();

	ЦветПалитрыКрасный = WebЦвета.Красный;
...
Процедура РассчитатьИтогиВыбранныхДополнительныхФайлов()
	...
	УстановитьТекстВПодвалеДополнительныеФайлы();
...
&НаКлиенте
Процедура УстановитьТекстВПодвалеДополнительныеФайлы()
	...
	Если РазмерПакетаВБайтах > МаксимальныйРазмерПакетаВБайтах Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДополнительныеФайлыРазмер",
			"ЦветТекстаПодвала", ЦветПалитрыКрасный);
	Иначе
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДополнительныеФайлыИмяФайла",
			"ТекстПодвала", "");
...
&НаСервере
Процедура ИзменитьИнтерфейсПодВерсиюТакси()
	
	ЭтоВерсияТакси = КлиентскоеПриложение.ТекущийВариантИнтерфейса() = ВариантИнтерфейсаКлиентскогоПриложения.Такси;
	
	Если Не ЭтоВерсияТакси Тогда
		Возврат;
	КонецЕсли;
	
	Элементы.ГруппаДополнительныеФайлы.Ширина = 50;
