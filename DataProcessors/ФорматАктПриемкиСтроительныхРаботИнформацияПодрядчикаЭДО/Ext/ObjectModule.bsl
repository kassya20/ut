...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	НеПоддерживаемыеСтавкиНДС = Новый Массив;
	Для Каждого Работа Из Этотобъект.ДанныеРабот Цикл
		Если ЗначениеЗаполнено(Работа.СтавкаНДС)
			И Не ОбщегоНазначенияБЭДКлиентСервер.СтруктураСодержитЗначение(НалоговыеСтавки, Работа.СтавкаНДС)
			И НеПоддерживаемыеСтавкиНДС.Найти(Работа.СтавкаНДС) = Неопределено Тогда
				
				НеПоддерживаемыеСтавкиНДС.Добавить(Работа.СтавкаНДС);
				
		КонецЕсли;
	КонецЦикла;
	Для Каждого СтавкаНДС Из НеПоддерживаемыеСтавкиНДС Цикл
		ПараметрыОшибки = ОбщегоНазначенияБЭДКлиентСервер.НовыеПараметрыОшибки();
		ШаблонОшибки = НСтр("ru = 'Ставка НДС %1 не поддерживается в данном формате электронного документа.
		|Укажите другую ставку НДС.'");
		ПараметрыОшибки.ТекстОшибки = СтрШаблон(ШаблонОшибки, СтавкаНДС);
