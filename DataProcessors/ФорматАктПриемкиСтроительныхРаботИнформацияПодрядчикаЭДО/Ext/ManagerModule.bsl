...
Функция НалоговыеСтавки() Экспорт
	...
	Данные.Вставить("НДС5", "5%");
	Данные.Вставить("НДС7", "7%");
	Данные.Вставить("НДС10", "10%");
	Данные.Вставить("НДС20", "20%");
	Данные.Вставить("НДС22", "22%");
...
Процедура ЗаполнитьСведенияОВыполненнойРаботе(Объект, Работа, КонструкторЭД, Ошибки)
	...
	Если ТипЗнч(Работа) = ТипыРабот.Работа Тогда
		...
		Если ИнформацияПодрядчика.НастройкиДокумента.СпособРасчетаНДС = СпособыРасчетаНДС().ТолькоВИтогах Тогда
			ОсновнаяСтавка = НалоговыеСтавки.НДС22;
			ОсновнаяСтавкаДо2026 = НалоговыеСтавки.НДС20;
			Если Работа.СтавкаНДС <> ОсновнаяСтавка И Работа.СтавкаНДС <> ОсновнаяСтавкаДо2026 Тогда
...
Процедура ЗаполнитьСуммуНДСВРазбивкеПоСтавкам(Объект, СтавкаИНалог, КонструкторЭД, Ошибки)
	...
	ОсновнаяСтавка = НалоговыеСтавки.НДС22;
	ОсновнаяСтавкаДо2026 = НалоговыеСтавки.НДС20;
	Если Ставка <> ОсновнаяСтавка И Ставка <> ОсновнаяСтавкаДо2026 И ЗначениеЗаполнено(Налог.ПозицииАкта) Тогда
...
Процедура ПрочитатьСведенияОВыполненнойРаботе(Объект, Работа, КонструкторЭД)
	...
	Если ТипЗнч(Работа) = ТипыРабот.Работа Тогда
		...
		Если Не ЗначениеЗаполнено(Ставка) Тогда
			Ставка = РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(Объект, "НалСтВИтог", НалоговыеСтавки().НДС22);
		КонецЕсли;
...
Процедура ПрочитатьСуммуНДСВРазбивкеПоСтавкам(Объект, СтавкаИНалог, КонструкторЭД)
	...
	ОсновнаяСтавка = НалоговыеСтавки.НДС22;
	ОсновнаяСтавкаДо2026 = НалоговыеСтавки.НДС20;
	Если Ставка <> ОсновнаяСтавка И Ставка <> ОсновнаяСтавкаДо2026 И ТипЗнч(ПозицииАкта) = Тип("СписокXDTO") Тогда
...
Функция ДатаИзПредставления(Представление)
	...
	Иначе
		Возврат СтроковыеФункцииКлиентСервер.СтрокаВДату(Представление, ЧастиДаты.ДатаВремя);
	КонецЕсли;
КонецФункции
