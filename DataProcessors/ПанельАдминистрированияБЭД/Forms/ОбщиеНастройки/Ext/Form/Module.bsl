...
Процедура РежимСтрогойПроверкиДоверенностейПриИзменении(Элемент)
	...
	Если РежимСтрогойПроверкиДоверенностей = 1 Тогда
		...
		ШаблонТекста = СтрШаблон("%1" + Символы.ПС + "%2",
...
Процедура ПроверитьПодписиМЧД(ОбновляемыйЭлементФормы)
	...
	ПараметрыОжиданияБЭД.ТекстСообщения = НСтр("ru = 'Выполняется проверка подписей доверенностей.
...
Процедура ОбработатьЗавершениеПроверкиПодписейМЧД(РезультатОперации, ДополнительныеПараметры) Экспорт
	...
	Если РезультатОперации = Истина Тогда
		...
		ТекстСообщения = НСтр("ru = 'Выполнена перепроверка всех МЧД.
									|Включена упрощенная проверка подписей документов по МЧД.
									|	'");
...
Процедура ОбработатьРезультатПроверкиПодписейМЧДНаСервере(РезультатВыполненияФоновогоЗадания, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(Результат.ДоверенностиКПроверкеНаКлиенте) Тогда
		...
		МашиночитаемыеДоверенностиКлиент.ПроверитьПодписиДоверенностей(
