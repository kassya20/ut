...
Процедура ДекорацияРезультатыВыгрузкиОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "ПерейтиВКаталогВыгрузки" Тогда
		...
		Если Не ДемонстрационныеСценарииКлиентСервер.КаталогНаДискеСуществует(ИмяРеальногоКаталогаВыгрузки(КаталогВыгрузки)) Тогда
			Возврат;
		КонецЕсли;
		
		ФайловаяСистемаКлиент.ОткрытьПроводник(ИмяРеальногоКаталогаВыгрузки(КаталогВыгрузки));
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Функция ИмяРеальногоКаталогаВыгрузки(КаталогВыгрузки)

	Возврат КаталогВыгрузки + "ProcessDescriptionUpload\";
...
Процедура ВыгрузитьОписанияПроцессовВКаталог()
	...
	Иначе
		...
		УдалитьФайлы(ИмяРеальногоКаталогаВыгрузки(КаталогВыгрузки),  ПолучитьМаскуВсеФайлыКлиента());
...
Функция РезультатВыгрузкиНаСервере(ДанныеСкриншотов)
	...
	Попытка
		...
		ПолныйПутьАрхив = КаталогВыгрузкиНаСервере + ИмяФайлаАрхива;
...
Процедура ПослеСохраненияВыгруженногоАрхива(Результат, ДополнительныеПараметры) Экспорт
	...
	СтрокаКаталог = СтроковыеФункцииКлиент.ФорматированнаяСтрока("<a href=""%1"">%2</a>",
	                                                              ...
	                                                              ИмяРеальногоКаталогаВыгрузки(КаталогВыгрузки));
...
Процедура ВыгрузитьОписанияПроцессов()
	...
	Если РезультатВыгрузки.Успешно Тогда
		...
		Если Не ДемонстрационныеСценарииКлиентСервер.КаталогНаДискеСуществует(ИмяРеальногоКаталогаВыгрузки(КаталогВыгрузки)) Тогда
			СоздатьКаталог(ИмяРеальногоКаталогаВыгрузки(КаталогВыгрузки));
		КонецЕсли;
		
		ИмяВыгруженногоАрхива = ИмяРеальногоКаталогаВыгрузки(КаталогВыгрузки) + РезультатВыгрузки.ИмяФайла;
...
Функция ДанныеСкриншотовДляАрхивирования()
	...
	ИмяКаталога = ИмяРеальногоКаталогаВыгрузки(КаталогВыгрузки) 
...
Процедура ПолучитьСкриншотыПоДаннымНавигационныхСсылокВОписаниях(ДанныеДляПолученияСкриншотов)
	...
	ПараметрыОткрытия.Вставить("ИмяКаталога",                         ИмяРеальногоКаталогаВыгрузки(КаталогВыгрузки));
