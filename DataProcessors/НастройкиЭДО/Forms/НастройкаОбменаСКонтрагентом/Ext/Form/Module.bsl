...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Параметры, "НастройкаРегламентаЭДО", Ложь)
		...
		ВерсияФорматаУстановленаВручную = Ложь;
		...
		Если НастройкаДокумента Тогда
			ВерсияФорматаУстановленаВручную = НастройкаОтправки.ВерсияФорматаУстановленаВручную;
		Иначе
		...
		АвтоматическиОбновлятьФорматИзСервисаНастроек = Не ВерсияФорматаУстановленаВручную;
	Иначе
		Если НастройкаОтправки <> Неопределено Тогда
			...
			АвтоматическиОбновлятьФорматИзСервисаНастроек = Не НастройкаОтправки.ВерсияФорматаУстановленаВручную;
	...
	НастройкаФорматЭДПриОткрытии = НастройкаФорматЭД;
	
	ИспользоватьЦентрЭДО = ИспользоватьЦентрЭДОПоИдентификаторуОтправителя(ИдентификаторОтправителя);
	...
	ЕстьДействующийСертификатОрганизации = КриптографияБЭД.ЕстьДействующийСертификатОрганизации(Организация);
	...
	Если Не ПредварительнаяНастройка Тогда
		...
		Элементы.АвтоматическиОбновлятьВерсиюФормата.Видимость = Не НастройкаДокумента;
	КонецЕсли;
	...
	УстановитьВидимостьТекстаПолученияПодписи(ЭтотОбъект);
...
Процедура ИдентификаторПолучателяОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
	...
	Если Не ПустаяСтрока(ИдентификаторЭДО)
		И Элементы.ИдентификаторПолучателя.СписокВыбора.НайтиПоЗначению(ИдентификаторЭДО) = Неопределено Тогда
...
Процедура НастройкаФорматЭДПриИзменении(Элемент)
	...
	АвтоматическиОбновлятьФорматИзСервисаНастроек = Ложь;
	Если НастройкаДокумента Тогда
		ПредупреждатьОРучнойУстановкеФормата = Истина;
	КонецЕсли;
	ПриИзмененииФорматаЭД();
...
&НаКлиенте
Процедура АвтоматическиОбновлятьФорматИзСервисаНастроекПриИзменении(Элемент)
	
	НастройкиИзменены = Истина;
	Если АвтоматическиОбновлятьФорматИзСервисаНастроек
		И Элементы.НастройкаФорматЭД.СписокВыбора.Количество() Тогда
		НастройкаФорматЭД = Элементы.НастройкаФорматЭД.СписокВыбора[0].Значение;
		ПриИзмененииФорматаЭД(Истина);
	КонецЕсли;
	ОбновитьЭтапыМастераНастройкиПодключения(ЭтотОбъект);
...
Процедура ОтправитьПриглашение()
	...
	Приглашение.ИспользоватьЦентрЭДО     = ИспользоватьЦентрЭДО;
	...
	СинхронизацияЭДОКлиент.ОтправитьПриглашения(Приглашения, ЭтотОбъект, Оповещение);
...
Процедура ПринятьПриглашение()
	...
	Если ЗначениеЗаполнено(ПриглашениеНаИдентификатор.КлючПриглашения) Тогда
		...
		ВходящееПриглашение.ИспользоватьЦентрЭДО = ИспользоватьЦентрЭДО;
...
Функция ТекущаяНастройкаОтправки()
	...
	НастройкаОтправки.ВерсияФорматаУстановленаВручную = Не АвтоматическиОбновлятьФорматИзСервисаНастроек;
...
Процедура Готово(Команда)
	...
	ИначеЕсли (ПредварительнаяНастройка И ПроверкаОтправкаПриглашений = "Отправлено"
		...
		Отказ = Ложь;
		СоздатьНастройкиОтправкиЭДО(Организация, Контрагент, ДоговорКонтрагента, ИдентификаторОтправителя, ИдентификаторПолучателя, Отказ);
...
// Отображает текст с пояснениями о получении ЭП только для юридических лиц или для физических и юридических лиц.
// Параметры:
//  Форма - ФормаКлиентскогоПриложения
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьВидимостьТекстаПолученияПодписи(Форма)
	Элементы = Форма.Элементы;
	Предлагать1СПодпись = Форма.ЕстьДействующийСертификатОрганизации;
	Если Форма.ПроверкаСертификатУстановлен = "Требуется" И Форма.ПредварительнаяНастройка Тогда
		Элементы.ГруппаДопПоляПолучениеФЛ.Видимость = Предлагать1СПодпись;
		Элементы.ГруппаДопПоляПолучениеЮЛ.Видимость = Не Предлагать1СПодпись;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбновитьЭтапыМастераНастройкиПодключения(Форма)
	...
	Форма.Элементы.НастройкаФорматЭД.ТолькоПросмотр = (Форма.АвтоматическиОбновлятьФорматИзСервисаНастроек
		И Не Форма.НастройкаДокумента) Или Не Форма.ЕстьПравоНастройкиОбмена;
	Элементы.СохранитьНастройки.ОтображениеПодсказки = ?(Форма.ПредупреждатьОРучнойУстановкеФормата,
...
Процедура ОбновитьДанныеПриглашенияКонтрагента(Форма, ПараметрыПриглашения = Неопределено)
	...
	Если ЗначениеЗаполнено(ПараметрыПриглашения.ИдентификаторОрганизации) Тогда
		...
		Форма.ИспользоватьЦентрЭДО = ИспользоватьЦентрЭДОПоИдентификаторуОтправителя(Форма.ИдентификаторОтправителя);
	КонецЕсли;
...
Процедура УстановитьДоступность()
	...
	Иначе
		Если ЭтоФормализованныйФорматБезОтветногоТитула(НастройкаФорматЭД) Тогда
			НастройкаОжидатьОтветнуюПодпись = Ложь;
			Элементы.НастройкаОжидатьОтветнуюПодпись.Доступность = Ложь;
		Иначе
...
Процедура УстановитьЗначенияПолейРегламентаЭДО()
	...
	СформироватьПредставлениеРегламентаЭДО(ЭтотОбъект);
	ИнициализироватьНастройкиЗаполненияДополнительныхПолей();
	ПеренестиНастройкиЗаполненияДополнительныхПолейПриИзмененииФормата();
	УстановитьВидимостьЕстьНастройкиЗаполненияДополнительныхПолей();
	ОбновитьЭтапыМастераНастройкиПодключения(ЭтотОбъект);
...
&НаСервере
Процедура ПеренестиНастройкиЗаполненияДополнительныхПолейПриИзмененииФормата()
	
	Если Не ЕстьНастройкиЗаполненияДополнительныхПолейДляФорматаПриОткрытии
		Или ЕстьНастройкиЗаполненияДополнительныхПолей Тогда
		Возврат;
	КонецЕсли;
	
	ДанныеДляПереносаНастроек = НастройкиОтправкиЭДО.НовыеДанныеДляПереносаНастроекЗаполненияДополнительныхПолей();
	ДанныеДляПереносаНастроек.Отправитель = Организация;
	ДанныеДляПереносаНастроек.Получатель = Контрагент;
	ДанныеДляПереносаНастроек.Договор = ДоговорКонтрагента;
	ДанныеДляПереносаНастроек.ВидДокумента = ВидДокумента;
	ДанныеДляПереносаНастроек.СтарыйФормат = НастройкаФорматЭДПриОткрытии;
	ДанныеДляПереносаНастроек.НовыйФормат = НастройкаФорматЭД;

	КлючНастроек = РегистрыСведений.НастройкиЗаполненияДополнительныхПолей.НовыйКлючНастройки();
	КлючНастроек.Отправитель = Организация;
	КлючНастроек.Получатель = Контрагент;
	КлючНастроек.Договор = ДоговорКонтрагента;
	КлючНастроек.ВидЭлектронногоДокумента = ВидДокумента;
	КлючНастроек.Формат = НастройкаФорматЭДПриОткрытии;
	ДанныеДляПереносаНастроек.Настройка = 
		РегистрыСведений.НастройкиЗаполненияДополнительныхПолей.НастройкаЗаполненияПолей(КлючНастроек);
	НастройкиОтправкиЭДО.ПеренестиНастройкиЗаполненияДополнительныхПолей(ДанныеДляПереносаНастроек);
	
	ЕстьНастройкиЗаполненияДополнительныхПолей = Истина;
	
КонецПроцедуры

&НаСервере
Функция КоличествоКонтрагентовСНастройкойФорматаДляИзменения()
	
	Возврат НастройкиОтправкиЭДО.КоличествоКонтрагентовСЗаданнымиНастройками(ВидДокумента, НастройкаФорматЭДПриОткрытии);
	
КонецФункции

&НаКлиенте
Асинх Процедура ПриИзмененииФорматаЭД(ЭтоИзмененияПризнакаАвтоматическогоОбновленияФормата = Ложь)
	
	Если НастройкаФорматЭД = НастройкаФорматЭДПриОткрытии Тогда
		ПриИзмененииФорматаНаСервере();
		Возврат;
	КонецЕсли;
	
	КоличествоКонтрагентовСИдентичнойНастройкой = КоличествоКонтрагентовСНастройкойФорматаДляИзменения();
	
	Если КоличествоКонтрагентовСИдентичнойНастройкой <= 1
		Или Не ЕстьПравоНастройкиОбмена
		Или НастройкаДокумента Тогда
		ПриИзмененииФорматаНаСервере();
		Возврат;
	КонецЕсли;
	
	ВыбранныйФормат = Элементы.НастройкаФорматЭД.СписокВыбора.НайтиПоЗначению(НастройкаФорматЭДПриОткрытии);
	Если ВыбранныйФормат = Неопределено Тогда
		ПриИзмененииФорматаНаСервере();
		Возврат;
	КонецЕсли;
	ПредставлениеВыбранногоФормата = ВыбранныйФормат.Представление;
		
	СтрокаСКоличествомКонтрагентов = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
		НСтр("ru=';%1 контрагентом;;%1 контрагентами;%1 контрагентами;'"),
		КоличествоКонтрагентовСИдентичнойНастройкой - 1);
	ТекстВопроса = СтрШаблон(НСтр("ru = 'Формат ""%1"" указан в настройках ЭДО еще с %2.
		|Хотите установить выбранный формат для этих настроек?'"),
		ПредставлениеВыбранногоФормата, СтрокаСКоличествомКонтрагентов);
	
	Ответ = Ждать ВопросАсинх(ТекстВопроса, РежимДиалогаВопрос.ДаНетОтмена);
	
	Если Ответ = КодВозвратаДиалога.Да Тогда
		ИзменитьФорматДляВсехКонтрагентов();
	ИначеЕсли Ответ = КодВозвратаДиалога.Нет Тогда
		ПриИзмененииФорматаНаСервере();
	Иначе
		НастройкаФорматЭД = НастройкаФорматЭДПриОткрытии;
		Если ЭтоИзмененияПризнакаАвтоматическогоОбновленияФормата Тогда
			АвтоматическиОбновлятьФорматИзСервисаНастроек = Ложь;
			ОбновитьЭтапыМастераНастройкиПодключения(ЭтотОбъект);
		КонецЕсли;
	КонецЕсли;
...
Процедура ИнициализироватьНастройкиЗаполненияДополнительныхПолей()
	...
	ЕстьНастройкиЗаполненияДополнительныхПолейДляФорматаПриОткрытии = 
		НастройкиОтправкиЭДО.ЕстьНастройкиЗаполненияДополнительныхПолей(КлючНастроекОтправки,
			НастройкаФорматЭДПриОткрытии);
...
Процедура ЗавершитьРаботуМастера(Результат)
	...
	Если ЭтоФормализованныйФорматБезОтветногоТитула(НастройкаОтправки.Формат) Тогда
		НастройкаОтправки.ТребуетсяОтветнаяПодпись = Ложь;
	КонецЕсли;
...
&НаСервере
Процедура ПриИзмененииФорматаНаСервере()
...
&НаКлиенте
Процедура ИзменитьФорматДляВсехКонтрагентов()
	
	ДлительнаяОперация = ЗапуститьИзменениеФорматаДляВсехКонтрагентов();
	...
	Оповещение = Новый ОписаниеОповещения("ПослеИзмененияФорматаДляВсехКонтрагентов", ЭтотОбъект);
...
&НаСервере
Функция ЗапуститьИзменениеФорматаДляВсехКонтрагентов()
	
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияФункции(УникальныйИдентификатор);
	ПараметрыВыполнения.ОжидатьЗавершение = 1;
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Изменение формата в настройках отправки ЭДО'");

	// См. НастройкиОтправкиЭДО.ИзменитьФорматВНастройкахОтправкиПоВидуДокументаДляВсехКонтрагентов
	Возврат ДлительныеОперации.ВыполнитьПроцедуру(ПараметрыВыполнения,
		"НастройкиОтправкиЭДО.ИзменитьФорматВНастройкахОтправкиПоВидуДокументаДляВсехКонтрагентов",
		ВидДокумента, НастройкаФорматЭДПриОткрытии, НастройкаФорматЭД);
	
КонецФункции

&НаКлиенте
Процедура ПослеИзмененияФорматаДляВсехКонтрагентов(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ПоказатьПредупреждение(, НСтр("ru = 'Изменение формата в настройках ЭДО с контрагентами выполнено'"));
...
Процедура ПодобратьФорматКонтрагента()
	...
	ПодбиратьФормат = АвтоматическиОбновлятьФорматИзСервисаНастроек
...
&НаСервереБезКонтекста
Функция ИспользоватьЦентрЭДОПоИдентификаторуОтправителя(ИдентификаторОтправителя)

	Если Не ЗначениеЗаполнено(ИдентификаторОтправителя) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Возврат УчетныеЗаписиЭДО.ИспользоватьЦентрЭДО(ИдентификаторОтправителя);
	
КонецФункции

&НаСервереБезКонтекста
Функция ЭтоФормализованныйФорматБезОтветногоТитула(Формат)

	Возврат ЭлектронныеДокументыЭДО.ЭтоФормализованныйФорматБезОтветногоТитула(Формат);
