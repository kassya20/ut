...
Процедура СписокПечатныхФормВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "СписокПечатныхФормПараметрПредставление" Тогда
		ДанныеТекущейПечатнойФормы = Элементы.СписокПечатныхФорм.ТекущиеДанные;
		...
		ПараметрыОткрытия.Вставить("ИмяПечатнойФормы", ДанныеТекущейПечатнойФормы.Наименование);
		ПараметрыОткрытия.Вставить("ИдентификаторПечатнойФормы", ДанныеТекущейПечатнойФормы.Идентификатор);
		ПараметрыОткрытия.Вставить("НаименованиеПараметра", ДанныеТекущейПечатнойФормы.НаименованиеПараметра);
		ПараметрыОткрытия.Вставить("ПараметрЗначение", ДанныеТекущейПечатнойФормы.ПараметрЗначение);
		ПараметрыОткрытия.Вставить("ДоступныеЗначения", ДанныеТекущейПечатнойФормы.ДоступныеПараметры);
		// Параметры генерации передаются и хранятся для этикеток, если текущая форма была открыта для одного заказа
		Если ЭтоЗаказДеловыхЛиний() И ДанныеТекущейПечатнойФормы.Идентификатор = 11 Тогда
			ПараметрыОткрытия.Вставить("ПараметрыГенерации", ДанныеТекущейПечатнойФормы.ПараметрыГенерации);
			ПараметрыОткрытия.Вставить("ПараметрыЗаказа", ПараметрыЗаказа);
		КонецЕсли;

		ОповещениеЗакрытия = Новый ОписаниеОповещения("ПараметрыПечатиЗавершение", ЭтотОбъект, ДанныеТекущейПечатнойФормы);
		ОткрытьФорму("Обработка.СервисДоставки.Форма.ПараметрыПечатнойФормы", ПараметрыОткрытия, ЭтотОбъект, , , , ОповещениеЗакрытия,
		...
		Для Каждого ПечатнаяФорма Из СписокПечатныхФорм Цикл
			...
			Если НайденныеСтроки.Количество() > 0 Тогда
				...
				Для Каждого ТекущаяСтрока Из НайденныеСтроки Цикл
					...
					Для Каждого ЭлементСписка Из ПечатнаяФорма.ПараметрыГенерации Цикл
						ПараметрыФормы.ПараметрыГенерации.Добавить(ЭлементСписка.Значение);
					КонецЦикла;
	...
	Если ДоступныеПараметры.Количество() > 0 Тогда
		Для Каждого ПечатнаяФорма Из СписокПечатныхФорм Цикл
			...
			Если ПечатнаяФорма.ДоступныеПараметры.Количество() > 0 Тогда
				...
				ПечатнаяФорма.ПараметрПредставление = СтрШаблон("%1: %2", 
					ПечатнаяФорма.НаименованиеПараметра, ДоступныйПараметрПоУмолчанию.Представление);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
&НаКлиенте
Функция ЭтоЗаказДеловыхЛиний()
	Возврат СервисДоставкиКлиентСервер.ЭтоДеловыеЛинии(ТипГрузоперевозки) 
		И ИдентификаторыЗаказов.Количество() = 1 И ЗначениеЗаполнено(ПараметрыЗаказа);
КонецФункции

&НаКлиенте
Процедура ПараметрыПечатиЗавершение(Результат, ДанныеТекущейСтроки) Экспорт

	Если ЗначениеЗаполнено(Результат) И ТипЗнч(Результат) = Тип("Структура") Тогда
		ЗаполнитьЗначенияСвойств(ДанныеТекущейСтроки, Результат); 
	КонецЕсли;
