...
// 
// Возвращаемое значение:
//  ФиксированныйМассив из Строка - Массив колонок.
//
...
//
// Параметры:
//  Типы			 - Массив из ОписаниеТипов, Строка - см. ОписаниеТипов().
...
// Возвращает структуру параметров для проверки преобразования строки XML
// 
// Возвращаемое значение:
//  Структура - Получить параметры проверки дерева данных:
...
// * Кодировка - Строка - Кодировка для XML
// * ТаблицаСтрок - ТаблицаЗначений - :
// ** Идентификатор - Строка - Идентификатор проверяемой строки
// ** Значение - Строка - Значение проверяемой строки
// ** Длина - Число - Ограничение длины проверяемой строки
// ** Путь - Строка - Путь реквизита в дереве данных
// ** НомерСтроки - Число, Неопределено - Номер строки для табличных частей
//
Функция НовыеПараметрыПроверкиДереваДанных() Экспорт

	ТаблицаСтрок = Новый ТаблицаЗначений;
	ТаблицаСтрок.Колонки.Добавить("Идентификатор", ОбщегоНазначения.ОписаниеТипаСтрока(40));
	ТаблицаСтрок.Колонки.Добавить("Значение", ОбщегоНазначения.ОписаниеТипаСтрока(0));
	ТаблицаСтрок.Колонки.Добавить("Длина", ОбщегоНазначения.ОписаниеТипаЧисло(4));
	ТаблицаСтрок.Колонки.Добавить("Путь", ОбщегоНазначения.ОписаниеТипаСтрока(0));
	ТаблицаСтрок.Колонки.Добавить("НомерСтроки", ОбщегоНазначения.ОписаниеТипаЧисло(4));

	ПараметрыПроверки = Новый Структура;
	ПараметрыПроверки.Вставить("ДатаПоФорматуФНС", Ложь);
	ПараметрыПроверки.Вставить("Кодировка", "windows-1251");
	ПараметрыПроверки.Вставить("ТаблицаСтрок", ТаблицаСтрок);
	
	Возврат ПараметрыПроверки;
	
КонецФункции

// Включить реквизит в проверку длин строк.
// 
// Параметры:
//  СтрокаДерева - СтрокаДереваЗначений - Строка дерева
//  РеквизитЗаполнен - Булево - Признак заполнения реквизита
//  ПараметрыПроверки - Структура: см. НовыеПараметрыПроверкиДереваДанных().
//  НомерСтроки - Неопределено, Число - Номер строки
//
Процедура ВключитьРеквизитВПроверкуДлинСтрок(СтрокаДерева, РеквизитЗаполнен, ПараметрыПроверки, НомерСтроки = Неопределено)
	
	ТипЗначения = СтрокаДерева.ТипЗначения;
	
	Если ЗначениеЗаполнено(ТипЗначения) И ТипЗначения.СодержитТип(Тип("Строка"))
			И ТипЗначения.КвалификаторыСтроки.Длина > 0 И РеквизитЗаполнен Тогда
			
			СтрокаТаблицы = ПараметрыПроверки.ТаблицаСтрок.Добавить();
			СтрокаТаблицы.Идентификатор = "xml-" + Строка(Новый УникальныйИдентификатор);
			СтрокаТаблицы.Значение = СтрокаДерева.Значение;
			СтрокаТаблицы.Длина = ТипЗначения.КвалификаторыСтроки.Длина;
			СтрокаТаблицы.Путь = СтрокаДерева.ПолныйПуть;
...
// Проверяет строковые значение на предмет превышения длины строки учитывая спецсимволы
// 
// Параметры:
//  ДеревоДанных - ДеревоЗначений - Дерево с данными.
//  ПараметрыПроверки - Структура: см. НовыеПараметрыПроверкиДереваДанных().
//  Ошибки - Массив из См. ОбщегоНазначенияБЭДКлиентСервер.НовыеПараметрыОшибки()
//
Процедура ПроверитьСтроковыеПараметрыНаПревышениеДлины(ДеревоДанных, ПараметрыПроверки, Ошибки)
	
	ДлинаТэга = 42; // Длина идентификатора - 40 + символы "<>"
	Пояснение = НСтр("ru='Некоторые символы ("", &, <, >) занимают больше места.'");
	
	Поток = Новый ПотокВПамяти;
	
	Запись = Новый ЗаписьXML();
	Запись.ОткрытьПоток(Поток, Новый ПараметрыЗаписиXML(ПараметрыПроверки.Кодировка,, Ложь, Ложь));
	Запись.ЗаписатьНачалоЭлемента("ДанныеСтрок");
		
	Для Каждого СтрокаТаблицы Из ПараметрыПроверки.ТаблицаСтрок Цикл
		
		Запись.ЗаписатьНачалоЭлемента(СтрокаТаблицы.Идентификатор);
		Запись.ЗаписатьТекст(СтрокаТаблицы.Значение);
		Запись.ЗаписатьКонецЭлемента();
		
	КонецЦикла;
		
	Запись.ЗаписатьКонецЭлемента();	// ДанныеСтрок
	Запись.Закрыть();
		
	Поток.Перейти(0, ПозицияВПотоке.Начало);
	Чтение = Новый ЧтениеДанных(Поток, ПараметрыПроверки.Кодировка);
	Чтение.ПрочитатьСтроку(, "<ДанныеСтрок>");
	
	Для Каждого СтрокаТаблицы Из ПараметрыПроверки.ТаблицаСтрок Цикл
		
		РазделительСтроки  = "</" + СтрокаТаблицы.Идентификатор + ">"; 
		СтрокаИзПотока = Чтение.ПрочитатьСтроку(, РазделительСтроки);
		СтрокаДанных = Прав(СтрокаИзПотока, СтрДлина(СтрокаИзПотока) - ДлинаТэга);
		
		Если СтрДлина(СтрокаДанных) > СтрокаТаблицы.Длина Тогда
			
			СтрокаДерева = ДеревоДанных.Строки.Найти(СтрокаТаблицы.Путь, "ПолныйПуть", Истина);
			
			НомерСтроки = ?(СтрокаТаблицы.НомерСтроки = 0, Неопределено, СтрокаТаблицы.НомерСтроки);
			ЧислоСимволов = ПолучитьСклоненияСтрокиПоЧислу("", СтрокаТаблицы.Длина, "символ", "ЧС=Количественное", "ПД=Винительный")[0];
			ТекстОшибки = СтрШаблон(НСтр("ru='Длина строки превышает допустимый размер %1 %2. %3'"),
				СтрокаТаблицы.Длина, ЧислоСимволов, Пояснение);
			
			ДобавитьОшибкуПроверкиДереваДанных(Ошибки, СтрокаДерева, ТекстОшибки,, НомерСтроки);
...
// Проверяет заполнение обязательных реквизитов дерева и их корректность.
// 
// Параметры:
//  ДеревоДанных - ДеревоЗначений - Дерево с данными. 
//  Ошибки - Массив из См. ОбщегоНазначенияБЭДКлиентСервер.НовыеПараметрыОшибки()
//  НомерСтроки - Число, Неопределено - Номер строки
...
//  ПараметрыПроверки - Структура: см. НовыеПараметрыПроверкиДереваДанных().
// 
Процедура ПроверитьЗаполнениеРеквизитовДереваДанных(ДеревоДанных, Ошибки, 
	НомерСтроки = Неопределено, ПроверятьОбязательностьПараметровОшибки = Истина, ПараметрыПроверки = Неопределено) Экспорт
	
	ЭтоВерхнийУровеньДерева = ТипЗнч(ДеревоДанных) = Тип("ДеревоЗначений");
	ПараметрыПроверки = ?(ПараметрыПроверки = Неопределено, НовыеПараметрыПроверкиДереваДанных(), ПараметрыПроверки);	
		
	Для Каждого СтрокаДерева Из ДеревоДанных.Строки Цикл
		
		ТекстОшибки = "";
		РеквизитЗаполнен = ЗначениеЗаполнено(СтрокаДерева.Значение);
		РеквизитОбязателен = НРег(СтрокаДерева.Обязательное) = "да";
		РеквизитЗаполненИлиОбязателен = РеквизитЗаполнен Или НРег(СтрокаДерева.Обязательное) = "да";
		...
		ПроверитьЗаполнениеРеквизитаДереваДанных(СтрокаДерева, ПроверятьОбязательностьПараметровОшибки, 
			ПараметрыПроверки.ДатаПоФорматуФНС, ТекстОшибки);
		...
		ВключитьРеквизитВПроверкуДлинСтрок(СтрокаДерева, РеквизитЗаполнен, ПараметрыПроверки, НомерСтроки);
		
		// Запускаем проверки подчиненных реквизитов, если это структурированное значение.
		Если СтрокаДерева.Признак = "Таблица" И (ТипЗнч(СтрокаДерева.Значение) = Тип("Число") И СтрокаДерева.Значение > 0 Или РеквизитОбязателен) Тогда
		
			Для Каждого СтрокаТаблицы Из СтрокаДерева.Строки Цикл
				ПроверитьЗаполнениеРеквизитовДереваДанных(СтрокаТаблицы, Ошибки, СтрокаТаблицы.Значение,
					РеквизитЗаполненИлиОбязателен И ПроверятьОбязательностьПараметровОшибки, ПараметрыПроверки);
			КонецЦикла;
			
		ИначеЕсли СтрокаДерева.Признак = "Выбор" И (РеквизитЗаполнен Или РеквизитОбязателен) Тогда
			...
			Если ЗначениеЗаполнено(ПроверяемаяСтрока) Тогда
				...
				ПроверитьЗаполнениеРеквизитаДереваДанных(ПроверяемаяСтрока, ПроверятьОбязательностьПараметровОшибки, 
					ПараметрыПроверки.ДатаПоФорматуФНС, ТекстОшибки);
			
				Если ЗначениеЗаполнено(ТекстОшибки) Тогда
					
					ДобавитьОшибкуПроверкиДереваДанных(Ошибки, ПроверяемаяСтрока, ТекстОшибки,, НомерСтроки);
					Продолжить;
					
				КонецЕсли;
				
				ПроверитьЗаполнениеРеквизитовДереваДанных(ПроверяемаяСтрока, Ошибки,,
					РеквизитЗаполненИлиОбязателен И ПроверятьОбязательностьПараметровОшибки, ПараметрыПроверки);
		...
		ИначеЕсли СтрокаДерева.Признак = "Группа" И СтрокаДерева.Значение = Истина И СтрокаДерева.Строки.Количество() > 0 Тогда
			...
			ПроверитьЗаполнениеРеквизитовДереваДанных(СтрокаДерева, Ошибки,,
				РеквизитЗаполненИлиОбязателен И ПроверятьОбязательностьПараметровОшибки, ПараметрыПроверки);
	...
	Если ЭтоВерхнийУровеньДерева Тогда
		
		ПроверитьСтроковыеПараметрыНаПревышениеДлины(ДеревоДанных, ПараметрыПроверки, Ошибки);
...
//
// Параметры:
//  Ошибки			 				 - Массив из Структура - ошибки, накопленные за время обработки. Элементами массива являются структуры.
...
//
// Возвращаемое значение:
//  Структура, Неопределено - информация для сопоставления товаров.
//
...
//
// Возвращаемое значение:
//  СтрокаТаблицыЗначений - содержит строку дерева.
//
