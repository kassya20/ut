...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ДоступныеОблачныеКассы", Истина);
	Методы.Вставить("ЗагрузкаСтатусовОперацийОблачныхКасс", Истина);
...
//
// Параметры:
//  НастройкаПодключения - СправочникСсылка.НастройкиПодключенияКОблачнымКассам - настройка подключения к 
//    облачной кассе.
...
Функция ТокенСервиса(
	...
	Если Не ЗначениеЗаполнено(НастройкаПодключения) Тогда
		РезультатОперации.СообщениеОбОшибке  = НСтр("ru = 'Не заполнена настройка подключения к облачной кассе.'");
		РезультатОперации.ИнформацияОбОшибке = НСтр("ru = 'Не заполнена настройка подключения к облачной кассе.'");
	...
	ПараметрыНастройки = ПараметрыПодключения(НастройкаПодключения);
	
	ДанныеТокенов = НовыйОписаниеТокеновСервиса();
	ЗаполнитьЗначенияСвойств(ДанныеТокенов, ПараметрыНастройки.ПараметрыАутентификации);
	
	ДанныеТокенаСервиса = ДанныеТокенов.ПотребителиТокенов[ИдентификаторСервиса];
	...
	Иначе
		...
		РезультатВызова = ОблачныеКассыСервис.ОперацияЗапросТокенаАутентификацииСервиса(
			ПараметрыНастройки,
			ИдентификаторТокена,
			ИмяСервиса);
		...
		ПараметрыНастройки.ПараметрыАутентификации.Вставить(
			"ПотребителиТокенов",
			ДанныеТокенов.ПотребителиТокенов);
		
		УстановитьПривилегированныйРежим(Истина);
		ОбщегоНазначения.ЗаписатьДанныеВБезопасноеХранилище(
			НастройкаПодключения,
			ПараметрыНастройки.ПараметрыАутентификации);
...
//
Процедура ОтозватьТокенСервиса(
	НастройкаПодключения,
	РезультатОперации,
	ИмяСервиса) Экспорт
	...
	ПараметрыНастройки = ПараметрыПодключения(НастройкаПодключения);
	ПараметрыАутентификации = ПараметрыНастройки.ПараметрыАутентификации;
	
	ДанныеТокенов = НовыйОписаниеТокеновСервиса();
	ЗаполнитьЗначенияСвойств(ДанныеТокенов, ПараметрыАутентификации);
	ДанныеТокенаСервиса = ДанныеТокенов.ПотребителиТокенов[ИдентификаторСервиса];
	
	Если ПустаяСтрока(ДанныеТокенаСервиса.Токен) Тогда
		Возврат;
	Иначе
		...
		РезультатВызова = ОблачныеКассыСервис.ОперацияОтзывТокеновАутентификацииСервиса(
		...
		ПараметрыАутентификации.ПотребителиТокенов[ИдентификаторСервиса].Идентификатор = "";
		ПараметрыАутентификации.ПотребителиТокенов[ИдентификаторСервиса].Токен = "";
...
//
// Параметры:
//  НастройкаПодключения - Массив Из СправочникСсылка.НастройкиПодключенияКОблачнымКассам -
//  используемые сервисом настройки подключения к облачной кассе.
//  ИмяСервиса - Строка - имя сервиса в котором использовались токены.
//  РезультатОперации - Структура - результат получения токена:
//    * КодОшибки - Строка - строковый код возникшей ошибки, который может быть обработан вызывающим методом.
//    * СообщениеОбОшибке  - Строка, ФорматированнаяСтрока - сообщение об ошибке для пользователя.
//    * ИнформацияОбОшибке - Строка, ФорматированнаяСтрока - сообщение об ошибке для администратора.
//
Процедура ПриРевизииТокенов(ИспользуемыеНастройки, ИмяСервиса, РезультатОперации) Экспорт
	...
	НастройкиПодключения = Справочники.НастройкиПодключенияКОблачнымКассам.НастройкиПодключения();
	
	Для Каждого НастройкаПодключения Из НастройкиПодключения Цикл
		
		Если ИспользуемыеНастройки.Найти(НастройкаПодключения.Ссылка) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ОтозватьТокенСервиса(
...
Процедура ЗаписатьДанныеФискализацииОнлайнЗаказа(
		НастройкаПодключения,
		ДанныеОперации,
		ДанныеДляОбработки) Экспорт
	
	РезультатЧтения = НовыйРезультатОперации();
	РезультатЧтения.Вставить("ДанныеОперации", ОблачныеКассыСервис.НовыйДанныеОперации());
	РезультатЧтения.Вставить("Результат"     , ДанныеДляОбработки);
	РезультатЧтения.ДанныеОперации.ИдентификаторОперации = ДанныеОперации.ИдентификаторОперации;
	Если ЗначениеЗаполнено(ДанныеОперации.СтатусОперации) Тогда
		ОблачныеКассыСервис.ОбработатьОтветВыполненияОперации(РезультатЧтения, ДанныеДляОбработки);
	КонецЕсли;
	
	РегистрыСведений.ДанныеОперацийОблачныхКасс.ЗаписатьДанныеФискализацииОнлайнЗаказа(
		НастройкаПодключения,
		ДанныеОперации.ДокументОперации,
		РезультатЧтения.ДанныеОперации,
		РезультатЧтения.Результат);
	
	Если РезультатЧтения.ДанныеОперации.СтатусОперации = ОблачныеКассыКлиентСервер.СтатусОперацииВыполнена() Тогда
		
		ПараметрыОперации = Новый Структура();
		ПараметрыОперации.Вставить("ИдентификаторОперации"  , РезультатЧтения.ДанныеОперации.ИдентификаторОперации);
		ПараметрыОперации.Вставить("СтатусОперации"         , РезультатЧтения.ДанныеОперации.СтатусОперации);
		ПараметрыОперации.Вставить("ОписаниеСтатусаОперации", РезультатЧтения.ДанныеОперации.ОписаниеСтатусаОперации);
		ПараметрыОперации.Вставить("ДанныеДляОбработки"     , РезультатЧтения.Результат);
		
		ОблачнаяКасса = Справочники.НастройкиПодключенияКОблачнымКассам.ОблачнаяКасса(НастройкаПодключения);
...
// Определяет список настроек подключения облачных касс организации.
//
// Параметры:
//  Организация - ОпределяемыйТип.Организация - организация, по которой должен быть выполнен поиск облачных касс.
//
// Возвращаемое значение:
//  Соответствие - перечень облачных касс с настройками подключения
//    *Ключ - ОпределяемыйТип.ОблачнаяКасса - ссылка на элемент справочника для которого может быть настроена
//      интеграция с Облачной кассой;
//    *Значение - СправочникСсылка.НастройкиПодключенияКОблачнымКассам - настройка подключения для облачной кассы,
//      Неопределено если настройка не выполнена.
//
Функция НастройкиОблачныхКассОрганизации(Организация) Экспорт
	
	Кассы = Новый Массив;
	
	ИнтеграцияПодсистемБИП.ПриПолученииОблачныхКассОрганизации(
		Организация,
		Кассы);
	ОблачныеКассыПереопределяемый.ПриПолученииОблачныхКассОрганизации(
		Организация,
		Кассы);
	...
	Иначе
		Возврат Справочники.НастройкиПодключенияКОблачнымКассам.НастройкиПодключенияПоОблачнымКассам(
			Кассы);
	КонецЕсли;
	
КонецФункции

// Определяет элемент справочника для которого записывается настройка интеграции с Облачной кассой.
//
// Параметры:
//  Организация - ОпределяемыйТип.Организация - организация для которой создается настройка подключения.
//  Наименование - Строка - наименование настройки подключения указанное пользователем.
//
// Возвращаемое значение:
//  ОпределяемыйТип.ОблачнаяКасса - ссылка на элемент справочника для которого записывается настройка
//      интеграции с Облачной кассой;
//
Функция КассаПриЗаписиНастройкиПодключения(Организация, Наименование) Экспорт
	
	Касса = Неопределено;
	
	ИнтеграцияПодсистемБИП.ПриЗаписиНастройкиПодключения(
		Организация,
		Наименование,
		Касса);
	ОблачныеКассыПереопределяемый.ПриЗаписиНастройкиПодключения(
		Организация,
		Наименование,
		Касса);
	
	Если Не ЗначениеЗаполнено(Касса) Тогда
		ВызватьИсключение НСтр("ru = 'Не удалось записать настройку по причине: Не заполнена касса.'");
	КонецЕсли;
...
// Результат выполнения записывается в журнал регистрации в виде статистической информации.
//
...
//    по принципу Resource Owner Password Credentials Grant.
//
...
Функция ПроверитьПараметрыПодключения(Знач ПараметрыПодключения) Экспорт
	...
	СостояниеКассы = ОблачныеКассыСервис.СостояниеКассы(ПараметрыПодключения, Истина);
...
// Выполняет проверку наличие секретных полей в параметрах аутентификации и при наличии выполняет рассекречивание их.
//
// Параметры:
//  ПараметрыПодключения - см. ОблачныеКассыКлиентСервер.НовыйПараметрыПодключения
//
...
Функция ПараметрыАутентификацииСлужебный(НастройкаПодключения)
	...
	Иначе
		...
		Результат.Вставить("ПотребителиТокенов", Новый Структура);
		ЗаполнитьЗначенияСвойств(Результат, НовыйОписаниеТокеновСервиса());
...
Функция НовыйОписаниеТокеновСервиса()
	...
	ОписаниеТокеновСервиса.Вставить("ПотребителиТокенов", ОписаниеСервисов);
...
// Возвращает информацию о актуальных Облачных кассах.
//
// Возвращаемое значение:
//  Структура:
//    * КодОшибки - Строка - строковый код возникшей ошибки, который может быть обработан вызывающим методом.
...
Функция ДоступныеОблачныеКассы() Экспорт
	...
	РезультатСервиса = ОблачныеКассыСервис.ДоступныеОблачныеКассы();
...
Функция СостояниеКассы(ОблачнаяКасса) Экспорт
	...
	СостояниеКассы = ОблачныеКассыСервис.СостояниеКассы(ПараметрыПодключения);
...
//  ПараметрыПробитияЧека - см. ОблачныеКассы.НовыйПараметрыПробитияЧека
//
// Возвращаемое значение:
...
//    * СообщениеОбОшибке - Строка, ФорматированнаяСтрока - сообщение об ошибке для пользователя.
//    * ИнформацияОбОшибке - Строка, ФорматированнаяСтрока - сообщение об ошибке для администратора.
//    * ДанныеОперации - см. ОблачныеКассыСервис.НовыйДанныеОперации
...
//
Функция ПробитьЧек(ДокументОперации, ОблачнаяКасса, ДанныеЧека, ПараметрыПробитияЧека = Неопределено) Экспорт
	
	ОтветСервиса        = Неопределено;	// Неопределено, Структура
	СписокСвойствОшибки = "КодОшибки,СообщениеОбОшибке,ИнформацияОбОшибке";
	
	Результат = НовыйРезультатОперации();
	Результат.Вставить("ДанныеОперации", ОблачныеКассыСервис.НовыйДанныеОперации());
	Результат.Вставить("Результат"     , ОтветСервиса);
	...
	ДополнительныеПараметры = ОблачныеКассы.НовыйПараметрыПробитияЧека();
	Если ПараметрыПробитияЧека <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(ДополнительныеПараметры, ПараметрыПробитияЧека);
	КонецЕсли;
	
	ПараметрыПодключения = ПараметрыПодключения(НастройкаПодключения);
	
	ДанныеЗапроса = ПодготовитьЗапросНаПробитиеЧека(
		ДокументОперации,
	...
	РезультатПробития = ОблачныеКассыСервис.ПробитьЧек(ДокументОперации, ПараметрыПодключения, ДанныеЗапроса.Результат);
...
// Формирование данных запроса выполняется в транзакции и при возникновении ошибки необходимо завершить выполнение всех
// вызывающих методов.
//
// Параметры:
//  ДокументОперации - ОпределяемыйТип.ДокументОперацииОблачнойКассы - ссылка на документ инициирующий пробитие чека.
...
//
Функция ПодготовитьЗапросНаПробитиеЧека(ДокументОперации, ПараметрыПодключения, ДанныеЧека, ПараметрыПробитияЧека)
	...
	// Заполнение идентификатора операции предоплаты
	Если ЗначениеЗаполнено(ПараметрыПробитияЧека.ДокументПредоплаты) Тогда
		ДанныеПредоплаты = РегистрыСведений.ДанныеОперацийОблачныхКасс.ДанныеОперации(
			ПараметрыПробитияЧека.ДокументПредоплаты);
		Если ДанныеПредоплаты = Неопределено Тогда
			
			Результат.КодОшибки          = КодОшибкиПрочаяОшибка();
			Результат.СообщениеОбОшибке  = НСтр("ru = 'Не найдены данные операции предоплаты.'");
			Результат.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не найдены данные операции предоплаты %1.'"),
				Строка(ПараметрыПробитияЧека.ДокументПредоплаты));
			
			ЗаписатьИнформациюВЖурналРегистрации(Результат.ИнформацияОбОшибке, Истина);
			
		Иначе
			ИдентификаторПредоплаты = ДанныеПредоплаты.ИдентификаторОперации;
		КонецЕсли;
	Иначе
		ИдентификаторПредоплаты = "";
	КонецЕсли;
	
	Если ПустаяСтрока(Результат.КодОшибки) Тогда
		
		ПозицияЧека = Новый Структура();
		ПозицияЧека.Вставить("operationId"          , ИдентификаторОперации);
		ПозицияЧека.Вставить("prepaymentOperationId", ИдентификаторПредоплаты);
...
Функция СтатусПробитияЧека(ДокументОперации, ВремяОжидания = 0) Экспорт
	...
	НастройкиВызова      = ОблачныеКассыСервис.НовыйИтеративныйВызовОперации(ВремяОжидания);
	
	СтатусОперации = ОблачныеКассыСервис.СтатусПробитияЧека(
		ПараметрыПодключения,
