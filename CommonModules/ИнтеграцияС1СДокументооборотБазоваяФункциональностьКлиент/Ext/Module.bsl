...
Процедура НачатьФормированиеПодписиОбъекта(ОбъектДО, Файлы, АдресДвоичныхДанныхОбъекта,
	...
	Если Не ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиентПовтИсп.СоздаватьЭПДляОбъектов1СДокументооборотаСредствамиИС() Тогда
		...
		Для Каждого Подпись Из ВладелецПодписи.ПолучитьЭлементы() Цикл
			...
			Если ЗначениеЗаполнено(Подпись.ДатаПроверкиПодписи) Тогда
				Подпись.КомуВыданСертификатИСтатус = СтрШаблон("%1 (%2)",
					...
					Формат(Подпись.ДатаПроверкиПодписи, "ДФ='dd.MM.yyyy HH:mm';"));
			КонецЕсли;
