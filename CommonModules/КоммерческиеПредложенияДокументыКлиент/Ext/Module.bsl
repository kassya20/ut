...
// Открыть форму выбора изображений из присоединенных файлов.
// 
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - Форма объекта, в которой выполняется команда
//  ПараметрыФормы - см. НовыеПараметрыОткрытияФормыВыбораИзображений
//  ОповещениеЗавершения - ОписаниеОповещения, Неопределено - описание оповещения о завершении
//  РежимОткрытия - РежимОткрытияОкнаФормы, Неопределено - Режим открытия окна владельца
Процедура ОткрытьФормуВыбораИзображенийИзПрисоединенныхФайлов(
		Форма, ПараметрыФормы, ОповещениеЗавершения = Неопределено, РежимОткрытия = Неопределено) Экспорт
	
	ИмяФормы  = "ОбщаяФорма.ВыборИзображенийИзПрисоединенныхФайлов";
	
	Если Не ЗначениеЗаполнено(РежимОткрытия) Тогда
		РежимОткрытия = РежимОткрытияОкнаФормы.БлокироватьОкноВладельца;
	КонецЕсли;
	
	ОткрытьФорму(ИмяФормы, ПараметрыФормы, Форма, , , , ОповещениеЗавершения, РежимОткрытия);
	
КонецПроцедуры

// Новые параметры открытия формы выбора изображений.
// 
// Параметры:
//  АдресТаблицыИзображений - Строка - Адрес временного хранилища с таблицей подобранных изображений
// 	ИдентификаторФормы - УникальныйИдентификатор - Идентификатор формы владельца
//  ВыбранныйФайл - ОпределяемыйТип.ПрисоединенныеФайлыИзображенийЗКППоставщика, Неопределено - Ранее выбранный файл 
//  	изображения. Если файл отсутствует - Неопределено.
// 
// Возвращаемое значение:
//  Структура:
//  * АдресТаблицыИзображений - Строка - Адрес временного хранилища с таблицей подобранных изображений
// 	* ИдентификаторФормыВладельца - УникальныйИдентификатор - Идентификатор формы владельца
//  * ВыбранныйФайл - ОпределяемыйТип.ПрисоединенныеФайлыИзображенийЗКППоставщика, Неопределено  - Ранее выбранный файл
// 		изображения. Если файл отсутствует - Неопределено.
Функция НовыеПараметрыОткрытияФормыВыбораИзображений(
		АдресТаблицыИзображений, ИдентификаторФормы, ВыбранныйФайл = Неопределено) Экспорт
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("АдресТаблицыИзображений", АдресТаблицыИзображений);
	ПараметрыФормы.Вставить("ИдентификаторФормыВладельца", ИдентификаторФормы);
...
// Настраивает отображение элементов формы группы ГруппаУправлениеИзображениями и заполнение свойства ТекстИзображения
// 
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - Форма, элементы и их значения которой необходимо настроить
Процедура НастроитьПанельИзображенийФормы(Форма) Экспорт
	
	Если Форма.ИмяФормы <> "Документ.ЗапросКоммерческогоПредложенияОтКлиента.Форма.ФормаДокумента"
		И Форма.ИмяФормы <> "Обработка.ЗапросыКоммерческихПредложений.Форма.КарточкаЗапроса" Тогда
		Возврат;
	КонецЕсли;

	Форма.ТекстИзображения = "";
	
	Если Не Форма.ПанельОтображается Тогда
		Возврат;
	КонецЕсли;
	
	Если Форма.КоличествоИзображенийТекущейСтроки > 1 Тогда
		
		Форма.ТекстИзображения = 
			СтрШаблон("%1/%2", Форма.ИндексТекущегоИзображенияСтроки + 1, Форма.КоличествоИзображенийТекущейСтроки);

		ЭлементПредыдущееИзображение = Форма.Элементы.Найти("ПредыдущееИзображение");
		ЭлементПредыдущееИзображение.Доступность = Форма.ИндексТекущегоИзображенияСтроки > 0;

		ЭлементСледующееИзображение = Форма.Элементы.Найти("СледующееИзображение");
		ЭлементСледующееИзображение.Доступность = 
			Форма.ИндексТекущегоИзображенияСтроки + 1 < Форма.КоличествоИзображенийТекущейСтроки;
			
	КонецЕсли;
	
	ИзменитьВидимость = Форма.КоличествоИзображенийТекущейСтроки < 2 И Форма.КоличествоИзображенийПредыдущейСтроки > 1
		ИЛИ Форма.КоличествоИзображенийТекущейСтроки > 1 И Форма.КоличествоИзображенийПредыдущейСтроки < 2;

	Если ИзменитьВидимость Тогда
								
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Форма.Элементы, "ГруппаУправлениеИзображениями", "Видимость", Форма.КоличествоИзображенийТекущейСтроки > 1);
							
	КонецЕсли;	

	Форма.КоличествоИзображенийПредыдущейСтроки = Форма.КоличествоИзображенийТекущейСтроки;								
	
КонецПроцедуры

Функция ИмяСобытияНовыйДокументЗКППЗаписан() Экспорт
	Возврат "НовыйДокументЗКППЗаписан";	
КонецФункции

Функция ИмяСобытияЗаписатьНовыйДокументЗКПП() Экспорт
	Возврат "ЗаписатьНовыйДокументЗКПП";
КонецФункции
