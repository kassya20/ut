...
//    ** МестоХранения                     - Строка - имя поля документа, по которому нужно делать отбор места хранения
//    ** Грузоотправитель                  - Строка - имя поля документа, по которому нужно делать отбор грузоотправителя.
...
Процедура УстановитьОтборПоОрганизацииСАТУРН(Список, Организации, Префиксы, ИмяПоляОтбора = "ОрганизацияСАТУРН",
	...
	Для Каждого Префикс Из МассивПрефиксов Цикл
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(
			...
			?(Организации.Количество() > 0, ВидСравненияКомпоновкиДанных.ВСписке, ВидСравненияКомпоновкиДанных.Равно),,
...
Функция СтруктураДанныхМестаХранения() Экспорт
	
	ДанныеМестаХранения = Новый Структура;
	
	ДанныеМестаХранения.Вставить("Ссылка",            ПредопределенноеЗначение("Справочник.МестаХраненияСАТУРН.ПустаяСсылка"));
	ДанныеМестаХранения.Вставить("Идентификатор",     "");
	ДанныеМестаХранения.Вставить("Статус",            ПредопределенноеЗначение("Перечисление.СтатусыОбъектовСАТУРН.ПустаяСсылка"));
	ДанныеМестаХранения.Вставить("Наименование",      "");
	ДанныеМестаХранения.Вставить("Адрес",             "");
	ДанныеМестаХранения.Вставить("ОКТМО",             "");
	ДанныеМестаХранения.Вставить("Широта",            "");
	ДанныеМестаХранения.Вставить("Долгота",           "");
	ДанныеМестаХранения.Вставить("ВидОтношений",      ПредопределенноеЗначение("Перечисление.ВидыИспользуемыхТерриторийСАТУРН.ПустаяСсылка"));
	
	Возврат ДанныеМестаХранения;
	
КонецФункции

Функция СтруктураДанныхМестаПрименения() Экспорт
	
	ДанныеМестаПрименения= Новый Структура;
	
	ДанныеМестаПрименения.Вставить("Ссылка",             ПредопределенноеЗначение("Справочник.МестаПримененияСАТУРН.ПустаяСсылка"));
	ДанныеМестаПрименения.Вставить("Идентификатор",      "");
	ДанныеМестаПрименения.Вставить("Статус",             ПредопределенноеЗначение("Перечисление.СтатусыОбъектовСАТУРН.ПустаяСсылка"));
	ДанныеМестаПрименения.Вставить("Наименование",       "");
	ДанныеМестаПрименения.Вставить("Адрес",              "");
	ДанныеМестаПрименения.Вставить("ОКТМО",              "");
	ДанныеМестаПрименения.Вставить("Широта",             "");
	ДанныеМестаПрименения.Вставить("Долгота",            "");
	ДанныеМестаПрименения.Вставить("ТипМестаПрименения", ПредопределенноеЗначение("Перечисление.ТипыМестПримененияСАТУРН.ПустаяСсылка"));
	ДанныеМестаПрименения.Вставить("ВидОтношений",       ПредопределенноеЗначение("Перечисление.ВидыИспользуемыхТерриторийСАТУРН.ПустаяСсылка"));
...
//		* ПересчитатьКоличествоСАТУРН   - Булево - Признак необходимости пересчета значения количества САТУРН.
//		* ПересчитатьКоличествоУпаковок - Булево - Признак необходимости пересчета значения количества упаковок.
//		* КоличествоСуффикс             - Строка - Суффиксы реквизитов Количество в документе для пересчета.
//                                                 Может быть указано несколько значений через запятую.
Функция ПараметрыЗаполненияТабличнойЧасти() Экспорт
	
	ПараметрыЗаполнения = Новый Структура;
	ПараметрыЗаполнения.Вставить("ПересчитатьКоличествоСАТУРН",   Истина);
...
Функция ИнициализироватьПараметрыЗаполненияТипаИзмеряемойВеличиныИКоличества() Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("ЕстьНоменклатура",                   Ложь);
	Результат.Вставить("ЕстьУпаковка",                       Ложь);
	Результат.Вставить("ЕстьПартия",                         Ложь);
	Результат.Вставить("ЕстьТипИзмеряемойВеличиныСАТУРН",    Ложь);
	Результат.Вставить("ЕстьКоличествоУпаковок",             Ложь);
	Результат.Вставить("ЕстьКоличествоВУпаковкеСАТУРН",      Ложь);
	
	Возврат Результат;
	
КонецФункции

Функция ПараметрыЗаполненияТипаИзмеряемойВеличиныИКоличества(Данные, КэшированныеЗначения = Неопределено) Экспорт
	
	ПараметрыЗаполнения = Неопределено;
	
	ЕстьКэшированныеЗначения = КэшированныеЗначения <> Неопределено;
	
	Если ЕстьКэшированныеЗначения
		И КэшированныеЗначения.Свойство("ПараметрыЗаполненияТипаИзмеряемойВеличиныИКоличестваВУпаковке") Тогда
		ПараметрыЗаполнения = КэшированныеЗначения.ПараметрыЗаполненияТипаИзмеряемойВеличиныИКоличестваВУпаковке;
	КонецЕсли;
	
	Если ПараметрыЗаполнения = Неопределено Тогда
		ПараметрыЗаполнения = ИнициализироватьПараметрыЗаполненияТипаИзмеряемойВеличиныИКоличества();
		Для Каждого Параметр Из ПараметрыЗаполнения Цикл
			ИмяПоля = Сред(Параметр.Ключ, 5);
			ПараметрыЗаполнения[Параметр.Ключ] = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Данные, ИмяПоля);
		КонецЦикла;
		Если ЕстьКэшированныеЗначения Тогда
			КэшированныеЗначения.Вставить("ПараметрыЗаполненияТипаИзмеряемойВеличиныИКоличестваВУпаковке", ПараметрыЗаполнения);
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьТипИзмеряемойВеличиныИКоличествоВУпаковке(
		ДанныеДляОбработки, ДанныеУпаковки = Неопределено,
		ПараметрыЗаполнения = Неопределено, ПерезаполнятьТипИзмеряемойВеличины = Истина) Экспорт
	...
	Если ПараметрыЗаполнения = Неопределено Тогда
		ПараметрыЗаполнения = ПараметрыЗаполненияТипаИзмеряемойВеличиныИКоличества(ДанныеДляОбработки[0]);
	КонецЕсли;
	
	Если ПараметрыЗаполнения.ЕстьНоменклатура Тогда
		
		Если ПараметрыЗаполнения.ЕстьТипИзмеряемойВеличиныСАТУРН Тогда
			...
			Если ДанныеУпаковки = Неопределено Тогда
				ДанныеУпаковок = ИнтеграцияИСВызовСервера.КоэффициентВесОбъемУпаковок(ДанныеДляОбработки);
			КонецЕсли;
			
			Если ПараметрыЗаполнения.ЕстьПартия Тогда
				ДанныеПартий = ИнтеграцияСАТУРНВызовСервера.ДанныеПартий(ДанныеДляОбработки);
			КонецЕсли;
			
			Для Каждого СтрокаТаблицы Из ДанныеДляОбработки Цикл
				
				Если ДанныеУпаковки = Неопределено Тогда
					Если ПараметрыЗаполнения.ЕстьУпаковка Тогда
						Упаковка = СтрокаТаблицы.Упаковка;
					Иначе
						Упаковка = Неопределено;
					КонецЕсли;
					ДанныеТекущейУпаковки = ИнтеграцияИСКлиентСервер.ПолучитьДанныеУпаковки(
						ДанныеУпаковок, СтрокаТаблицы.Номенклатура, Упаковка);
				Иначе
					ДанныеТекущейУпаковки = ДанныеУпаковки;
				КонецЕсли;
					
				Если ПараметрыЗаполнения.ЕстьПартия Тогда
					ДанныеПартии = ДанныеПартий[СтрокаТаблицы.Партия]
				Иначе
					ДанныеПартии = Неопределено;
				КонецЕсли;
				
				Если ПерезаполнятьТипИзмеряемойВеличины
					Или Не ЗначениеЗаполнено(СтрокаТаблицы.ТипИзмеряемойВеличиныСАТУРН) Тогда
					
					Если ДанныеПартии = Неопределено Тогда
						Если ДанныеТекущейУпаковки.ТипИзмеряемойВеличиныОбъем Тогда
							СтрокаТаблицы.ТипИзмеряемойВеличиныСАТУРН = ТипИзмеряемойВеличиныОбъем;
						ИначеЕсли ДанныеТекущейУпаковки.ТипИзмеряемойВеличиныВес Тогда
							СтрокаТаблицы.ТипИзмеряемойВеличиныСАТУРН = ТипИзмеряемойВеличиныВес;
						ИначеЕсли ДанныеТекущейУпаковки.Объем > 0 Тогда
							СтрокаТаблицы.ТипИзмеряемойВеличиныСАТУРН = ТипИзмеряемойВеличиныОбъем;
						КонецЕсли;
					Иначе
						СтрокаТаблицы.ТипИзмеряемойВеличиныСАТУРН = ДанныеПартии.ТипИзмеряемойВеличиныСАТУРН;
					КонецЕсли;
				...
				Если ПараметрыЗаполнения.ЕстьКоличествоВУпаковкеСАТУРН Тогда
					Если ДанныеПартии <> Неопределено
						И ДанныеПартии.КоличествоВУпаковкеСАТУРН > 0 Тогда
						СтрокаТаблицы.КоличествоВУпаковкеСАТУРН = ДанныеПартии.КоличествоВУпаковкеСАТУРН;
					Иначе
						СтрокаТаблицы.КоличествоВУпаковкеСАТУРН = 1;
					КонецЕсли;
				КонецЕсли;
...
Функция РезультатВыбораПартииСАТУРН(ВыбранноеЗначение) Экспорт
	...
	РезультатВыбора.Вставить("КоличествоВУпаковкеСатурн",     ВыбранноеЗначение.КоличествоВУпаковкеСатурн);
