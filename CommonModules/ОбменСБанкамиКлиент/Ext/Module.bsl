...
// Открывает настройку обмена с банком.
//
// Параметры:
//  Организация - СправочникСсылка.Организации - ссылка на организацию.
//  Банк - СправочникСсылка.КлассификаторБанков - ссылка на банк.
//  Страница - Строка - Наименование страницы на форме элемента справочника Настройки обмена с банком
//
Процедура ОткрытьНастройкуОбменаНаСтранице(Организация, Банк, Страница = "") Экспорт
	
	ЕстьПравоПросмотраНастройкиЭДО = Ложь; 
	ФОВключенаСейчас = Ложь; 
	ЕстьПравоНастройкиЭДО = Ложь;
	...
	Если ЗначениеЗаполнено(ТекущаяНастройка) Тогда
		...
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("Ключ", ТекущаяНастройка);
		ПараметрыФормы.Вставить("ТекущаяСтраницаФормы", Страница);
		ОткрытьФорму("Справочник.НастройкиОбменСБанками.ФормаОбъекта", ПараметрыФормы);
	КонецЕсли;
...
Процедура СинхронизироватьСБанком(Организация = Неопределено, Банк = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Организация) И ЗначениеЗаполнено(Банк) Тогда
		...
		Если ЗначениеЗаполнено(НастройкаОбмена) Тогда
			...
			Реквизиты = Новый Структура("ПрограммаБанка,Банк");
			ОбменСБанкамиСлужебныйВызовСервера.ПолучитьЗначенияРеквизитовНастройкиОбмена(НастройкаОбмена, Реквизиты);
			Если Не ОбменСБанкамиСлужебныйКлиент.ДоступнаРаботаНаМобильномКлиенте(Реквизиты.ПрограммаБанка) Тогда
				
				Банк = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Реквизиты, "Банк");
				ТекстСообщения = ОбменСБанкамиСлужебныйКлиентСервер.ТекстПредупрежденияМобильныйКлиентНедоступнаРабота(
					Реквизиты.ПрограммаБанка, Банк);
				ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
				Возврат;
...
Процедура ПриНажатииНаГиперссылку(ДокументСсылка, СтандартнаяОбработка) Экспорт
	...
	Если ЗначениеЗаполнено(РеквизитыЭД.ПрограммаБанка) Тогда
		Если НЕ ОбменСБанкамиСлужебныйКлиент.ДоступнаРаботаНаМобильномКлиенте(РеквизитыЭД.ПрограммаБанка) Тогда
			
			Банк = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(РеквизитыЭД, "Банк");
			ТекстСообщения = ОбменСБанкамиСлужебныйКлиентСервер.ТекстПредупрежденияМобильныйКлиентНедоступнаРабота(
				РеквизитыЭД.ПрограммаБанка, Банк);
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
			Возврат;
...
// Открывает и заполняет форму для отправки письма в банк 
// с шаблоном запроса о подключении автоматической выписки по 1С:ДиректБанк.
//
// Параметры:
//  Организация - СправочникСсылка.Организации - ссылка на организацию.
//  Банк - СправочникСсылка.КлассификаторБанков - ссылка на банк.
//
Процедура ОтправитьПисьмоПоАвтоматическойВыписке(Организация, Банк) Экспорт
	
	ТемаПисьма = НСтр("ru = 'Запрос о подключении автоматической выписки по 1С:ДиректБанк'");
	ПараметрыПисьма = ОбменСБанкамиСлужебныйВызовСервера.ПараметрыПисьмаПоАвтоматическойВыписке(Организация, Банк);
	ТекстПисьма = ПараметрыПисьма.ТекстПисьма;
	ОписаниеНастройки = ПараметрыПисьма.ОписаниеНастройки;
	Если ЗначениеЗаполнено(ОписаниеНастройки.Ссылка) Тогда
		РеквизитыНовогоПисьма = Новый Структура;
		РеквизитыНовогоПисьма.Вставить("Тема", ТемаПисьма);
		РеквизитыНовогоПисьма.Вставить("Текст", ТекстПисьма);
		РеквизитыНовогоПисьма.Вставить("Организация", Организация);
		РеквизитыНовогоПисьма.Вставить("Банк", Банк);
		
		ПараметрыОткрытияФормы = Новый Структура;
		ПараметрыОткрытияФормы.Вставить("РеквизитыНовогоПисьма", РеквизитыНовогоПисьма);
		ПараметрыОткрытияФормы.Вставить("БезПодписи", Истина);
		ОткрытьФорму("Документ.ПисьмоОбменСБанками.Форма.ПисьмоВБанк", ПараметрыОткрытияФормы);
	Иначе
		Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСПочтовымиСообщениями") Тогда
			...
			ПараметрыОтправкиПисьма.Вставить("Текст", ТекстПисьма);
...
Процедура ОбработатьПолучениеВыписки(Оповещение, ПараметрыОбмена, ДатаНачала, ДатаОкончания)
	...
	Если НЕ ОбменСБанкамиСлужебныйКлиент.ДоступнаРаботаНаМобильномКлиенте(РеквизитыНастройкиОбмена.ПрограммаБанка) Тогда
		
		Банк = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(РеквизитыНастройкиОбмена, "Банк");
		ТекстСообщения = ОбменСБанкамиСлужебныйКлиентСервер.ТекстПредупрежденияМобильныйКлиентНедоступнаРабота(
			РеквизитыНастройкиОбмена.ПрограммаБанка, Банк);
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
		Возврат;
	...
	ИначеЕсли РеквизитыНастройкиОбмена.ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.SberAPI") Тогда
		ОбменСБанкамиSberAPIСлужебныйКлиент.ПолучитьВыписку(Оповещение, ПараметрыЗапроса);
	Иначе
