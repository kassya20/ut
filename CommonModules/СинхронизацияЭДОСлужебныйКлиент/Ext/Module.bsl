...
// Параметры:
//  ПроцессВыполнения - см. НовыйПроцессВыполненияОперацииЭДО
Процедура ЗавершитьВыполнениеОперацииЭДО(Знач ПроцессВыполнения)
	...
	ПаролиСертификатов = ПроцессВыполнения.РезультатВыполнения.ОперацияЭДО.СлужебныеПараметры.ПаролиСертификатов;
	Результат.ОперацияЭДО.СлужебныеПараметры.ПаролиСертификатов = ПаролиСертификатов;
	Если Не ЗначениеЗаполнено(ПроцессВыполнения.ОперацияЭДО.Результат.УчетнаяЗапись)
		Или Не ЗначениеЗаполнено(ПаролиСертификатов) Тогда
		ВыполнитьОбработкуОповещения(ПроцессВыполнения.ОбработкаЗавершения, Результат);
		Возврат;
	КонецЕсли;
	
	Контекст = Новый Структура;
	Контекст.Вставить("ОбработкаЗавершения", ПроцессВыполнения.ОбработкаЗавершения);
	Контекст.Вставить("РезультатПодключенияЭДО", Результат);
	
	Оповещение = Новый ОписаниеОповещения(
		"ЗавершитьВыполнениеОперацииПодключенияЭДОПослеВключенияАвтоматическогоПолученияЭДО", ЭтотОбъект, Контекст);
	...
	ПараметрыАвторизации.ИдентификаторыЭДО = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(
		ПроцессВыполнения.ОперацияЭДО.Результат.УчетнаяЗапись);
...
// Параметры:
//  РезультатВключения - см. АвтоматическоеПолучениеЭДОКлиент.ВключитьПоУчетнымЗаписямЭДОЗавершение
//  Контекст - Структура:
//  * ОбработкаЗавершения - см. СинхронизацияЭДОКлиент.НачатьВыполнениеОперацииЭДО.ОбработкаЗавершения
//  * РезультатПодключенияЭДО - Структура:
//    ** Выполнено - Булево
//    ** ОперацияЭДО - см. СинхронизацияЭДОКлиентСервер.НоваяОперацияПодключенияЭДО
Процедура ЗавершитьВыполнениеОперацииПодключенияЭДОПослеВключенияАвтоматическогоПолученияЭДО(РезультатВключения, Контекст) Экспорт
	
	ВыполнитьОбработкуОповещения(Контекст.ОбработкаЗавершения, Контекст.РезультатПодключенияЭДО);
...
Функция СоздатьУчетнуюЗапись(ОперацияЭДО)
	...
	Если ЗначениеЗаполнено(ОперацияЭДО.Параметры.Сертификат) Тогда
		ОписаниеУчетнойЗаписи.Сертификаты.Добавить(ОперацияЭДО.Параметры.Сертификат);
	КонецЕсли;
...
Процедура ВыполнитьОбновлениеСертификата(Знач РезультатТекущегоЭтапа, Знач ПроцессВыполнения) Экспорт
	...
	Если ОперацияЭДО.Действие = УчетныеЗаписиЭДОКлиентСервер.ДействиеОбновлениеСертификата()
			И ЗначениеЗаполнено(ОперацияЭДО.Параметры.Доверенность) Тогда
		Доверенность = ОперацияЭДО.Параметры.Доверенность;
	КонецЕсли;
	
	// Добавляем новый сертификат по одобренным заявкам.
	СинхронизацияЭДОВызовСервера.ДобавитьСертификатВУчетныеЗаписиЭДО(
		ПроцессВыполнения.ОдобренныеЗаявки, ОперацияЭДО.Параметры.НовыйСертификат, Доверенность);
...
Процедура НачатьОтправкуЗаявкиНаДобавлениеСертификата(Знач УчетнаяЗапись, Знач НовыйСертификат, Знач ОбработкаПродолжения)
	...
	Если УчетнаяЗапись.СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезОператораЭДОТакском") Тогда
		...
		Если ИнтеграцияБИПБЭДКлиент.ИспользуетсяИнтернетПоддержкаПользователей() Тогда
			Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.Подключение1СТакском") Тогда
				...
				МодульПодключение1СТакскомКлиент.ЗарегистрироватьСертификатАбонента(
					НовыйСертификат, УчетнаяЗапись.ИдентификаторЭДО, УчетнаяЗапись.Организация, ОбработкаЗавершения);
				Возврат;
			Иначе
				Подключение1СТакскомКлиентБЭД.ЗарегистрироватьСертификатАбонента(
					НовыйСертификат, УчетнаяЗапись.ИдентификаторЭДО, УчетнаяЗапись.Организация, ОбработкаЗавершения);
				Возврат;
			КонецЕсли;
		Иначе
	...
	ИначеЕсли УчетнаяЗапись.СпособОбменаЭД = ПредопределенноеЗначение("Перечисление.СпособыОбменаЭД.ЧерезСервис1СЭДО") Тогда
		...
		ПараметрыРегистрации = СервисЭДОКлиент.НовыеПараметрыРегистрацииВСервисе1СЭДО();
			...
			ПараметрыРегистрации.ВидОперацииЭДО = УчетныеЗаписиЭДОКлиентСервер.ДействиеОбновлениеСертификата();
			
			Действие = ОбработкаПродолжения.ДополнительныеПараметры.ОперацияЭДО.Действие;
			Если Действие = УчетныеЗаписиЭДОКлиентСервер.ДействиеОбновлениеСертификата()
					И ЗначениеЗаполнено(ОбработкаПродолжения.ДополнительныеПараметры.ОперацияЭДО.Параметры.Доверенность) Тогда
				Доверенность = ОбработкаПродолжения.ДополнительныеПараметры.ОперацияЭДО.Параметры.Доверенность;
				ПараметрыРегистрации.Доверенности.Вставить(НовыйСертификат, Доверенность);
			КонецЕсли;
...
Процедура ОбработатьОшибкуНекорректныйТокенДоступаКСервисуЭДО(КонтекстДиагностики, ИдентификаторУчетнойЗаписи) Экспорт
	
	Оповещение = Новый ОписаниеОповещения("ОбработатьОшибкуНекорректныйТокенДоступаКСервисуЭДОЗавершение", ЭтотОбъект);
	
	ПараметрыАвторизации = УчетныеЗаписиЭДОКлиент.НовыеПараметрыАвторизацииВСервисеЭДО();
	ПараметрыАвторизации.ИдентификаторыЭДО = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ИдентификаторУчетнойЗаписи);
...
Процедура ОбработатьОшибкуНекорректныйТокенДоступаКСервисуЭДОЗавершение(Результат, Контекст = Неопределено) Экспорт
