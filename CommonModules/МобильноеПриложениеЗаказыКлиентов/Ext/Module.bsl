...
Функция СтруктураСтатусаЗаказаКлиента() Экспорт
	...
	Структура.Вставить("ПечатнаяФормаДокумента", 
		Новый ХранилищеЗначения(Новый ТабличныйДокумент, Новый СжатиеДанных(9))); //@NON-NLS-1
...
Процедура ЗаписатьЗаданиеТорговомуПредставителю(Данные, УзелОбмена, Сообщение, НастройкиЗадания)
	...
	Для каждого Свойство Из Данные.Свойства() Цикл
		Если ТипЗнч(Данные[СокрЛП(Свойство)]) = Тип("ОбъектXDTO") Тогда
			...
			И НЕ СокрЛП(Свойство) = "ДолгиПоЗаказам" И НЕ СокрЛП(Свойство) = "РасшифровкаДебиторскойЗадолженности" Тогда //@NON-NLS-1 @NON-NLS-2 @NON-NLS-3 @NON-NLS-4
				Значение = СериализаторXDTO.ПрочитатьXDTO(Данные[СокрЛП(Свойство)]);
			КонецЕсли;
		Иначе
			Значение = Данные[СокрЛП(Свойство)];
		...
		И НЕ СокрЛП(Свойство) = "ДолгиПоЗаказам" И НЕ СокрЛП(Свойство) = "РасшифровкаДебиторскойЗадолженности" Тогда //@NON-NLS-1 @NON-NLS-2 @NON-NLS-4 @NON-NLS-5
	...
	Для каждого Свойство Из Данные.Свойства() Цикл
		...
		Если ТипЗнч(Данные[СокрЛП(Свойство)]) = Тип("ОбъектXDTO") Тогда
			Если НЕ ЭтоТаблицаОбъекта(Данные[СокрЛП(Свойство)])
				...
				И НЕ СокрЛП(Свойство) = "ДополнительныеРеквизиты" Тогда //@NON-NLS-1 @NON-NLS-2
				Значение = СериализаторXDTO.ПрочитатьXDTO(Данные[СокрЛП(Свойство)]);
			Иначе
				Значение = Данные[СокрЛП(Свойство)];
	...
	Для каждого Свойство Из Данные.Свойства() Цикл
		...
		Если ТипЗнч(Данные[СокрЛП(Свойство)]) = Тип("ОбъектXDTO") Тогда
			Если НЕ ЭтоТаблицаОбъекта(Данные[СокрЛП(Свойство)])
				...
				И НЕ СокрЛП(Свойство) = "КонтактнаяИнформация" Тогда //@NON-NLS-1 @NON-NLS-2
				Значение = СериализаторXDTO.ПрочитатьXDTO(Данные[СокрЛП(Свойство)]);
			Иначе
				Значение = Данные[СокрЛП(Свойство)];
				ЭтоТабличнаяЧасть = Истина;
...
Процедура ПрочитатьДанныеПакетаОбмена(УзелОбмена, ПараметрыЧтения, ЧтениеXML, ОтветноеСообщение)
	...
	Пока ЧтениеXML.ЛокальноеИмя = "DocumentObject.ЗаказКлиента"
		...
		ИЛИ СериализаторXDTO.ВозможностьЧтенияXML(ЧтениеXML) Цикл //@NON-NLS-1 @NON-NLS-2 @NON-NLS-3 @NON-NLS-4 @NON-NLS-5 @NON-NLS-6 @NON-NLS-7 @NON-NLS-8 @NON-NLS-9 @NON-NLS-10 @NON-NLS-11 @NON-NLS-12
