...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "ОбновлениеИнформационнойБазыБИД.ПерейтиНаВерсию_3_0_4_1";
	Обработчик.РежимВыполнения = "Оперативно";
	Обработчик.Комментарий = НСтр("ru = 'Обновление правил интеграции с 1С:Документооборотом'");
...
// Обработчик обновления БИД на версию 3.0.4.1.
//
Процедура ПерейтиНаВерсию_3_0_4_1() Экспорт
	
	Менеджер = Константы.ИспользоватьСогласованиеЧерез1СДокументооборот.СоздатьМенеджерЗначения();
	Менеджер.Значение = Константы.УдалитьИспользоватьСогласованиеЧерез1СДокументооборот.Получить();
	...
	ВыборкаПравила = ЗапросВсеПравила.Выполнить().Выбрать();
	Пока ВыборкаПравила.Следующий() Цикл
		...
		Если СтрНачинаетсяС(ПравилоОбъект.ТипОбъектаИС, "Справочник.") Тогда
			Для Каждого Строка Из ПравилоОбъект.ПравилаЗаполненияРеквизитовИС Цикл
				Строка.РежимИзмененияДанныхПроведенногоДокумента = ПустойРежим;
			КонецЦикла;
		КонецЕсли;
		
		Если ТипЗнч(ВыборкаПравила.Ссылка) = Тип("СправочникСсылка.ПравилаИнтеграцииС1СДокументооборотом") Тогда
			СтруктураПоиска = Новый Структура("Обновлять", Истина);
			ПравилоОбъект.ВыгружатьИзменения = Ложь;
			Если ЗначениеЗаполнено(ПравилоОбъект.ТипОбъектаИС) Тогда
				ОбъектПотребителя = Метаданные.НайтиПоПолномуИмени(ПравилоОбъект.ТипОбъектаИС);
				Если ОбъектПотребителя <> Неопределено
						И Метаданные.ПланыОбмена.ИнтеграцияС1СДокументооборотомПереопределяемый.Состав.Содержит(
							ОбъектПотребителя) Тогда
					ОбновляемыеРеквизиты = ПравилоОбъект.ПравилаЗаполненияРеквизитовДО.НайтиСтроки(СтруктураПоиска);
					ОбновляемыеПечатныеФормы = ПравилоОбъект.ПрисоединяемыеПечатныеФормы.НайтиСтроки(СтруктураПоиска);
					ПравилоОбъект.ВыгружатьИзменения = (ОбновляемыеРеквизиты.Количество() <> 0
						Или ОбновляемыеПечатныеФормы.Количество() <> 0);
				КонецЕсли;
			КонецЕсли;
			ПравилоОбъект.ЗагружатьИзменения = Ложь;
			Если ЗначениеЗаполнено(ПравилоОбъект.ТипОбъектаДО) Тогда
				ОбновляемыеРеквизиты = ПравилоОбъект.ПравилаЗаполненияРеквизитовИС.НайтиСтроки(СтруктураПоиска);
				ПравилоОбъект.ЗагружатьИзменения = (ОбновляемыеРеквизиты.Количество() <> 0);
			КонецЕсли;
		КонецЕсли;
		
		Если ПравилоОбъект.Модифицированность() Тогда
			ОбновлениеИнформационнойБазы.ЗаписатьДанные(ПравилоОбъект);
		КонецЕсли;
