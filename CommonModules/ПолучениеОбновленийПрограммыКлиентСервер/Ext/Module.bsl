...
// Выполняет проверку принадлежности кода возврата к неустранимой ошибке при установке программы в операционной системе
// Windows (код ошибки 1603).
// Код ошибки 1603 может возникать по следующим причинам:
//  - проблема с правами доступа;
//  - конфликт программного обеспечения с другим ПО;
//  - повреждены системные компоненты;
//  - проблема с .NET Framework;
//  - установка на зашифрованный/замененный диск.
//
// Параметры:
//  КодВозврата - Число - код завершения работы программы.
//
// Возвращаемое значение:
//  Булево
//
Функция ЭтоКодВозвратаНеустранимойОшибкиПриУстановке(КодВозврата) Экспорт
	
	Возврат КодВозврата = 1603;
	
КонецФункции

// Выполняет проверку принадлежности кода возврата к ошибкам нарушения прав доступа в операционной системе Windows.
// Это критическая ошибка, которая возникает, когда программа пытается получить доступ к ресурсу, к которой у нее нет
// разрешения.
//
// Параметры:
//  КодВозврата - Число - код завершения работы программы.
...
//  Булево
//
Функция ЭтоКодВозвратаОшибкиНарушенияДоступа(КодВозврата) Экспорт
	
	Возврат КодВозврата = -1073741819;
...
Функция ИдентификаторОбщихНастроек() Экспорт
	...
	// В идентификаторе ключа общих настроек была допущена орфографическая ошибка.
	// Библиотека Интернет-поддержки пользователей поддерживает возможность обновления с любой старой версии
	// библиотеки на любую новую версию библиотеки. Из-за этого существует риск потери сохраненных данных в
	// сценарии когда библиотека будет обновлена с версии, где отсутствовал обработчик обновления на версию
	// библиотеки где обработчик обновления уже будет удален.
	
	Возврат "ИнтеррнетПоддержка";	// АПК:1036 Для обратной совместимости
