...
//                  # ИмяФайла - Строка - полное имя файла вложения. Важно: При передаче файла по имени,
//                    он должен обязательно располагатся в каталоге временных файлов, то есть, путь к
//                    файлу должен быть получен методом ПолучитьИмяВременногоФайла.
...
//    ** ОбработчикЗавершения - ОписаниеОповещения, Неопределено - описание оповещения о
//                               закрытии формы нового обращения;
//    ** ДополнительныеДанные - Структура - произвольная структура данных, которую 
...
Функция ПодготовитьСообщение(
	...
	// Дополнительная проверка вложения если ВидДанных = ИмяФайла
	РезультатПроверки = ДополнительнаяПроверкаВложений(Вложения);
	ЗаполнитьЗначенияСвойств(Результат, РезультатПроверки);
	Если ЗначениеЗаполнено(Результат.КодОшибки) Тогда
		Возврат Результат;
	КонецЕсли;
...
// Дополнительная проверка вложений только при работе на сервере.
//  Если файл передается по имени, то он должен располагаться в каталоге временных файлов
//  и иметь допустимое расширение.
// 
// Параметры:
//  Вложения - см. ПодготовитьСообщение.Вложения.
//  
Функция ДополнительнаяПроверкаВложений(Вложения)
	...
	НедопустимоеРасширение = Новый Массив;
	ОшибкаВременныйКаталог = Новый Массив;
	...
	Для Каждого ОписаниеФайла Из Вложения Цикл
		...
		РасширениеИмениФайла = ВРег(
			ОбщегоНазначенияКлиентСервер.ПолучитьРасширениеИмениФайла(
				ОписаниеФайла.Данные));
		
		Если Не ЗначениеЗаполнено(РасширениеИмениФайла) 
			Или ДопустимыеРасширения.Найти(РасширениеИмениФайла) = Неопределено Тогда
			НедопустимоеРасширение.Добавить(ОписаниеФайла.Представление);
			Продолжить;
		КонецЕсли;
		
		Если Не ЭтоИмяВременногоФайла(ОписаниеФайла.Данные) Тогда
			ОшибкаВременныйКаталог.Добавить(ОписаниеФайла.Представление);
		КонецЕсли;
		
	КонецЦикла;
	
	Ошибки = Новый Массив;
	Если НедопустимоеРасширение.Количество() > 0 Тогда
		Ошибки.Добавить(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Следующие вложения имеют недопустимое расширение:
				|%1'"),
				СтрСоединить(НедопустимоеРасширение, Символы.ПС)));
	КонецЕсли;
	
	Если ОшибкаВременныйКаталог.Количество() > 0 Тогда
		Ошибки.Добавить(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Отправка вложений по имени файла доступна только из каталога временных файлов.
				|Следующие вложения расположены в другом каталоге и не могут быть отправлены:
				|%1'"),
				СтрСоединить(ОшибкаВременныйКаталог, Символы.ПС)));
	КонецЕсли;
...
// Проверяет, что указанный путь к файлу ссылается на каталог временных файлов.
// 
// Параметры:
//  Путь - Строка - путь к файлу или полное имя файла с каталогами.
//  
// Возвращаемое значение:
//  Булево - Истина, если файл расположен в каталоге временных файлов
// 
...
// Определяет список допустимых для отправки расширений файлов.
//
// Возвращаемое значение:
//  Массив Из Строка - расширения файлов.
//
Функция ДопустимыеРасширения()
	...
	Возврат СтрРазделить("TXT;JSON;XML;BMP;JPG;JPEG;TIF;TIFF;GIF;PNG;MP4;M4V;MPEG;MPG;MOV;WMV;AVI;MKV", ";");
