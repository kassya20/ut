...
Процедура ДополнитьКонтекстДиагностики(КонтекстДиагностики, ОбъектыДействий) Экспорт
	
	Если ЗначениеЗаполнено(КонтекстДиагностики)
		И ТипЗнч(КонтекстДиагностики) = Тип("Структура") Тогда
	
		КонтекстДиагностики.ДополнительныеСвойства.Вставить("ОбъектыДействий", ОбъектыДействий);
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗаполнитьДанныеСлужбыПоддержкиЕИС(ТелефонСлужбыПоддержки,
		АдресЭлектроннойПочтыСлужбыПоддержки, КонтекстДиагностики) Экспорт
	
	ЭлектронноеАктированиеЕИСВызовСервера.ЗаполнитьДанныеСлужбыПоддержкиЕИС(ТелефонСлужбыПоддержки,
		АдресЭлектроннойПочтыСлужбыПоддержки, КонтекстДиагностики);
