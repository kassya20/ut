...
// Подбирает изображения в документ "Запрос коммерческих предложений у поставщика" для одного из двух сценариев:
// 1) Авто подстановка изображения в строку товарной позиции при выборе товарной позиции из прикладного справочника.
// 2) Открытие собственной формы выбора изображений, доступных для товарной позиции из прикладного справочника.
// С целью дальнейшей публикации этих изображений в сервисе.
// 
// Примечание:
//  1. Для первого сценария достаточно одного изображения. Для второго - все доступные для текущей товарной позиции.
//  2. Для определения, какой вариант выполняется в текущем вызове, во входящем параметре ПараметрыПодбора
//     присутствует свойство РежимПодбораОдиночный с типом Булево.
//     Истина - авто подстановка, нужно одно изображение. Ложь - выбор из доступных.
//  3. При добавлении нескольких файлов в результирующую таблицу, необходимо заполнить порядок сортировки. 
//     Более приоритетными считаются записи с меньшей величиной числового значения в порядке сортировки.
//  4. Файлы не должны быть помечены на удаление 
//  5. Файлы должны принадлежать одному владельцу
//  6. Максимальное количество изображений должно быть 100
// 
// 1) Примеры реализации подбора изображения для авто подстановки в документ ЗКП при добавлении товара в табличную часть:
// * Пример 1. В справочнике присоединенных файлов есть признак публикации файла в сервисах(ПубликуетсяВСервисах) 
//             И в справочнике номенклатуры есть реквизит ФайлКартинки
//       * Получает все файлы с признаком ПубликуетсяВСервисах 
//       * Если среди полученных с признаком ПубликуетсяВСервисах файлов есть тот, 
//         который используется в качестве файла картинки, добавляет в таблицу результата его.  
//         Иначе добавляет в таблицу результата любой из полученных с признаком ПубликуетсяВСервисах
// 
// * Пример 2. В справочнике присоединенных файлов отсутствует признак публикации файла в сервисах
//             (ПубликуетсяВСервисах). И в справочнике номенклатуры есть реквизит ФайлКартинки.
//       Файл картинки добавляет в таблицу результата в качестве подобранного файла изображения для публикации.
// 
// * Пример 3. В справочнике присоединенных файлов есть признак публикации файла в сервисах (ПубликуетсяВСервисах) 
//             И в справочнике номенклатуры отсутствует реквизит ФайлКартинки
//       Добавляет в таблицу результата любой из файлов с признаком ПубликуетсяВСервисах.
// 
// 2) Пример реализации подбора изображений для вывода в форму выбора изображений 
//    для возможности последующей подстановки в документ ЗКП: 
// Добавляет все присоединенные файлы, соответствующие параметрам подбора, с сортировкой по дате добавления 
// файла в систему.
// 
// Параметры:
//  ПараметрыПодбора - см. КоммерческиеПредложенияСлужебный.НовыеПараметрыПодбораИзображенийВЗКП
//  ТаблицаПодобранных - см. КоммерческиеПредложенияСлужебный.НовыйРезультатПодбораИзображенийВЗКП
Процедура ПодобратьИзображенияДляПубликацииВЗКП(ПараметрыПодбора, ТаблицаПодобранных) Экспорт	

	КоммерческиеПредложенияУТ.ПодобратьИзображенияДляПубликацииВЗКП(ПараметрыПодбора, ТаблицаПодобранных);
		
КонецПроцедуры

// Выполняется при добавлении изображения из файловой системы в документ ЗКППоставщика для пары 
// Номенклатура, Характеристика. 
// Необходимо записать файл в систему и заполнить параметр Изображение ссылкой на записанный файл. 
// Доступен отказ от записи файла с указанием ошибок. Например, если файл не прошел валидацию. 
// 
// Примечание:
// 	При Отказ = Истина, параметр Ошибки обязателен к заполнению 
// 
// Параметры:
//  ДанныеДляДобавленияФайла - см. КоммерческиеПредложенияСлужебный.НовыеДанныеДляДобавленияИзображенияИзФайловойСистемы
//  Изображение - ОпределяемыйТип.ПрисоединенныеФайлыИзображенийЗКППоставщика - 
//  Отказ - Булево - Признак не успешности добавления файла в систему. Истина - файл не добавлен. 
//  Ошибки - Массив Из Строка - Массив текстов ошибок. 
Процедура ПриДобавленииИзображенияИзФайловойСистемы(ДанныеДляДобавленияФайла, Изображение, Отказ, Ошибки) Экспорт

	КоммерческиеПредложенияУТ.ПриДобавленииИзображенияИзФайловойСистемы(ДанныеДляДобавленияФайла, Изображение, Отказ, Ошибки);
