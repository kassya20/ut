...
//  Запустить - см. ДобавитьЗаданияКОтражениюСостоянияЗаказов.Запустить.
//
Процедура ОтразитьСостоянияДокументов(ДокументИсточник, МенеджерВременныхТаблиц, Запустить = Истина) Экспорт
	
	ОписаниеЗамера = ОценкаПроизводительности.НачатьЗамерДлительнойОперации("СостоянияДокументов.ОтразитьСостоянияДокументов");
	ЗапуститьВТекущемСеансе = СостоянияДокументовПовтИсп.ВыполнитьОтражениеВТекущемСеансе(ДокументИсточник.Ссылка.Метаданные().Имя);
	
	Если ЗапуститьВТекущемСеансе Тогда
		
		ИмяРегистра = СостоянияДокументовПовтИсп.ИмяРегистраДляОтраженияДокумента(ДокументИсточник.Ссылка.Метаданные().Имя);
		Если ЗначениеЗаполнено(ИмяРегистра) Тогда
			
			МассивДокументов = Новый Массив;
			МассивДокументов.Добавить(ДокументИсточник.Ссылка);
			ОшибкиОтражения = Новый Соответствие;
			
			РегистрыСведений[ИмяРегистра].ОтразитьСостоянияЗаказов(МассивДокументов, ОшибкиОтражения);
			
			ТекстОшибкиОтражения = ОшибкиОтражения.Получить(ДокументИсточник.Ссылка);
			
			Если ТекстОшибкиОтражения <> Неопределено Тогда
			
				ВызватьИсключение ТекстОшибкиОтражения;	
	...
	Если ЗначениеЗаполнено(ТекстВложенногоЗапроса) Тогда // есть хотя бы один контрольный регистр.
		...
		Если ЗапуститьВТекущемСеансе Тогда // Документ-инициатор изменения состояния уже был отражен.
			ОбщегоНазначенияКлиентСервер.УдалитьЗначениеИзМассива(МассивОтражаемыхДокументов, ДокументИсточник.Ссылка);
		КонецЕсли;
		
		ДобавитьЗаданияКОтражениюСостоянияЗаказов(ДокументИсточник, МассивОтражаемыхДокументов,,, Запустить);
	КонецЕсли;
...
//  Запустить - Булево - признак запуска выполнения заданий.
//                       По умолчанию ИСТИНА.
//
Процедура ДобавитьЗаданияКОтражениюСостоянияЗаказов(
			...
			ОтражаемыеДокументы,
			Действие = "",
			РегистрыКОтражению = Неопределено,
			Запустить = Истина) Экспорт
	...
	Очередь = "ЗаданияКОтражениюСостоянияЗаказов";
	
	ТаблицаЗаданий = ?(Запустить,
						ОтложенныеЗадания.ИнициализироватьТаблицуЗаданий(Очередь),
						ПроведениеДокументов.ТаблицаОчередейЗаданий(Источник, Очередь));
	...
	Если Запустить И ЗначениеЗаполнено(ОтражаемыеДокументы) Тогда
