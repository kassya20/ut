...
Процедура АктуализироватьЗашифрованныеМаркерыВБезопасномХранилищеПоСрокуДействия(ИдентификаторУчетнойЗаписи,
	...
	Для Каждого ДанныеМаркераПоСертификату Из ДанныеМаркеровПоСертификатам Цикл
		...
		Если ДанныеМаркера.ДатаОкончанияДействияМаркера = Неопределено Тогда
			Продолжить; // при авторизации не удалось гарантированно установить срок действия маркера
		КонецЕсли;
...
//  * ДатаОкончанияДействияМаркера - Неопределено - дату окончания действия маркера не удалось определить.
//                                 - Дата
Функция НовыеДанныеЗашифрованногоМаркера()
	
	Результат = Новый Структура;
	Результат.Вставить("ЗашифрованныйМаркер", Неопределено);
	Результат.Вставить("ДатаОкончанияДействияМаркера", Неопределено);
	Возврат Результат;
