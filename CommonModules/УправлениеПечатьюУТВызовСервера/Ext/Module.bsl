...
Процедура ПриВыполненииКомандыПечати(Объекты, ОписаниеКоманды) Экспорт
	
	УправлениеПечатью.ПриВыполненииКомандыПечати(Объекты, ОписаниеКоманды);
	
КонецПроцедуры

Функция ПодготовитьКомандыПечати(ОбъектыПечати) Экспорт
	
	МетаданныеОбъектаПечати = ОбъектыПечати[0].Метаданные();
	
	КомплектПечатныхФорм = РегистрыСведений.НастройкиПечатиОбъектов.КомплектПечатныхФорм(
		МетаданныеОбъектаПечати.ПолноеИмя(),
		ОбъектыПечати, Неопределено);
	
	СтрокаИзТаблицыКоманд = УправлениеПечатью.СоздатьКоллекциюКомандПечати().Добавить();
	ШаблонКомандыПечати = ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(СтрокаИзТаблицыКоманд);
	
	Результат = Новый Массив();
	
	КомандыПечатиОбъекта = УправлениеПечатью.КомандыПечатиОбъекта(МетаданныеОбъектаПечати);
	...
	Для Каждого КомандаПечатиОбъекта Из КомандыПечатиОбъекта Цикл
		...
		Если НайденныеКоманды.Количество() И НайденныеКоманды[0].Печатать Тогда
			КомандаВКомплект = ОбщегоназначенияКлиентСервер.СкопироватьРекурсивно(ШаблонКомандыПечати);
