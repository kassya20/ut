...
// 
// Возвращаемое значение:
//  Массив из НовыйРезультатОбмена
//
...
//          2 - включено логирование при обмене с ЕИС
...
// Формирует структуру адрес сервиса ГИС НР
//
// Возвращаемое значение:
...
// Отключает регламентное задание для повторной отправки событий информационного обмена с ЕИС
//
...
// Проверяет необходимость запуска регламентного задания для повторной отправки событий информационного обмена с ЕИС
//
Процедура ЗапуститьРегламентноеЗадание() Экспорт
	...
	|		СтатусОтправкиДанных = ЗНАЧЕНИЕ(Перечисление.СтатусыОтправкиДанныхВГИСНР.ОшибкаОтправки)) КАК
	|		РезультатыОбменаДаннымиСГИСНР
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РезультатыОбменаДаннымиСГИСНР.СрезПоследних(, СтатусОтправкиДанных В
	|		(ЗНАЧЕНИЕ(Перечисление.СтатусыОтправкиДанныхВГИСНР.Отправлено),
	|			ЗНАЧЕНИЕ(Перечисление.СтатусыОтправкиДанныхВГИСНР.Просрочено))) КАК РезультатыОбменаДаннымиСГИСНРСУспешнойОтправкой
	|		ПО РезультатыОбменаДаннымиСГИСНР.ИдентификаторСообщения = РезультатыОбменаДаннымиСГИСНРСУспешнойОтправкой.ИдентификаторСообщения
	|ГДЕ
	|	РезультатыОбменаДаннымиСГИСНРСУспешнойОтправкой.ИдентификаторСообщения ЕСТЬ NULL
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
...
Процедура ДополнитьСтруктуруДаннымиДокументаСЧФ(СтруктураДанных, СодержимоеПакета, Кодировка, Результат)
	...
	МассивДанных.Добавить("Документ.СвСчФакт.НомерДок");
	МассивДанных.Добавить("Документ.СвСчФакт.ДатаДок");
	...
	МассивДанных.Добавить("ВерсФорм");
	...
	Иначе
		Если СтруктураЗначений.ВерсФорм = "5.03" Тогда
			СтруктураДанных.Вставить("НомерДокумента", СтруктураЗначений.ДокументСвСчФактНомерДок);
			ДатаДокумента =
				ПреобразоватьДатуИзДокументаЭДОВФорматГИСНР(СтруктураЗначений.ДокументСвСчФактДатаДок, Результат);
		Иначе
...
// 
// Возвращаемое значение:
//   - Структура - Структура с ключами именованными как пути к элементам без разделителей ("."),
...
Функция ЗначенияЭлементовИзДвоичныхДанныхФайлаXML(ДвоичныеДанныеИлиBase64Строка, Поля, Кодировка)
	...
	Для Каждого ЭлементКоллекцииПоля Из КоллекцияПолей Цикл
		ПолеДляПоиска = СтрЗаменить(ЭлементКоллекцииПоля, " ", "");
...
// Включает регламентное задание для повторонй отправки событий информационного обмена с ЕИС
//
