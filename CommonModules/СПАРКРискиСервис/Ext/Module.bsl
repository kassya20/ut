...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("ЗагрузитьИндексыКонтрагентовВФоне", Истина);
...
Процедура ЗагрузитьИндексыКонтрагентовВФоне(Параметры, АдресРезультата) Экспорт
	...
	ДлительныеОперации.СообщитьПрогресс(
		100,
		НСтр("ru='СПАРКРискиСервис.ЗагрузитьИндексыКонтрагентовВФоне завершено'"),
...
Функция СобытияМониторингаСПАРККонтрагента(
	...
	Если Параметры = Неопределено Тогда
		ПараметрыЗапроса = ПараметрыЗапросаСобытияМониторингаПоУмолчанию();
	Иначе
		ПараметрыЗапроса = ПараметрыЗапросаСобытияМониторингаПоУмолчанию();
		ЗаполнитьЗначенияСвойств(ПараметрыЗапроса, Параметры);
	КонецЕсли;
	Результат = НовыйРезультатВызоваСервиса();
	Результат.Вставить("ЗначенияСобытийМониторинга", Неопределено);
	...
	ЗначенияСобытийМониторинга = ОписаниеСобытийМониторинга(ОписаниеКонтрагента);
	Результат.Вставить("ЗначенияСобытийМониторинга", ЗначенияСобытийМониторинга);
	
	ПолучитьДанныеСобытийМониторингаИзСервиса(
		Результат,
		ДанныеАутентификации,
		ОписаниеКонтрагента,
		ПараметрыЗапроса);
	
	Результат.ЗначенияСобытийМониторинга.ПодлежитПроверке = Истина;
...
Процедура ПередВызовомОперацииСервиса(
	...
	ДанныеАутентификации = Неопределено,
	ЗаполнитьДанныеАутентификации = Истина) Экспорт
	...
	Если ЗаполнитьДанныеАутентификации
		И Результат.ВидОшибки.Пустая() Тогда
...
Процедура ПослеВызоваОперацииСервиса(Результат) Экспорт
...
Процедура ВызватьОперациюИндексыСПАРКЮридическогоЛица(
	...
	URLОперации = URLОперацииСервиса("rest/organizationInfo");
...
Процедура ВызватьОперациюИндексыСПАРКИндивидуальногоПредпринимателя(
	...
	URLОперации = URLОперацииСервиса("rest/enterpreneur/info");
	ЗаполнитьТикетАутентификации(ДанныеАутентификации, URLОперации, Результат);
	Если Не Результат.ВидОшибки.Пустая() Тогда
		Возврат;
	КонецЕсли;
...
Процедура ВызватьОперациюНачатьПодготовкуСправокЮридическихЛиц(
	...
	URLОперации = URLОперацииСервиса("rest/report/pdf/request");
	ЗаполнитьТикетАутентификации(ДанныеАутентификации, URLОперации, Результат);
	Если Не Результат.ВидОшибки.Пустая() Тогда
		Возврат;
	КонецЕсли;
...
Процедура ВызватьОперациюСостояниеПодготовкиСправокЮридическихЛиц(
	...
	URLОперации = URLОперацииСервиса("rest/report/pdf/check");
	ЗаполнитьТикетАутентификации(ДанныеАутентификации, URLОперации, Результат);
	Если Не Результат.ВидОшибки.Пустая() Тогда
		Возврат;
	КонецЕсли;
...
Процедура ВызватьОперациюЗагрузитьФайлСправкиЮридическихЛиц(
	...
	URLОперации = URLОперацииСервиса("rest/report/pdf/download");
	ЗаполнитьТикетАутентификации(ДанныеАутентификации, URLОперации, Результат);
	Если Не Результат.ВидОшибки.Пустая() Тогда
		Возврат;
	КонецЕсли;
	...
	ПараметрыЗапроса = НовыйПараметрыЗапросаКСервису();
	ПараметрыЗапроса.ДанныеДляОбработки = ТелоЗапроса;
	ПараметрыЗапроса.ФорматОтвета = 18; // Возвратить HTTPОтвет
	ВызватьОперациюСервиса(
		URLОперации,
		ПараметрыЗапроса,
		Результат,
...
Процедура ВызватьОперациюНачатьПодготовкуСправокИндивидуальныхПредпринимателей(
	...
	URLОперации = URLОперацииСервиса("rest/enterpreneur/report/pdf/request/create");
	ЗаполнитьТикетАутентификации(ДанныеАутентификации, URLОперации, Результат);
	Если Не Результат.ВидОшибки.Пустая() Тогда
		Возврат;
	КонецЕсли;
...
Процедура ВызватьОперациюСостояниеПодготовкиСправокИндивидуальныхПредпринимателей(
	...
	URLОперации = URLОперацииСервиса("rest/enterpreneur/report/pdf/request/check");
	ЗаполнитьТикетАутентификации(ДанныеАутентификации, URLОперации, Результат);
	Если Не Результат.ВидОшибки.Пустая() Тогда
		Возврат;
	КонецЕсли;
...
Процедура ВызватьОперациюЗагрузитьФайлСправкиИндивидуальныхПредпринимателей(
	...
	URLОперации = URLОперацииСервиса("rest/enterpreneur/report/pdf/download");
	ЗаполнитьТикетАутентификации(ДанныеАутентификации, URLОперации, Результат);
	Если Не Результат.ВидОшибки.Пустая() Тогда
		Возврат;
	КонецЕсли;
...
Процедура ВызватьОперациюВключитьМониторингСобытийЮридическихЛиц(
	...
	URLОперации = URLОперацииСервиса("rest/monitoring/organizations");
	ЗаполнитьТикетАутентификации(ДанныеАутентификации, URLОперации, Результат);
	Если Не Результат.ВидОшибки.Пустая() Тогда
		Возврат;
	КонецЕсли;
...
Процедура ВызватьОперациюРезультатВключенияМониторингаСобытийЮридическихЛиц(
	...
	URLОперации = URLОперацииСервиса("rest/monitoring/queryStatus");
	ЗаполнитьТикетАутентификации(ДанныеАутентификации, URLОперации, Результат);
	Если Не Результат.ВидОшибки.Пустая() Тогда
		Возврат;
	КонецЕсли;
...
Процедура ВызватьОперациюВключитьМониторингСобытийИндивидуальныхПредпринимателей(
	...
	URLОперации = URLОперацииСервиса("rest/monitoring/enterpreneur/request/create");
	ЗаполнитьТикетАутентификации(ДанныеАутентификации, URLОперации, Результат);
	Если Не Результат.ВидОшибки.Пустая() Тогда
		Возврат;
	КонецЕсли;
...
Процедура ВызватьОперациюРезультатВключенияМониторингаСобытийИндивидуальныхПредпринимателей(
	...
	URLОперации = URLОперацииСервиса("rest/monitoring/enterpreneur/request/check-status");
	ЗаполнитьТикетАутентификации(ДанныеАутентификации, URLОперации, Результат);
	Если Не Результат.ВидОшибки.Пустая() Тогда
		Возврат;
	КонецЕсли;
...
Процедура ВызватьОперациюСобытияМониторингаЮридическихЛиц(
	...
	URLОперации = URLОперацииСервиса("rest/monitoring/events");
	ЗаполнитьТикетАутентификации(ДанныеАутентификации, URLОперации, Результат);
	Если Не Результат.ВидОшибки.Пустая() Тогда
		Возврат;
	КонецЕсли;
...
Процедура ВызватьОперациюСобытияМониторингаИндивидуальныхПредпринимателей(
	...
	URLОперации = URLОперацииСервиса("rest/enterpreneur/events/search");
	ЗаполнитьТикетАутентификации(ДанныеАутентификации, URLОперации, Результат);
	Если Не Результат.ВидОшибки.Пустая() Тогда
		Возврат;
	КонецЕсли;
...
Функция ВызватьОперациюТипыСобытийСообщенийМониторинга(Результат, ТелоОтвета)
	...
	Если ТелоОтвета = Неопределено Тогда
		URLОперации = URLОперацииСервиса("rest/monitoring/eventTypeList");
...
Процедура ВызватьОперациюПолучитьURLВебВерсииСПАРК(
	...
	URLОперации = URLОперацииСервиса("rest/web-cards/get-card-url");
	ЗаполнитьТикетАутентификации(ДанныеАутентификации, URLОперации, Результат);
	Если Не Результат.ВидОшибки.Пустая() Тогда
		Возврат;
	КонецЕсли;
...
//
Процедура ЗаполнитьТикетАутентификации(ДанныеАутентификации, URLОперации, Результат) Экспорт
...
Процедура ПроверитьДоступностьСервиса(РезультатОперации)
	...
	URLОперацииPing = URLОперацииСервиса("rest/ping");
	...
	ИначеЕсли РезультатПроверки.ИмяОшибки = "LockError" Тогда
		...
		РезультатОперации.ВидОшибки = Перечисления.ВидыОшибокСПАРКРиски.ЗапрещенДоступКИнтернетСервисам;
		РезультатОперации.СообщениеОбОшибке =
...
Процедура ВызватьОперациюСервиса(
	URLОперации,
	ПараметрыЗапроса,
	Результат,
	ТелоОтвета) Экспорт
	...
	Если ПараметрыЗапроса = Неопределено Тогда
		ПараметрыПолученияСодержимого = НовыйПараметрыЗапросаКСервису();
	Иначе
		ПараметрыПолученияСодержимого = ПараметрыЗапроса;
	КонецЕсли;
...
Функция НовыйПараметрыЗапросаКСервису() Экспорт
	...
	Результат.Вставить("ДанныеДляОбработки"      , Неопределено);
...
Процедура ПриОшибкеВызоваОперацииСервиса(РезультатЗагрузкиСодержимого, URLОперации, Результат)
	...
	ИначеЕсли РезультатЗагрузкиСодержимого.КодСостояния = 422 Тогда
		
		Если СтрНайти(Результат.СообщениеОбОшибке, "is not found") Тогда
			Результат.ВидОшибки = Перечисления.ВидыОшибокСПАРКРиски.НеизвестныйИНН;
		Иначе
			Результат.ВидОшибки = Перечисления.ВидыОшибокСПАРКРиски.НекорректныйЗапрос;
		КонецЕсли;
	...
	ИначеЕсли РезультатЗагрузкиСодержимого.КодСостояния < 0 Тогда
		...
		Результат.ВидОшибки         = Перечисления.ВидыОшибокСПАРКРиски.ЗапрещенДоступКИнтернетСервисам;
...
Процедура ПриОшибкеОбработкиОтветаСервиса(ИнформацияОбОшибке, URLОперации, ТелоОтвета, Результат) Экспорт
...
Функция ОписаниеСобытийМониторинга(ОписаниеКонтрагента)
	...
	Результат.Вставить("ИНН", ОписаниеКонтрагента.ИНН);
	
	Возврат Результат;
	
КонецФункции

Функция ПараметрыЗапросаСобытияМониторингаПоУмолчанию()
	
	Результат = Новый Структура;
	Результат.Вставить("ВключитьКритичные");
	Результат.Вставить("ВключитьИсполнительныеПроизводства", Ложь);
	Результат.Вставить("ИдентификаторСтраницы");
	
	Возврат Результат;
КонецФункции

Процедура ПолучитьДанныеСобытийМониторингаИзСервиса(
	Результат,
	ДанныеАутентификации,
	ТекущийКонтрагент,
	ПараметрыЗапроса)
	...
	ДополнительныеПараметрыЗапроса = ДополнительныеПараметрыВызоваОперацииСервиса();
	Если ТекущийКонтрагент.ВидКонтрагента = Перечисления.ВидыКонтрагентовСПАРКРиски.ЮридическоеЛицо Тогда
		ВызватьОперациюСобытияМониторингаЮридическогоЛица(
			ТекущийКонтрагент.ИНН,
			ПараметрыЗапроса,
			Результат,
			ДанныеАутентификации,
			ДополнительныеПараметрыЗапроса);
	ИначеЕсли ТекущийКонтрагент.ВидКонтрагента = Перечисления.ВидыКонтрагентовСПАРКРиски.ИндивидуальныйПредприниматель Тогда
		ВызватьОперациюСобытияМониторингаИндивидуальногоПредпринимателя(
			ТекущийКонтрагент.ИНН,
			ПараметрыЗапроса,
			Результат,
			ДанныеАутентификации,
...
////////////////////////////////////////////////////////////////////////////////
// Вызов операции /monitoring/organizations/dossier/events

Процедура ВызватьОперациюСобытияМониторингаЮридическогоЛица(
		ИНН,
		ПараметрыЗапроса,
		Результат,
		ДанныеАутентификации,
	...
	URLОперации = URLОперацииСервиса("rest/monitoring/organizations/dossier/events");
	ЗаполнитьТикетАутентификации(ДанныеАутентификации, URLОперации, Результат);
	Если Не Результат.ВидОшибки.Пустая() Тогда
		Возврат;
	КонецЕсли;

	ТелоЗапроса = НовыйТелоЗапросаMonitoringOrganizationDossierEvents(
		ИНН,
		ПараметрыЗапроса,
		ДанныеАутентификации,
		ДополнительныеПараметрыЗапроса);
	...
	Попытка
		ОбработатьТелоОтветаMonitoringOrganizationDossierEvents(ТелоОтвета, Результат);
	Исключение
		ПриОшибкеОбработкиОтветаСервиса(ИнформацияОбОшибке(), URLОперации, ТелоОтвета, Результат);
	КонецПопытки;
...
Функция НовыйТелоЗапросаMonitoringOrganizationDossierEvents(
	ИНН,
	ПараметрыЗапроса,
	ДанныеАутентификации,
	ДополнительныеПараметрыЗапроса)
	...
	Если ПараметрыЗапроса.ВключитьКритичные <> Неопределено Тогда
		ЗаписьДанныхСообщения.ЗаписатьИмяСвойства("critical");
		ЗаписьДанныхСообщения.ЗаписатьЗначение(ПараметрыЗапроса.ВключитьКритичные);
	КонецЕсли;
	Если ПараметрыЗапроса.ВключитьИсполнительныеПроизводства <> Неопределено Тогда
		ЗаписьДанныхСообщения.ЗаписатьИмяСвойства("enforcementProceedings");
		ЗаписьДанныхСообщения.ЗаписатьЗначение(ПараметрыЗапроса.ВключитьИсполнительныеПроизводства);
	КонецЕсли;
	Если ПараметрыЗапроса.ИдентификаторСтраницы <> Неопределено Тогда
		ЗаписьДанныхСообщения.ЗаписатьИмяСвойства("scrollId");
		ЗаписьДанныхСообщения.ЗаписатьЗначение(ПараметрыЗапроса.ИдентификаторСтраницы);
	КонецЕсли;
...
Процедура ОбработатьТелоОтветаMonitoringOrganizationDossierEvents(ТелоОтвета, Результат)
	...
	СобытияМониторингаИзСервиса = ДанныеСервиса.Получить("events");
	ТипыСобытий = ДанныеСервиса.Получить("eventTypes");
	
	Результат.ЗначенияСобытийМониторинга.ДоступЗапрещен = ДанныеСервиса.Получить("accessRestricted");
	Результат.ЗначенияСобытийМониторинга.НомерСледующейСтраницы = ДанныеСервиса.Получить("scrollId");
	
	Для Каждого СобытиеМониторингаИзСервиса Из СобытияМониторингаИзСервиса Цикл
		
		ТипСобытия = ТипыСобытий.Получить(СобытиеМониторингаИзСервиса.Получить("eventTypeUeid"));
		
		СобытиеМониторинга = НовыйСобытиеМониторинга();
		СобытиеМониторинга.Дата = СобытиеМониторингаИзСервиса.Получить("eventDate");
		СобытиеМониторинга.НовоеЗначение = СобытиеМониторингаИзСервиса.Получить("newValue");
		СобытиеМониторинга.СтароеЗначение = СобытиеМониторингаИзСервиса.Получить("oldValue");
		
		СобытиеМониторинга.Описание = ТипСобытия.Получить("description");
		СобытиеМониторинга.Критическое = ТипСобытия.Получить("critical");
		СобытиеМониторинга.ИсполнительноеПроизводство = ТипСобытия.Получить("enforcementProceeding");
	
		Результат.ЗначенияСобытийМониторинга.СобытияМониторинга.Добавить(СобытиеМониторинга);
	КонецЦикла;
...
Функция ИменаСвойствСоЗначениямиДата()
	...
	Результат.Добавить("eventDate");
	
	Возврат Результат;
	
КонецФункции

Функция НовыйСобытиеМониторинга()
	
	Результат = Новый Структура;
	Результат.Вставить("Дата");
	Результат.Вставить("Описание");
	Результат.Вставить("НовоеЗначение");
	Результат.Вставить("СтароеЗначение");
	Результат.Вставить("Критическое");
	Результат.Вставить("ИсполнительноеПроизводство");
	
	Возврат Результат;
	
КонецФункции 
////////////////////////////////////////////////////////////////////////////////
// Вызов операции /enterpreneur/events/dossier/events

Процедура ВызватьОперациюСобытияМониторингаИндивидуальногоПредпринимателя(
		ИНН,
		ПараметрыЗапроса,
		Результат,
		ДанныеАутентификации,
	...
	URLОперации = URLОперацииСервиса("rest/enterpreneur/events/dossier/events");
	ЗаполнитьТикетАутентификации(ДанныеАутентификации, URLОперации, Результат);
	Если Не Результат.ВидОшибки.Пустая() Тогда
		Возврат;
	КонецЕсли;

	ТелоЗапроса = НовыйТелоЗапросаEnterpreneurEventsDossierEvents(
		ИНН,
		ПараметрыЗапроса,
		ДанныеАутентификации,
		ДополнительныеПараметрыЗапроса);
	...
	Попытка
		ОбработатьТелоОтветаEnterpreneurEventsDossierEvents(ТелоОтвета, Результат);
	Исключение
		ПриОшибкеОбработкиОтветаСервиса(ИнформацияОбОшибке(), URLОперации, ТелоОтвета, Результат);
	КонецПопытки;
...
Функция НовыйТелоЗапросаEnterpreneurEventsDossierEvents(
	ИНН,
	ПараметрыЗапроса,
	ДанныеАутентификации,
	ДополнительныеПараметрыЗапроса)
...
Процедура ОбработатьТелоОтветаEnterpreneurEventsDossierEvents(ТелоОтвета, Результат)
