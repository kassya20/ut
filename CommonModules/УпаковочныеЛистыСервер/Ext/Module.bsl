...
Процедура ЗаполнитьСлужебныеРеквизиты(Форма,
	...
	Для Каждого КлючИЗначение Из СтруктураДействий Цикл
		Если ПакетнаяОбработкаТабличнойЧастиСервер.КлючЗаполненияСлужебныхРеквизитовПоддерживается(КлючИЗначение.Ключ) Тогда
	...
	Если СтруктураДействий <> Неопределено
		...
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(ТЧТовары, СтруктураДействий, КэшированныеЗначения);
	КонецЕсли;
