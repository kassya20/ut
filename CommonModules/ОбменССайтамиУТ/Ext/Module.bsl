...
Процедура ЗаполнитьТабличнуюЧастьДокумента(Документ, ТаблицаТоваровСайт, ПрикладныеПараметры) Экспорт
	...
	Для Каждого Набор Из СоставНаборов Цикл
		...
		НоменклатураСуммаНДС = ?(НоменклатураНабора.СуммаНДС = Неопределено, 0, НоменклатураНабора.СуммаНДС); 
		Для Каждого Комплектующие Из Набор.Значение Цикл
			СуммаНабора = СуммаНабора + Комплектующие.Сумма; 
			СуммаНабораНДС = СуммаНабораНДС + Комплектующие.СуммаНДС;
		КонецЦикла;
		КоэффициентПересчета = СуммаНабора / ?(НоменклатураСумма = 0, 1, НоменклатураСумма);
		Если КоэффициентПересчета = 1 И СуммаНабораНДС = 0 Тогда
			Продолжить
		КонецЕсли; 
		Для Каждого Комплектующие Из Набор.Значение Цикл
			ПогрешностьСтоимости = СуммаНабора;
			ПогрешностьНДС       = ?(ЗначениеЗаполнено(СуммаНабораНДС), СуммаНабораНДС, 0);
			СтрокаДляИзменения = ТаблицаТоварыДокумента.Найти(Комплектующие.ИдентификаторСтрокиВТ, "ИдентификаторСтрокиВТ");
			Если КоэффициентПересчета <> 1 Тогда
				СтрокаДляИзменения.Сумма = Комплектующие.Сумма * КоэффициентПересчета;
				СтрокаДляИзменения.СуммаНДС = Комплектующие.СуммаНДС * КоэффициентПересчета;
			КонецЕсли;
			ПогрешностьСтоимости = ПогрешностьСтоимости - Комплектующие.Сумма;
			ПогрешностьНДС       = ПогрешностьНДС - Комплектующие.СуммаНДС; 
		КонецЦикла;	
		...
		Если ПогрешностьСтоимости <> 0 или ПогрешностьНДС <> 0 Тогда
			...
			СтрокаДляИзменения.СуммаНДС  = СтрокаДляИзменения.СуммаНДС + ПогрешностьНДС;
			СтрокаДляИзменения.СуммаСНДС = СтрокаДляИзменения.Сумма + ?(Документ.ЦенаВключаетНДС, 0, Комплектующие.СуммаНДС);
		КонецЕсли;	
