...
// Заполняет данные о способах получения технической поддержки по обмену с ЕИС Госзакупки.
// 
// Параметры:
// 	ТелефонСлужбыПоддержки - Строка
// 	АдресЭлектроннойПочтыСлужбыПоддержки - Строка
// 	КонтекстДиагностики - Структура
Процедура ЗаполнитьДанныеСлужбыПоддержкиЕИС(
	ТелефонСлужбыПоддержки,
	АдресЭлектроннойПочтыСлужбыПоддержки,
	КонтекстДиагностики) Экспорт

	Если Не КонтекстДиагностики.Свойство("ДополнительныеСвойства") Тогда
		Возврат;
	КонецЕсли;

	ОбъектыДействий = Неопределено;
	КонтекстДиагностики.ДополнительныеСвойства.Свойство("ОбъектыДействий", ОбъектыДействий);
	
	Если НЕ ТипЗнч(ОбъектыДействий) = Тип("Структура")
		ИЛИ НЕ ОбъектыДействий.Свойство("ТранспортныеКонтейнеры")
		ИЛИ НЕ ОбъектыДействий.Свойство("ЭлектронныеДокументы") Тогда
		Возврат;
	КонецЕсли;
	
	Если ОбъектыДействий.ТранспортныеКонтейнеры.Количество() = 0
		И ОбъектыДействий.ЭлектронныеДокументы.Количество() =0 Тогда
		Возврат;
	КонецЕсли;
	
	ЕстьДокументыНеДляЕИС = Ложь;
	ЕстьКонтейнерыНеДляЕИС = Ложь;
	Для Каждого Документ из ОбъектыДействий.ЭлектронныеДокументы Цикл
		Если НЕ ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Документ, "СпособОбмена")
				= Перечисления.СпособыОбменаЭД.ЧерезСерверЕИС Тогда
			ЕстьДокументыНеДляЕИС = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Для каждого ТранспортныйКонтейнер из ОбъектыДействий.ТранспортныеКонтейнеры Цикл
		Если НЕ ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТранспортныйКонтейнер, "СпособОбменаЭД")
				= Перечисления.СпособыОбменаЭД.ЧерезСерверЕИС Тогда
			ЕстьКонтейнерыНеДляЕИС = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Если ЕстьДокументыНеДляЕИС ИЛИ ЕстьКонтейнерыНеДляЕИС Тогда
		Возврат;
	КонецЕсли;
	
	// Контактные данные горячей линии 1С
	ТелефонСлужбыПоддержки = "+7-495-956-1181";
	АдресЭлектроннойПочтыСлужбыПоддержки = "v8@1c.ru";
