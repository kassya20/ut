...
Процедура ОбновитьПоставляемыеДрайвера(ДрайвераОборудования) Экспорт
	...
	Драйвер = ДрайвераОборудования.Добавить();
	Драйвер.ТипОборудования = Перечисления.ТипыПодключаемогоОборудования.ДисплейПокупателя;
	Драйвер.ИмяДрайвера  = "ДрайверAQSIСигнальныйФонарьКСО";
	Драйвер.Наименование = НСтр("ru = 'aQsi:Драйвер сигнального фонаря КСО'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
	Драйвер.ИдентификаторОбъекта = "aQsiDrvSCOAddIn"; 
	// -- Локализация      
	// -- НеМобильноеПриложение     
...
Функция ПодготовитьДанныеОперации(ПараметрыПодключения, Команда, ПараметрыОперации) Экспорт
	...
	Если Команда = "DisplayInfo" Тогда      
		...
		ИначеЕсли ПараметрыОперации.Операция = "ПроверкаВозраста" Тогда
			СтраницаПроверкаВозраста(ПараметрыПодключения, ДанныеОперации, ПараметрыОперации);
		ИначеЕсли ПараметрыОперации.Операция = "Картинка" Тогда
			СтраницаКартинка(ПараметрыПодключения, ДанныеОперации, ПараметрыОперации);
		КонецЕсли;
...
Функция ТекстHTMLКартинкаПоШаблонуДисплеяПокупателя(ПараметрыПодключения, ПараметрыШаблона)
	
	ДисплейОтрабатываетНажатия = ПараметрыПодключения.ДисплейОтрабатываетНажатия;
	Шаблон = МенеджерОборудованияПовтИсп.СодержимоеОбщегоМакета("ШаблонДисплеяПокупателяКартинка");
	Шаблон = СтрЗаменить(Шаблон, "%IMAGE%", ПараметрыШаблона.Картинка);
...
Процедура СтраницаПроверкаВозраста(Знач ПараметрыПодключения, Знач ДанныеОперации, Знач ПараметрыОперации)
	
	// ++ Локализация	
	ДвоичныеДанные = Истина;
	QRКодСслыкаНаПроверкуВозраста = БиблиотекаКартинок.QRКодДляПроверкиВозрастаВМаксДисплейПокупателяБПО.ПолучитьДвоичныеДанные();
	КартинкаBase64 = Base64Строка(QRКодСслыкаНаПроверкуВозраста);
	
	Параметры = ПараметрыШаблонаДисплеяПокупателя();
	Параметры.Картинка = КартинкаBase64;
	ДанныеОперации.ТекстHTML = ТекстHTMLКартинкаПоШаблонуДисплеяПокупателя(ПараметрыПодключения, Параметры);
	// -- Локализация	
...
Процедура СтраницаКартинка(Знач ПараметрыПодключения, Знач ДанныеОперации, Знач ПараметрыОперации)
	...
	СтрокиТекста = СтрЗаменить(СтрокиТекста, Символы.ПС, "<br>"); 
	Параметры.Информация = СтрокиТекста;
	Параметры.Картинка = ПараметрыОперации.КартинкаBase64;
