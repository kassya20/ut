...
Процедура СообщитьОРезультатахКонтроляИзменений(РезультатыКонтроля, Документ, Отказ) Экспорт
	...
	Если ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ, "ДвиженияУслугиКОформлениюОтчетовПринципалуИзменение") Тогда
		...
		ШаблонСообщенияСНДС = НСтр("ru = 'По организации %1 не хватает к оформлению %2 %3 на сумму %4 %5 и сумму НДС %6 %7 услуг: %8, у партнера %9'");
		ШаблонСообщенияБезНДС = НСтр("ru = 'По организации %1 не хватает к оформлению %2 %3 на сумму %4 %5 услуг: %6, у партнера %7'");
		...
		Для каждого СтрокаОшибки Из РезультатыКонтроля.ОшибкиУслугиКОформлениюОтчетовПринципалу Цикл
			...
			Если СтрокаОшибки.СуммаНДС < 0 Тогда
				
				ТекстСообщения = СтрШаблон(ШаблонСообщенияСНДС, СтрокаОшибки.Организация, -СтрокаОшибки.Количество, СтрокаОшибки.ЕдиницаИзмерения, -СтрокаОшибки.СуммаВыручки, СтрокаОшибки.Валюта,
					-СтрокаОшибки.СуммаНДС,
					СтрокаОшибки.Валюта,
					ПредставлениеНоменклатуры,
					СтрокаОшибки.Комитент);
			Иначе
				
				ТекстСообщения = СтрШаблон(ШаблонСообщенияБезНДС, СтрокаОшибки.Организация, -СтрокаОшибки.Количество, СтрокаОшибки.ЕдиницаИзмерения, -СтрокаОшибки.СуммаВыручки, СтрокаОшибки.Валюта,
					ПредставлениеНоменклатуры,
					СтрокаОшибки.Комитент);
