...
Процедура ДополнитьОписаниеНастроекПересчетаРеквизитовТабличнойЧасти(Объект,
	...
	КонецЕсли;
	Если Не МестоХраненияВТабличнойЧасти
		И СтруктураПересчетаКоличестваРНПТ.Свойство("МестоХранения") Тогда
		СтруктураДействий.Вставить("МестоХраненияПоРНПТ", СтруктураПересчетаКоличестваРНПТ.МестоХранения);
	КонецЕсли;
	...
	Если Кэш = Неопределено Тогда
		#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
			...
			Если УчетПрослеживаемыхТоваровЛокализация.ИспользоватьУчетПрослеживаемыхИмпортныхТоваров(Дата) Тогда
				...
				ЗначенияРеквизитов = ПакетнаяОбработкаТабличнойЧастиСервер.ПолучитьКоэффициентРНПТ(ИсключаемыйДокумент,
																							Организация,
																							Товары,
...
Функция КлючКэшаКоэффициентРНПТ(ТекущаяСтрока, ПараметрыПересчета) Экспорт
	...
	КлючКэша = КлючКэша + ?(ЗначениеЗаполнено(МестоХранения),
							...
							ПустоеЗначение);
	КлючКэша = КлючКэша + ?(ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ТекущаяСтрока, "НомерГТД")
