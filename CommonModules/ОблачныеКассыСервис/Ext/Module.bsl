...
// ОбщийМодуль.ОблачныеКассыСервис.
//
...
//          "SalesRegisterGroup" - особый тип аутентификации работающий по принципу Resource Owner Password Credentials
//            Grant.
...
// Формирует новый объект содержащий служебные операции пробития чека которые возвращает сервер Облачной кассы.
//
// Возвращаемое значение:
//  Структура:
...
Функция ПараметрыАутентификацииОблачнойКассы(ПараметрыПодключения, ОбновитьТокен = Ложь)
	...
	КлючевыеПараметрыАутентификации = Новый Структура("ТипАутентификации");
	Если ТипЗнч(ПараметрыАутентификации) = Тип("Структура") Тогда
		ЗаполнитьЗначенияСвойств(КлючевыеПараметрыАутентификации, ПараметрыАутентификации);
	КонецЕсли;
	
	Если КлючевыеПараметрыАутентификации.ТипАутентификации = Неопределено Тогда
		
		Результат.КодОшибки = КодОшибкиОшибкаАутентификации();
		Результат.СообщениеОбОшибке = НСтр("ru = 'Не заполнены параметры авторизации в сервисе Облачной кассы.'");
		Результат.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Запись безопасного хранилища с параметрами аутентификации Облачной кассы ""%1"" не найдена или повреждена.'"),
			Строка(ПараметрыПодключения.НастройкаПодключения));
		Возврат Результат;
		
	ИначеЕсли ПараметрыАутентификации.ТипАутентификации = ОблачныеКассыКлиентСервер.ТипАутентификацииSalesRegisterGroup() Тогда
...
// Выполняет запрос к сервису получения токена аутентификации в сервисах Облачной кассы.
//
// Параметры:
//  ПараметрыПодключения - см. ОблачныеКассыКлиентСервер.НовыйПараметрыПодключения
//
...
////////////////////////////////////////////////////////////////////////////////
// Вызов операции POST /program-1c/private/tokens/{serviceNick}/request

// Получает токен для аутентификации в сервисе облачной кассы.
//
// Параметры:
...
Функция ОперацияЗапросТокенаАутентификацииСервиса(
	...
	ОблачныеКассыСлужебный.ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ru = 'Получение токена в сервисе облачной кассы.'"),
		Ложь);
	
	РезультатОперации = ОблачныеКассыСлужебный.НовыйРезультатОперации();
	...
	URLОперации = URLОперацииСервиса(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			"/private/tokens/%1/request",
			ИмяСервиса));
	
	ПараметрыАутентификацииИПП = ПараметрыАутентификацииИПП(URLОперации);
	Если Не ПустаяСтрока(ПараметрыАутентификацииИПП.КодОшибки) Тогда
		ЗаполнитьЗначенияСвойств(РезультатОперации, ПараметрыАутентификацииИПП);
	...
	Заголовки.Вставить("Authorization", ПараметрыАутентификацииИПП.Аутентификация);
...
Функция tokens_request(
	...
	Если ПараметрыНастройки.ПараметрыАутентификации.ТипАутентификации
		= ОблачныеКассыКлиентСервер.ТипАутентификацииSalesRegisterGroup() Тогда
		
		ЗаписьДанныхСообщения.ЗаписатьИмяСвойства("shopId");
		ЗаписьДанныхСообщения.ЗаписатьЗначение(ПараметрыНастройки.ПараметрыАутентификации.ИдентификаторМагазина);
		
		ЗаписьДанныхСообщения.ЗаписатьИмяСвойства("shopSecret");
		ЗаписьДанныхСообщения.ЗаписатьЗначение(ПараметрыНастройки.ПараметрыАутентификации.СекретныйКлючМагазина);
		
		ЗаписьДанныхСообщения.ЗаписатьИмяСвойства("kktsGroupId");
		ЗаписьДанныхСообщения.ЗаписатьЗначение(ПараметрыНастройки.ПараметрыАутентификации.ИдентификаторГруппыКасс);
		
	ИначеЕсли ПараметрыНастройки.ПараметрыАутентификации.ТипАутентификации
		= ОблачныеКассыКлиентСервер.ТипАутентификацииBearer() Тогда
		
		ЗаписьДанныхСообщения.ЗаписатьИмяСвойства("token");
		ЗаписьДанныхСообщения.ЗаписатьЗначение(ПараметрыНастройки.ПараметрыАутентификации.Токен);
		
	ИначеЕсли  ПараметрыНастройки.ПараметрыАутентификации.ТипАутентификации
		= ОблачныеКассыКлиентСервер.ТипАутентификацииBasic() Тогда
		
		ЗаписьДанныхСообщения.ЗаписатьИмяСвойства("userName");
		ЗаписьДанныхСообщения.ЗаписатьЗначение(ПараметрыНастройки.ПараметрыАутентификации.Логин);
		
		ЗаписьДанныхСообщения.ЗаписатьИмяСвойства("password");
		ЗаписьДанныхСообщения.ЗаписатьЗначение(ПараметрыНастройки.ПараметрыАутентификации.Пароль);
	...
	ЗаписьДанныхСообщения.ЗаписатьЗначение(ПараметрыНастройки.Идентификатор);
...
// Чтение ответа операции /program-1c/private/tokens/{serviceNick}/request
//
...
Функция ОперацияОтзывТокеновАутентификацииСервиса(
	...
	ОблачныеКассыСлужебный.ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ru = 'Отзыв токенов в сервисе облачной кассы.'"),
		Ложь);
	...
	ПараметрыЗапросаJSON = tokens_revoke(ИдентификаторыТокенов);
...
Функция НовыйИтеративныйВызовОперации(Длительность) Экспорт
	...
	// Для небольшого значения длительности будет выполнено второй вызова с одним ожиданием.
...
//
// Возвращаемое значение:
//  Булево - если Истина, то правила настройки вызова разрешают выполнить очередной вызов.
//
...
//
// Параметры:
//  Приемник - см. СтатусПробитияЧека
//  ОтветСервиса - Структура - данные ответа из сервиса.
//
Процедура ОбработатьОтветВыполненияОперации(Приемник, ОтветСервиса) Экспорт
	...
	// Результат фискализации возвращается как есть
...
//    ошибку, в противном случае будут сохраняться в журнал регистраций все полученные ответы.
//    Значение по умолчанию: Ложь.
...
Функция ВыполнитьОперациюСервиса(
	...
	ИдентификаторОперации = ПараметрыЗапроса.Заголовки["X-Correlation-ID"];
	ДанныеДляОбработки.requestHeaders.Вставить("X-Correlation-ID", ИдентификаторОперации);
...
Функция ОбработатьОтветСервиса(ОтветСервиса)
	...
	Исключение
		...
		// Ошибка сериализации ответа может быть если в поле responseBodyJsonObject есть поля не отвечающие требованию к
		// именам ключей структуры. Такие поля могут быть только при ошибке фискализации. Попытаемся получить информацию
		// об ошибке.
		Попытка
			ДанныеОтвета = ОбработатьАльтернативныйОтветСервиса(ОтветСервиса);
		Исключение
			Результат.КодОшибки          = КодОшибкиОшибкаСервиса();
			Результат.СообщениеОбОшибке  = НСтр("ru = 'Ошибка работы с сервисом Облачной кассы.'");
			Результат.ИнформацияОбОшибке = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
// Парсит ответ сервиса в альтернативном формате, извлекая статус и тело ответа.
//
// Параметры:
//  ОтветСервиса - Строка - JSON-строка с ответом от сервиса.
//
// Возвращаемое значение:
//  Структура:
//    * responseStatus - Число - статус выполнения запроса.
//    * responseBodyJsonObject - Неопределено - ответ сервиса не сожержит ключ responseBodyJsonObject.
//                             - см. ДанныеОшибкиСервиса
//
Функция ОбработатьАльтернативныйОтветСервиса(ОтветСервиса)
	
	ОтветСтатус = 0;
	ОтветТелоСервис = Неопределено;
	
	ДанныеОтвета = JSONВЗначение(ОтветСервиса, Истина);	// Соответствие из КлючИЗначение
	Для Каждого КлючИЗначение Из ДанныеОтвета Цикл
		Если СтрСравнить(КлючИЗначение.Ключ, "responseStatus") = 0 Тогда
			ОтветСтатус = КлючИЗначение.Значение;	// Число
		ИначеЕсли СтрСравнить(КлючИЗначение.Ключ, "responseBodyJsonObject") = 0 Тогда
			ОтветТелоСервис = КлючИЗначение.Значение;	// Соответствие из КлючИЗначение
		КонецЕсли;
	КонецЦикла;
	
	Результат = Новый Структура();
	Результат.Вставить("responseStatus", ОтветСтатус);
	Результат.Вставить("responseBodyJsonObject", Неопределено);
	
	Если ОтветТелоСервис = Неопределено Тогда
		Возврат Результат;
	КонецЕсли;
	
	ОтветТело = Новый Структура();
	ОтветТело.Вставить("type", "");
	ОтветТело.Вставить("title", "");
	ОтветТело.Вставить("detail", "");
	ОтветТело.Вставить("status", 0);
	ОтветТело.Вставить("instance", "");
	
	Для Каждого КлючИЗначение Из ОтветТелоСервис Цикл
		Если ОтветТело.Свойство(КлючИЗначение.Ключ) Тогда
			ОтветТело[КлючИЗначение.Ключ] = КлючИЗначение.Значение;
		КонецЕсли;
	КонецЦикла;
	
	Результат.responseBodyJsonObject = ОтветТело;
...
//      см. КодОшибкиНеизвестнаяОшибка - прочая ошибка выполнения запроса.
//
...
//
Функция ПереопределитьСообщениеПользователю(КодОшибки, КодОшибкиСервиса = "", КодСостояния = 0)
...
// Смотри ограничения в синтакс-помощнике.
//
// Параметры:
//  Строка - Строка - значение в формате JSON.
//  ПрочитатьВСоответствие - Булево - если установлено Истина, чтение объекта JSON будет выполнено в Соответствие, иначе
//    в Структура.
//
// Возвращаемое значение:
//  Произвольный
//
Функция JSONВЗначение(Строка, ПрочитатьВСоответствие = Ложь)
