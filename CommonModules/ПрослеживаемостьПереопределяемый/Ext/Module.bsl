...
// Получает единицу измерения товаров по РНПТ.
//
// Параметры:
//  РНПТ - СправочникСсылка.НомераГТД - номер РНПТ.
//
// Возвращаемое значение:
//  СправочникСсылка.УпаковкиЕдиницыИзмерения - единица измерения.
//
Функция ЕдиницаПрослеживаемостиПоРНПТ(РНПТ) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	НомераГТД.ЕдиницаИзмерения КАК ЕдиницаИзмерения
	|ИЗ
	|	Справочник.НомераГТД КАК НомераГТД
	|ГДЕ
	|	НомераГТД.Ссылка = &РНПТ
