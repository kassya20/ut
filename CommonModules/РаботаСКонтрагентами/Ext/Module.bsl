...
//			** ДатаРегистрации - Дата - дата постановки на учет в ФСС.
//		* РегистрацияВСФР - Структура, Неопределено - данные о регистрации в СФР.
//			*** РегистрационныйНомерСФР - Строка - регистрационный номер предпринимателя в СФР.
//			*** КодОрганаСФР - Строка - код органа СФР.
//			*** НаименованиеОрганаСФР - Строка - наименование органа СФР.
//			*** ДатаРегистрации - Дата - дата постановки на учет в СФР.
...
Функция ЮридическиеЛицаПоНаименованию(Знач Наименование, Знач КодРегиона = "", Знач Адрес = "") Экспорт
	...
	Если ТипЗнч(ЗначениеCorporations) = Тип("Массив") Тогда
		Для Каждого ТекущееCorporation Из ЗначениеCorporations Цикл
			...
			РеквизитыОрганизации.РегистрационныйНомер = ТекущееCorporation.Получить("ogrn");
			РеквизитыОрганизации.ДатаРегистрации = ДатаИзJSON(ТекущееCorporation.Получить("registrationDate"));
			ЗначениеterminationDate = ТекущееCorporation.Получить("terminationDate");
			Если ЗначениеterminationDate <> Неопределено Тогда
				РеквизитыОрганизации.ДатаПрекращенияДеятельности = ДатаИзJSON(ЗначениеterminationDate);
			КонецЕсли;
			...
			ЗаполнитьДанныеОРуководителях(ТекущееCorporation, РеквизитыОрганизации);
			ЗаполнитьСведенияОРуководителях(ТекущееCorporation, РеквизитыОрганизации);
			ЗаполнитьИнформациюОСтатусеСтруктура(ТекущееCorporation, РеквизитыОрганизации);
			ЗаполнитьИнформациюОРегистрирующемОргане(ТекущееCorporation, РеквизитыОрганизации);
...
//		Содержит основные данные о юридическом лице - см. НоваяИнформацияОСвязяхЮридическогоЛица.
...
Функция ИнформацияОСвязяхЮридическогоЛицаПоИНН(Знач ИНН) Экспорт
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Получение данных юридического лица с ИНН %1'"),
			ИНН),
		"Контрагент",
		ДополнительноеСобытиеПолучениеДанных());
	...
	Если ТелоОтветаОбъект.Получить("inn") = Неопределено Тогда
		Результат.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось найти данные для юридического лица с ИНН %1.'"),
			Результат.ИНН);
		ЗаписатьИнформациюВЖурналРегистрации(
			Результат.ОписаниеОшибки,
			ИдентификаторСервисаКонтрагент(),
	...
	ЗаполнитьРеквизитыСФР(ТелоОтветаОбъект, Результат);
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Успешно завершено получение данных юридического лица с ИНН %1'"),
			ИНН),
		"Контрагент",
		ДополнительноеСобытиеПолучениеДанных());
...
Функция ИнформацияОСвязяхПредпринимателяПоИНН(Знач ИНН) Экспорт
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Получение данных предпринимателя с ИНН %1'"),
			ИНН),
		"Контрагент",
		ДополнительноеСобытиеПолучениеДанных());
	...
	Если ТелоОтветаОбъект.Получить("inn") = Неопределено Тогда
		Результат.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось найти данные для предпринимателя с ИНН %1.'"),
			Результат.ИНН);
		ЗаписатьИнформациюВЖурналРегистрации(
			Результат.ОписаниеОшибки,
			ИдентификаторСервисаКонтрагент(),
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Завершено получение данных предпринимателя с ИНН %1'"),
			ИНН),
		"Контрагент",
		ДополнительноеСобытиеПолучениеДанных());
...
Функция ИнформацияОПроверкахКонтролирующимиОрганамиПоСпискуИНН(
	...
	Возврат ИнформацияОПроверкахКонтролирующимиОрганамиПоСпискуИННСлужебная(
		СписокИНН, Плановые, Внеплановые, Завершенные, Незавершенные);
...
Функция РеквизитыНалоговогоОрганаПоКоду(Знач КодНалоговогоОргана) Экспорт
	...
	РезультатВызова = ПроверитьУслугуИВызватьОперациюSOAPСервиса(
		"findIfnsByCode",
		Результат,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	...
	Ответ = РезультатВызова.ТелоОтветаОбъект;
...
Процедура ПриОпределенииМетодовРазрешенныхДляВызоваКакПроизвольныйКод(Методы) Экспорт
	...
	Методы.Вставить("РеквизитыПредпринимателяПоИНН", Истина);
	Методы.Вставить("ЮридическиеЛицаПоИНН", Истина);
	Методы.Вставить("ЮридическиеЛицаПоНаименованию", Истина);
...
Функция ПереопределитьОписаниеОшибки(
	...
	ИначеЕсли РезультатОперации.КодСостояния < 0 Тогда
		...
		ОписаниеОшибки = "ЗапрещенДоступКИнтернетСервисам";
		ЗаписатьОшибкуВЖурналРегистрации(
...
Функция НовыеРеквизитыЮридическогоЛица()
	...
	РеквизитыОрганизации.Вставить("РегистрацияВСФР");
...
Функция НовыеРеквизитыПредпринимателя()
	...
	РеквизитыПредпринимателя.Вставить("РегистрацияВСФР");
...
Функция НоваяРегистрацияВСФР()
	
	Результат = Новый Структура;
	Результат.Вставить("РегистрационныйНомерСФР");
	Результат.Вставить("КодОрганаСФР");
	Результат.Вставить("НаименованиеОрганаСФР");
	Результат.Вставить("ДатаРегистрации");
	Возврат Результат;
...
//		Содержит основные данные о юридическом лице - см. НоваяИнформацияОСвязяхЮридическогоЛица.
//		Также содержит данные:
//		* Статус - Структура, Неопределено - текущий статус юридического лица.
//			Поля:
//			** Код - Строка - код статуса;
//			** Наименование - Строка - наименование статуса;
//		* РегистрирующийОрган - Структура, Неопределено - орган, зарегистрировавший юридическое лицо. Поля:
//			** Код - Строка - код органа;
//			** Наименование - Строка - наименование органа;
//		* УставныйКапитал - Число, Неопределено - размер уставного капитала, рублей.
//		* ВидыДеятельности - ТаблицаЗначений - информация о видах деятельности.
//			** Код - Строка - код вида деятельности;
//			** Наименование - Строка - наименование вида деятельности;
//			** Классификатор - Строка - тип классификатор экономической деятельности, согласно которому указаны код и наименование;
...
//			Для обработки ошибки на клиентской части необходимо использовать метод
//			РаботаСКонтрагентамиКлиент.ОбработатьОшибку;
//		* НаименованиеСтатуса - Строка, Неопределено - устарело, необходимо использовать Статус.Наименование - наименование текущего статуса;
//		* КодСтатуса - Строка, Неопределено - устарело, необходимо использовать Статус.Код - код текущего статуса.
//
Функция НоваяИнформацияОСвязяхЮридическогоЛица() Экспорт
...
//		Содержит основные данные о предпринимателе - см. РеквизитыПредпринимателяПоИНН.
//		Также содержит данные:
//		* Статус - Структура, Неопределено - текущий статус предпринимателя.
//			Поля:
//			** Код - Строка - код статуса;
//			** Наименование - Строка - наименование статуса;
//		* ВидыДеятельности - ТаблицаЗначений - информация о видах деятельности.
...
//		* КодСтатуса - Строка,Неопределено - устарело, необходимо использовать Статус.Код - код текущего статуса.
//
Функция НоваяИнформацияОСвязяхПредпринимателя() Экспорт
...
Процедура ЗаполнитьДанныеЮридическогоЛица(
	...
	Если ТипЗнч(ЗначениеCorporations) = Тип("Массив")
		...
		ДанныеЮридическогоЛица = ЗначениеCorporations[0];
	КонецЕсли;
	
	Если ДанныеЮридическогоЛица <> Неопределено Тогда
		
		РеквизитыЮридическогоЛица = НовыеРеквизитыЮридическогоЛица();
		РеквизитыЮридическогоЛица.РегистрационныйНомер = ДанныеЮридическогоЛица.Получить("ogrn");
		РеквизитыЮридическогоЛица.ИНН = ИНН;
		ЗначениеKpp = ДанныеЮридическогоЛица.Получить("kpp");
		Если ЗначениеKpp <> Неопределено Тогда
			РеквизитыЮридическогоЛица.КПП = ЗначениеKpp.Получить("value");
		КонецЕсли;
		
		РеквизитыЮридическогоЛица.ДатаРегистрации = ДатаИзJSON(ДанныеЮридическогоЛица.Получить("registrationDate"));
		ЗначениеterminationDate = ДанныеЮридическогоЛица.Получить("terminationDate");
		Если ЗначениеterminationDate <> Неопределено Тогда
			РеквизитыЮридическогоЛица.ДатаПрекращенияДеятельности = ДатаИзJSON(ЗначениеterminationDate);
		КонецЕсли;
		
		ЗначениеstandardCharterNumber = ДанныеЮридическогоЛица.Получить("standardCharterNumber");
		Если ЗначениеstandardCharterNumber <> Неопределено Тогда
			РеквизитыЮридическогоЛица.НомерТиповогоУстава = ЗначениеstandardCharterNumber;
		КонецЕсли;
		
		ЗаполнитьНаименованияЮридическогоЛица(ДанныеЮридическогоЛица.Получить("name"), РеквизитыЮридическогоЛица);
		ЗаполнитьКодОКВЭД(ДанныеЮридическогоЛица, РеквизитыЮридическогоЛица);
		ЗаполнитьВидДеятельности(ДанныеЮридическогоЛица, РеквизитыЮридическогоЛица);
		ЗаполнитьЮридическийАдрес(ДанныеЮридическогоЛица.Получить("address"), РеквизитыЮридическогоЛица);
		ЗаполнитьРегистрациюВНалоговомОргане(ДанныеЮридическогоЛица, РеквизитыЮридическогоЛица);
		ЗаполнитьРеквизитыПенсионногоФонда(ДанныеЮридическогоЛица, РеквизитыЮридическогоЛица);
		ЗаполнитьРеквизитыФондаСоциальногоСтрахования(ДанныеЮридическогоЛица, РеквизитыЮридическогоЛица);
		ЗаполнитьРеквизитыСФР(ДанныеЮридическогоЛица, РеквизитыЮридическогоЛица);
		ЗаполнитьСведенияОРуководителях(ДанныеЮридическогоЛица, РеквизитыЮридическогоЛица);
		ЗаполнитьДанныеОРуководителях(ДанныеЮридическогоЛица, РеквизитыЮридическогоЛица);
		ЗаполнитьИнформациюОСтатусеСтруктура(ДанныеЮридическогоЛица, РеквизитыЮридическогоЛица);
		ЗаполнитьИнформациюОРегистрирующемОргане(ДанныеЮридическогоЛица, РеквизитыЮридическогоЛица);
		ЗаполнитьОткрытыеГосударственныеДанныеФНС(ДанныеЮридическогоЛица.Получить("openDataInfo"), РеквизитыЮридическогоЛица);
		ЗаполнитьИсториюРеквизитовЮридическогоЛица(ДанныеЮридическогоЛица, РеквизитыЮридическогоЛица);
...
Процедура ЗаполнитьИнформациюОПодразделениях(ТелоОтветаОбъект, ИнформацияОСвязях)
	...
	Для Каждого ТекущееUnitInfos Из ЗначениеUnitInfos Цикл
		...
		Иначе
			ЗначениеForeignCountry = ТекущееUnitInfos.Получить("foreignCountry");
			Если ЗначениеForeignCountry <> Неопределено Тогда
				ОписаниеПодразделения = ЗначениеForeignCountry;
			Иначе
...
Процедура ЗаполнитьРеквизитыПенсионногоФонда(ТелоОтветаОбъект, Реквизиты)
	...
	РегистрацияВПФР.ДатаРегистрации         = ДатаИзJSON(
		ЗначениеPensionFundRegistrationInfo.Получить("registrationDate"));
	
	Если РегистрацияВПФР.РегистрационныйНомерПФР <> Неопределено Тогда
		РегистрацияВПФР.РегистрационныйНомерПФР = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1-%2-%3'"),
	...
	Если РегистрацияВПФР.КодОрганаПФР <> Неопределено Тогда
		РегистрацияВПФР.КодОрганаПФР = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1-%2'"),
			Лев(РегистрацияВПФР.КодОрганаПФР, 3),
			Прав(РегистрацияВПФР.КодОрганаПФР, 3));
	КонецЕсли;
...
Процедура ЗаполнитьРеквизитыФондаСоциальногоСтрахования(ТелоОтветаОбъект, Реквизиты)
	...
	РегистрацияВФСС.ДатаРегистрации = ДатаИзJSON(
		ЗначениеSocialInsuranceFundRegistrationInfo.Получить("registrationDate"));
	Реквизиты.РегистрацияВФСС = РегистрацияВФСС;
	
КонецПроцедуры

Процедура ЗаполнитьРеквизитыСФР(ТелоОтветаОбъект, Реквизиты)
	
	ЗначениеSocialFundRegistrationInfo = ТелоОтветаОбъект.Получить("socialFundRegistrationInfo");
	Если ЗначениеSocialFundRegistrationInfo = Неопределено Тогда
		Возврат;
	КонецЕсли;
		
	РегистрацияВСФР = НоваяРегистрацияВСФР();
	РегистрацияВСФР.РегистрационныйНомерСФР = ЗначениеSocialFundRegistrationInfo.Получить("registrationNumber");
	РегистрацияВСФР.КодОрганаСФР            = ЗначениеSocialFundRegistrationInfo.Получить("fundCode");
	РегистрацияВСФР.НаименованиеОрганаСФР   = ЗначениеSocialFundRegistrationInfo.Получить("fundName");
	РегистрацияВСФР.ДатаРегистрации         = ДатаИзJSON(ЗначениеSocialFundRegistrationInfo.Получить("registrationDate"));
	
	Реквизиты.РегистрацияВСФР = РегистрацияВСФР;
...
Процедура ЗаполнитьСведенияОПодразделениях(ТелоОтветаОбъект, Реквизиты)
	...
	Для Каждого ДанныеПодразделения Из ДанныеПодразделений Цикл
		...
		Подразделение = НовыйДанныеПодразделения();
		Если ДанныеПодразделения = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Если ДанныеПодразделения.Получить("inn") <> Неопределено Тогда
			Подразделение.ИНН = ДанныеПодразделения.Получить("inn");
		КонецЕсли;
		Если ДанныеПодразделения.Получить("name") <> Неопределено Тогда
			Подразделение.Наименование = ДанныеПодразделения.Получить("name");
		КонецЕсли;
		Если ДанныеПодразделения.Получить("id") <> Неопределено Тогда
			Подразделение.Идентификатор = ДанныеПодразделения.Получить("id");
		КонецЕсли;
		
		Если ДанныеПодразделения.Получить("address") <> Неопределено Тогда
			ЗаполнитьЮридическийАдрес(ДанныеПодразделения.Получить("address"), Подразделение);
		ИначеЕсли ДанныеПодразделения.Получить("foreignCountry") <> Неопределено Тогда
			ДанныеАдреса = Новый Соответствие;
			ДанныеАдреса.Вставить("country", ДанныеПодразделения.Получить("foreignCountry"));
			ДанныеАдреса.Вставить("munValue", ДанныеПодразделения.Получить("foreignCountry"));
			ЗаполнитьЮридическийАдрес(ДанныеАдреса, Подразделение);
		Иначе
			Подразделение.ЮридическийАдрес = Неопределено;
		КонецЕсли;
...
Функция НовыйДанныеПодразделения()
...
Процедура ЗаполнитьДанныеПодразделения(Результат, ДанныеПодразделения)
	
	Если ДанныеПодразделения.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	Результат.ИНН = ДанныеПодразделения.Получить("inn");
	
	РеквизитыПодразделения = НовыеРеквизитыПодразделения();
	РеквизитыПодразделения.ИНН = ДанныеПодразделения.Получить("inn");
	РеквизитыПодразделения.Наименование = СокрЛП(ДанныеПодразделения.Получить("name"));
	РеквизитыПодразделения.РегистрационныйНомер = ДанныеПодразделения.Получить("ogrn");
	РеквизитыПодразделения.КПП = ДанныеПодразделения.Получить("kpp");
	РеквизитыПодразделения.ВидДеятельности = Новый Структура(
		"Код, Наименование",
		ДанныеПодразделения.Получить("okvedCode"),
		ДанныеПодразделения.Получить("okvedName"));
	ЗаполнитьВидПодразделения(ДанныеПодразделения.Получить("type"), РеквизитыПодразделения);
	Если ДанныеПодразделения.Получить("address") <> Неопределено Тогда
		ЗаполнитьЮридическийАдрес(ДанныеПодразделения.Получить("address"), РеквизитыПодразделения);
	ИначеЕсли ДанныеПодразделения.Получить("foreignCountry") <> Неопределено Тогда
		ДанныеАдреса = Новый Соответствие;
		ДанныеАдреса.Вставить("country", ДанныеПодразделения.Получить("foreignCountry"));
		ДанныеАдреса.Вставить("munValue", ДанныеПодразделения.Получить("foreignCountry"));
		ЗаполнитьЮридическийАдрес(ДанныеАдреса, РеквизитыПодразделения);
	КонецЕсли;
	...
	Для Каждого ТекущееFounderInfos Из ДанныеСервиса Цикл
		...
		Иначе
			...
			ИначеЕсли ЗначениеType = "MUNICIPALITY" Тогда
				...
				Если ПустаяСтрока(ТекущееFounderInfos.Получить("name")) Тогда
					СтрокаУчредителя.Учредитель = ТекущееFounderInfos.Получить("regionName");
				Иначе
					СтрокаУчредителя.Учредитель = ТекущееFounderInfos.Получить("name");
				КонецЕсли;
			КонецЕсли;
...
Функция ПроверитьУслугуИВызватьОперациюSOAPСервиса(
	ИмяМетода,
	Параметры,
	ШаблонОшибки = "")
	...
	Если Не УслугаПодключена(ИдентификаторУслуги) Тогда
		Результат.ОписаниеОшибки = "УслугаНеПодключена";
	КонецЕсли;
	...
	Если ПустаяСтрока(Результат.ОписаниеОшибки) Тогда
		ЗаполнитьТикетАутентификации(Прокси, ПараметрыСервиса.URL, ИмяМетода, Результат.ОписаниеОшибки);
	КонецЕсли;
	...
	Если ПустаяСтрока(Результат.ОписаниеОшибки) Тогда
		...
		ВходныеПараметры.Code = Параметры.Код;
		
		ПараметрыМетода = Новый Массив();
		ПараметрыМетода.Добавить(Параметры.Код);
		...
		Попытка
			Результат.ТелоОтветаОбъект = ВызватьОперациюSOAPСервиса(
				Прокси,
				ИмяМетода,
				ПараметрыМетода);
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			Результат.ОписаниеОшибки = ШаблонОшибки
				+ Символы.ПС
				+ ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
		КонецПопытки;
...
Функция ВызватьОперациюSOAPСервиса(Прокси, ИмяМетода, Параметры)
	
	Если ИмяМетода = "findPfrByCode" Тогда
		Возврат Прокси.findPfrByCode(Параметры[0], Параметры[1])
	ИначеЕсли ИмяМетода = "findFssByCode" Тогда
		Возврат Прокси.findFssByCode(Параметры[0], Параметры[1])
	ИначеЕсли ИмяМетода = "findIfnsByCode" Тогда
		Возврат Прокси.findIfnsByCode(Параметры[0], Параметры[1])
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Некорректное значение параметра ""Имя метода"" (%1).'"),
			ИмяМетода);
	КонецЕсли;
...
Процедура ПриДобавленииОписанийСервисовСопровождения(СервисыСопровождения) Экспорт
	...
	ОписательСервиса.Наименование    = НСтр("ru = '1С:Контрагент'");
	ОписательСервиса.Описание        = НСтр("ru = 'Быстрая проверка информации о контрагентах, автоматическое заполнение реквизитов контрагентов в различных документах, поиск контрагентов и пр.'");
