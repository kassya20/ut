...
Процедура ВключитьПослеОтправкиПолученияДокументовЭДО(ОповещениеОЗавершении, ПаролиСертификатов) Экспорт
	
	Контекст = НовыйКонтекстВключенияПослеОтправкиПолученияДокументовЭДО();
	Контекст.ОповещениеОЗавершении = ОповещениеОЗавершении;
	
#Если МобильныйКлиент Тогда
	ВключитьПослеОтправкиПолученияДокументовЭДОЗавершение(Неопределено, Контекст);
	Возврат;
#КонецЕсли
	...
	Сертификаты = ОбщегоНазначенияБЭДКлиент.ВыгрузитьКолонку(ПаролиСертификатов, "Ключ");
	
	ПараметрыВключения = АвтоматическоеПолучениеЭДОВызовСервера.ПараметрыАвтоматическогоВключения(Сертификаты);
	Если Не ЗначениеЗаполнено(ПараметрыВключения.ИдентификаторыЭДО) Тогда
		ВключитьПослеОтправкиПолученияДокументовЭДОЗавершение(Неопределено, Контекст);
		Возврат;
	КонецЕсли;
	
	Контекст.ЭтоВозобновление = ПараметрыВключения.ЭтоВозобновление;
	
	Оповещение = Новый ОписаниеОповещения("ВключитьПослеОтправкиПолученияДокументовЭДОЗавершение",
		ЭтотОбъект, Контекст);
	
	ПараметрыАвторизации = УчетныеЗаписиЭДОКлиент.НовыеПараметрыАвторизацииВСервисеЭДО();
	ПараметрыАвторизации.ИдентификаторыЭДО = ПараметрыВключения.ИдентификаторыЭДО;
...
// Возвращаемое значение:
//  Структура:
//  * ОповещениеОЗавершении - см. ВключитьПослеОтправкиПолученияДокументовЭДО.ОповещениеОЗавершении
//  * ЭтоВозобновление - Булево
Функция НовыйКонтекстВключенияПослеОтправкиПолученияДокументовЭДО()
	
	Контекст = Новый Структура;
	Контекст.Вставить("ОповещениеОЗавершении", Новый ОписаниеОповещения);
	Контекст.Вставить("ЭтоВозобновление", Ложь);
	Возврат Контекст;
...
//                     - см. ВключитьПоУчетнымЗаписямЭДОЗавершение
//  Контекст - см. НовыйКонтекстВключенияПослеОтправкиПолученияДокументовЭДО
// 
// Возвращаемое значение:
//  См. НовыйРезультатВключенияПослеОтправкиПолученияДокументовЭДО
Функция ВключитьПослеОтправкиПолученияДокументовЭДОЗавершение(РезультатВключения, Контекст) Экспорт
	
	Результат = НовыйРезультатВключенияПослеОтправкиПолученияДокументовЭДО();
	Если ЗначениеЗаполнено(РезультатВключения) Тогда
		Результат.Успех = ЗначениеЗаполнено(РезультатВключения.ИзменениеСостояния);
		Результат.КонтекстДиагностики = РезультатВключения.КонтекстДиагностики;
	КонецЕсли;
	
	Если Результат.Успех И Не Контекст.ЭтоВозобновление Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Новые документы теперь будут загружаться автоматически сразу по мере поступления.'"));
	КонецЕсли;
...
Функция НовыйРезультатВключенияПослеОтправкиПолученияДокументовЭДО()
...
Процедура ОповеститьОПолученииЭДО(РезультатПолучения, Контекст = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(РезультатПолучения.ИтогДействийПоЭДО.ОбработанныеДокументы) Тогда
		...
		ИнтерфейсДокументовЭДОКлиент.ПослеОтправкиПолученияДокументовЭДО(Результат, Новый ОписаниеОповещения);
