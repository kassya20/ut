...
Функция ЗапросКМИзXMLПакета(ДанныеXML) Экспорт
	...
	Параметры = ОборудованиеЧекопечатающиеУстройстваКлиентСервер.ПараметрыЗапросКМ();
	
	Если Не ПустаяСтрока(ДанныеXML) Тогда
		ЧтениеXML = Новый ЧтениеXML; 
		ЧтениеXML.УстановитьСтроку(ДанныеXML);
		ЧтениеXML.ПерейтиКСодержимому();
		Если ЧтениеXML.Имя = "RequestKM" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда     
			Параметры.ИдентификаторЗапроса = ЧтениеXML.ЗначениеАтрибута("GUID");
			Параметры.ОжидатьПолучениеОтветаОИСМ = ВРег(ЧтениеXML.ЗначениеАтрибута("WaitForResult")) = "TRUE";  
			Параметры.ОтправлятьНаСерверОИСМ = ВРег(ЧтениеXML.ЗначениеАтрибута("NotSendToServer")) = "TRUE"; 
			Параметры.ПланируемыйСтатусТовара = Число(ЧтениеXML.ЗначениеАтрибута("PlannedStatus")); 
			Значение = ЧтениеXML.ЗначениеАтрибута("Quantity");
			Если Не ПустаяСтрока(Значение) Тогда
				Параметры.Количество = Число(Значение);  
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
