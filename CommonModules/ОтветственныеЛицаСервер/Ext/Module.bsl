...
// Очищает ответственного в документе.
// 
// Параметры:
//  Объект - ДокументОбъект - Учетный документ.
//
Процедура ОчиститьОтветственногоВДокументе(Объект) Экспорт
	...
	УникальныйИдентификаторСлужебный = Новый УникальныйИдентификатор("aa00559e-ad84-4494-88fd-f0826edc46f0");
	СтандартнаяСсылка = Справочники.Пользователи.ПолучитьСсылку(УникальныйИдентификаторСлужебный);
...
// Параметры заполнения менеджера.
// 
// Возвращаемое значение:
//   Структура - Параметры заполнения менеджера:
//     * Перезаполнить - Булево - 
//     * МенеджерИмя - Строка - 
//     * ЗаполнитьПодразделение - Булево - 
//     * ПодразделениеИмя - Строка - 
//
Функция ПараметрыЗаполненияМенеджера() Экспорт
	
	Параметры = Новый Структура;
	Параметры.Вставить("Перезаполнить", Истина);
	Параметры.Вставить("МенеджерИмя", "Менеджер");
	Параметры.Вставить("ЗаполнитьПодразделение", Ложь);
	Параметры.Вставить("ПодразделениеИмя", "Подразделение");
	
	Возврат Параметры;
	
КонецФункции

// Устанавливает менеджера из документа основания, по статистике, текущим пользователем.
// 
// Параметры:
//   Объект - ДокументОбъект, ДанныеФормыСтруктура - заполняемый объект.
//   ДокументОснование - ДокументСсылка, Неопределено - документ основание.
//   ПараметрыЗаполнения - Структура, Неопределено - См. ПараметрыЗаполненияМенеджера
// 
Процедура УстановитьМенеджера(Объект, ДокументОснование = Неопределено, ПараметрыЗаполнения = Неопределено) Экспорт
	
	Если ПараметрыЗаполнения = Неопределено Тогда
		ПараметрыЗаполнения = ПараметрыЗаполненияМенеджера();
	КонецЕсли;
	
	Перезаполнить = ПараметрыЗаполнения.Перезаполнить;
	МенеджерИмя = ПараметрыЗаполнения.МенеджерИмя;
	ЗаполнитьПодразделение = ПараметрыЗаполнения.ЗаполнитьПодразделение;
	ПодразделениеИмя = ПараметрыЗаполнения.ПодразделениеИмя;
	
	Если ЗначениеЗаполнено(Объект[МенеджерИмя]) И Не Перезаполнить Тогда
		Возврат;
	КонецЕсли;
	
	ЗначениеДоИзменений = Объект[МенеджерИмя];
	МенеджерУстановлен = Ложь;
	
	// Менеджер из документа основания
	Если ЗначениеЗаполнено(ДокументОснование) Тогда
		УстановитьПривилегированныйРежим(Истина);
		МенеджерОснования = Неопределено;
		Если ОбщегоНазначения.ЕстьРеквизитОбъекта(МенеджерИмя, ДокументОснование.Метаданные()) Тогда
			МенеджерОснования = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование, МенеджерИмя);
		ИначеЕсли ОбщегоНазначения.ЕстьРеквизитОбъекта("Ответственный", ДокументОснование.Метаданные()) Тогда
			МенеджерОснования = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование, "Ответственный");
		КонецЕсли;
		Если ЗначениеЗаполнено(МенеджерОснования) Тогда
			Объект[МенеджерИмя] = МенеджерОснования;
			МенеджерУстановлен = Истина;
		КонецЕсли;
		УстановитьПривилегированныйРежим(Ложь);
	КонецЕсли;
	
	// Заполним менеджера по статистике
	Если Не МенеджерУстановлен Тогда
		
		СсылкаДляСтатистики = Объект.Ссылка;
		РазрезыСбораСтатистики = Новый Массив;
		РеквизитыДляОтбора = Новый Структура;
		
		Если ОбщегоНазначения.ЕстьРеквизитОбъекта("Партнер", СсылкаДляСтатистики.Метаданные())
			И ЗначениеЗаполнено(Объект.Партнер) Тогда
			РазрезыСбораСтатистики.Добавить("Партнер");
			РеквизитыДляОтбора.Вставить("Партнер", Объект.Партнер);
		КонецЕсли;
		Если ОбщегоНазначения.ЕстьРеквизитОбъекта("Соглашение", СсылкаДляСтатистики.Метаданные())
			И ЗначениеЗаполнено(Объект.Соглашение) Тогда
			РазрезыСбораСтатистики.Добавить("Соглашение");
			РеквизитыДляОтбора.Вставить("Соглашение", Объект.Соглашение);
		КонецЕсли;
		Если ОбщегоНазначения.ЕстьРеквизитОбъекта("Договор", СсылкаДляСтатистики.Метаданные())
			И ЗначениеЗаполнено(Объект.Договор) Тогда
			РазрезыСбораСтатистики.Добавить("Договор");
			РеквизитыДляОтбора.Вставить("Договор", Объект.Договор);
		КонецЕсли;
		Если ОбщегоНазначения.ЕстьРеквизитОбъекта("Организация", СсылкаДляСтатистики.Метаданные())
			И ЗначениеЗаполнено(Объект.Организация) Тогда
			РазрезыСбораСтатистики.Добавить("Организация");
			РеквизитыДляОтбора.Вставить("Организация", Объект.Организация);
		КонецЕсли;
		
		Если РазрезыСбораСтатистики.Количество() > 0 Тогда
			ОписаниеРеквизитов = Новый Структура;
			Параметры = ЗаполнениеОбъектовПоСтатистике.ПараметрыЗаполняемыхРеквизитов();
			Параметры.РазрезыСбораСтатистики.ИспользоватьТолькоЗаполненные = СтрСоединить(РазрезыСбораСтатистики, ", ");
			...
			ЗаполнениеОбъектовПоСтатистике.ДобавитьОписаниеЗаполняемыхРеквизитов(ОписаниеРеквизитов, МенеджерИмя, Параметры);
			ЗаполняемыеРеквизиты = ЗаполнениеОбъектовПоСтатистике.ПолучитьЗначенияРеквизитов(
				СсылкаДляСтатистики, ОписаниеРеквизитов, РеквизитыДляОтбора);
				
			Если ЗначениеЗаполнено(ЗаполняемыеРеквизиты[МенеджерИмя]) Тогда
				Объект[МенеджерИмя] = ЗаполняемыеРеквизиты[МенеджерИмя];
				МенеджерУстановлен = Истина;
			КонецЕсли;
		КонецЕсли;
	...
	Если Не МенеджерУстановлен Тогда
		Если Пользователи.ЭтоСеансВнешнегоПользователя() Тогда
			Объект[МенеджерИмя] = СамообслуживаниеСервер.ОсновнойМенеджерВнешнегоПользователя(Пользователи.АвторизованныйПользователь());
		Иначе
			Объект[МенеджерИмя] = Пользователи.ТекущийПользователь();
		КонецЕсли;
	КонецЕсли;
	
	// Подчиненные реквизиты
	Если ЗначениеДоИзменений <> Объект[МенеджерИмя] И ЗаполнитьПодразделение Тогда
		Объект[ПодразделениеИмя] = ЗначениеНастроекПовтИсп.ПодразделениеПользователя(Объект[МенеджерИмя]);
	КонецЕсли;
	
КонецПроцедуры

// Заполняет список выбора менеджера.
// 
// Параметры:
//  ЭлементМенеджер - ПолеФормы - Элемент формы, в котором указывается Менеджер.
//  Объект - ДанныеФормыСтруктура - Объект, реквизиты которого используются при заполнении.
//
Процедура ЗаполнитьСписокВыбораМенеджера(ЭлементМенеджер, Объект) Экспорт
	
	СписокВыбора = ЭлементМенеджер.СписокВыбора;
	СписокВыбора.Очистить();
	
	Реквизиты = Новый Структура("Партнер, Соглашение, Договор");
	ЗаполнитьЗначенияСвойств(Реквизиты, Объект);
	
	УстановитьПривилегированныйРежим(Истина);
	
	Если ЗначениеЗаполнено(Реквизиты.Партнер) Тогда
		Менеджер = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Реквизиты.Партнер, "ОсновнойМенеджер");
		Если ЗначениеЗаполнено(Менеджер) Тогда
			СписокВыбора.Добавить(Менеджер,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1 (%2)", Менеджер, НСтр("ru = 'Партнер'")));
		КонецЕсли;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Реквизиты.Соглашение) Тогда
		Менеджер = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Реквизиты.Соглашение, "Менеджер");
		Если ЗначениеЗаполнено(Менеджер) Тогда
			СписокВыбора.Добавить(Менеджер,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1 (%2)", Менеджер, НСтр("ru = 'Соглашение'")));
		КонецЕсли;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Реквизиты.Договор) Тогда
		Менеджер = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Реквизиты.Договор, "Менеджер");
		Если ЗначениеЗаполнено(Менеджер) Тогда
			СписокВыбора.Добавить(Менеджер,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1 (%2)", Менеджер, НСтр("ru = 'Договор'")));
		КонецЕсли;
	КонецЕсли;
	...
	Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1 (%2)", ТекущийПользователь, НСтр("ru = 'Текущий пользователь'"));
	СписокВыбора.Добавить(ТекущийПользователь, Представление);
	
	Менеджеры = ОбщегоНазначенияКлиентСервер.СвернутьМассив(СписокВыбора.ВыгрузитьЗначения());
	Если Менеджеры.Количество() = 1 Тогда
		СписокВыбора.Очистить();
		СписокВыбора.Добавить(Менеджеры[0]);
	КонецЕсли;
