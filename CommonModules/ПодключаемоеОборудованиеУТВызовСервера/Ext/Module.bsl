...
Функция ДанныеФискальнойОперацииСУчетомЧекаККМКоррекции(ДокументОснование) Экспорт
	...
	Запрос.УстановитьПараметр("ЧекККМ", ДокументОснование);
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		ДанныеФискальнойОперации = ОборудованиеЧекопечатающиеУстройства.ДанныеФискальнойОперации(Выборка.Ссылка);
	КонецЕсли;
