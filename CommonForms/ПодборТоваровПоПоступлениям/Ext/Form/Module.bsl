...
Процедура ЗаполнитьТаблицуТоваров()
	...
	Иначе
		...
		ВалютыПоступления = Новый Соответствие;
		Для каждого Строка Из ТаблицаТоваровВыгрузка Цикл
			
			НоваяСтрока = ТаблицаТоваров.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Строка);
			ОбрабатываемыеСтроки.Добавить(НоваяСтрока);
			ВалютыПоступления.Вставить(НоваяСтрока, Строка.ВалютаПоступления);
		...
		Для каждого НоваяСтрока Из ОбрабатываемыеСтроки Цикл
			ВалютаПоступления = ВалютыПоступления.Получить(НоваяСтрока);
			
			Если Валюта <> ВалютаПоступления Тогда
				КурсНовойВалюты = РаботаСКурсамиВалютУТ.ПолучитьКурсВалюты(Валюта, Дата, БазоваяВалюта);
				КурсСтаройВалюты = РаботаСКурсамиВалютУТ.ПолучитьКурсВалюты(ВалютаПоступления, Дата, БазоваяВалюта);
